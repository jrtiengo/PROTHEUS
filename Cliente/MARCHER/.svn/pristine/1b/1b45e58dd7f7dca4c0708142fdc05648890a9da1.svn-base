...
Monta Browse CRUD MVC da tabela de Configuracao de Etiquetas 						
@type function
@Sample	  U_ZEBRACFG()
...
//-------------------------------------------------------------------------------+
// Valida a existencia de funcoes e tabelas para a utilizaóóo deste configurador |
//-------------------------------------------------------------------------------+
	If !( FindFunction("U_MSCBPrinter") )
		FwAlertWarning("O configurador exige a inclusao da rotina U_MSCBPrinter. " + CRLF + ;
			"Conte o suporte Protheus!","ZebraCfg")
		Return( .F. )
	ElseIf ( !AliasInDic("PDI") .Or. !AliasInDic("PDJ") .Or. !AliasInDic("PDK") )
		FwAlertWarning("Tabelas utilizadas neste configurador nao foram incluidas." + CRLF + ;
			"Tabelas: PDI\PDJ\PDK " + CRLF + ;
...
//-- Faz a inclusao dos 3 modelos de exemplo
	FWMsgRun(, {|| GravaModel() },"Modelo de Etiqueta","Validando modelo..." )
...
// Define as Colunas
	...
	oSize:AddObject( "CABECALHO",(oSize:aWindSize[4]),(oSize:aWindSize[3]) , .F., .F. ) // Nóo dimensionavel
	oSize:aMargins 	:= { 3, 3, 3, 3  } 	// Espaco ao lado dos objetos 0, entre eles 3
	oSize:lProp 		:= .F. 			// Proporcional
	oSize:Process() 	   				// Dispara os calculos


	DEFINE MSDIALOG oDlg TITLE OemToAnsi( "Configurador Etiquetas" ) ;
...
// Cria o conteiner onde seróo colocados os paineis
...
//Cria os paineis onde serao colocados os browses
	...
	oBrowse:SetDescription("Configurador Etiquetas")

	oBrowse:SetColumns( GetColumns() )

	oBrowse:DisableReport()
	oBrowse:SetUseFilter() // Habilita a utilizaóóo do Filtro de registros
	oBrowse:SetLocate() // Habilita a Localizaóóo de registros
	oBrowse:SetSeek() // Habilita a Pesquisa de registros
	oBrowse:SetDBFFilter()
// Define aóóo na troca de linha (Change)
	oBrowse:SetChange({|oBrowse| ChangePict(oBrowse) })

	oBrowse:Activate()


	nAltura		:= (oPanelDown:nHeight/2) - 05			//Altura padróo dos paineis
	nLargura	:= (oPanelDown:nWidth/2) - 10			//Largura padróo dos paineis
//310 x 260 
...
Static Function MenuDef(nOper)
	...
	Else
		...
		cTitulo     := "Configurador Etiquetas"
		cPrograma   := 'ZEBRACFG'
		nOperation  := nOper
...
//-- Descricao de cada submodelo (componentes)
	oModel:SetDescription("Modelo Configurador Etiquetas")
...
// oStruPDI:RemoveField("PDI_MEMO")
	oStruPDI:RemoveField("PDI_FILIAL")
//-- PDJ
	oStruPDJ:RemoveField("PDJ_FILIAL")
	oStruPDJ:RemoveField("PDJ_CODPDI")
//-- PDK
	oStruPDK:RemoveField("PDK_FILIAL")
...
//--
	...
	oView:CreateHorizontalBox( 'ID_ABA02_SUPERIOR'   , 35,,, 'PASTA', 'ABA02' )
	oView:CreateHorizontalBox( 'ID_ABA02_INFERIOR'   , 65,,, 'PASTA', 'ABA02' )
...
// Relaciona o identificador (ID) da View com o "box" para exibição
	...
	oView:SetDescription( "Modelo Configurador Etiquetas" )
...
//-- Para nóo tirar a ordem dos registros  
...
Static Function ActiveModel(oModel)
	...
	If nOperation == MODEL_OPERATION_UPDATE .Or. nOperation == MODEL_OPERATION_DELETE
		...
		If PDI->PDI_PROPRI <> "U"
			Help( ,, 'Help',"ZebraCfg", "Este modelo de etiqueta nÃo permite Alteração e Exclusão!", 1, 0 )
			lRet := .F.
		EndIf
...
//-- Gatilha os parametros no grid PDK 
    ...
    */
		...
		Case cTipo == "20"
			...
			oGridPDK:SetValue("PDK_VALOR"   , "10")
			oGridPDK:SetValue("PDK_TIPO"    , "C")
		...
		Case cTipo == "30"
			...
			// Compacta			Array of Record			Parámetro fora de uso
			oGridPDK:AddLine()
			oGridPDK:SetValue("PDK_PARAM"   , "Parámetro lCompacta fora de uso")
			oGridPDK:SetValue("PDK_VALOR"   , ".F.")
			oGridPDK:SetValue("PDK_TIPO"    , "L")
...
// //AddLine foróa um refresh no grid, os aSizes removem a nova linha em branco.
// oSubModel:AddLine()
// ASIZE(oSubModel:aDataModel, 1)
// ASIZE(oSubModel:aCols, 1)
...
//     //-- Exclui tudo menos a primeira que nao ó permitido.
//     If nI > 1
//         oSubModel:GoLine(nI)
...
Chama a rotina para montagem do bloco de codigo. Bloco com as funcoes que desenham a atiqueta.  						
@type function
@author	  Lucas.Brustolin
@since	  01/10/2019
@version 12.1.27
	...
	If nOperation ==  MODEL_OPERATION_INSERT .Or. nOperation == MODEL_OPERATION_UPDATE
		...
		If ( lRet := CreateBlock( oModel, @cMemo ) )
			If !( lRet := oModel:GetModel("PDIMASTER"):SetValue("PDI_MEMO",cMemo  ) )
				Help( ,, 'Help',"ZebraCfg", "Não foi possível gravar a configuração da etiqueta!", 1, 0 )
			EndIf
		Else
			Help( ,, 'Help',"ZebraCfg", "Não foi possível gerar o corpo da etiqueta. Verifique a parametrização!", 1, 0 )
		EndIf
...
Cria bloco com as funcoes da atiqueta.
@type function
@author	  Lucas.Brustolin
@since	  01/10/2019
@version 12.1.27
...
// ---------------------------------------------------------+
	For n1 := 1 To oGridPDJ:Length()
		...
		If !( oGridPDJ:IsDeleted() )
			...
			For n2 := 1 To oGridPDK:Length()
				...
				If !( oGridPDK:IsDeleted() )
					...
					If !Empty( cFormul )
						...
						cVariaveis  += " __ZPAR" + cValToChar(n1) + cValToChar(n2) + " := &( '" + cFormul + "' ) " + CRLF
					...
					Else
						...
						If !Empty(cConteudo)
							...
							ElseIf oGridPDK:GetValue("PDK_TIPO") == "I" //-- Tipo Imagem - Char Texto ()
								...
								//-- Nome do imagem + extensóo
								If aScan(aImage, cConteudo) == 0
									aAdd( aImage, cConteudo  )
								EndIf

								//-- Nome do imagem  sem extensóo
								nPos := At(".",cConteudo)
								nPos -= 1
			...
			//-----------
			cCorpo      += cFuncao + "(" + cParametro + ")" + CRLF
			cFuncao     := ""
			cParametro  := ""
	...
	If !Empty(cCorpo)
		...
		cMemo += " __oPrint := MSCBPrinter():New() " + CRLF

		//-- - Configura Impressora
		cMemo += " __oPrint:MSCBPRINTER( '" + cModelo + "','" + cPorta + "',,,.F.,,,,) " + CRLF

		//-- Seta ou visualiza o status do sistema com a impressora
		cMemo += " __oPrint:MSCBCHKStatus(.F.) " + CRLF


		For n1 := 1 To Len( aImage )
			cImage += " __oPrint:MSCBLOADGRF('" + aImage[n1] + "')" + CRLF
		Next
		...
		cMemo += " __oPrint:MSCBBEGIN(1,6) " + CRLF
		...
		cMemo += " __oPrint:MSCBEND() " + CRLF

		//-- Finaliza a conexão com a impressora
		cMemo += " __oPrint:MSCBClosePrinter() " + CRLF

	EndIf


	cAux  := cVariaveis + CRLF + cMemo
	cMemo := cAux
...
/*/{Protheus.doc} DePara 
Faz o De \ Para. Recebe o codigo da funcao e devolve sua Syntax.  
@type function
@author	  Lucas.Brustolin
@since	  01/10/2019
@version 12.1.27
...
Executa a impressao da etiqueta
@type function
@author	  Lucas.Brustolin
@since	  01/10/2019
@version 12.1.27
...
User Function IMPZEBRA(nModo)
	...
	If Select("PDI") > 0
		...
		If nModo == IMP_SPOOL_PREVIEW
			...
			If FwAlertYesNo("Confirma a impressão da etiqueta?","ZebraCfg")

				FWMsgRun(, {|| __lRet := ExecBloco(PDI->PDI_MEMO,@cErro) },"Etiqueta Zebra","Imprimindo..." )
		...
		ElseIf nModo == IMP_PREVIEW
			...
			__lRet := ExecBloco(cPDIMEMO,@cErro,IMP_PREVIEW)
...
Executa o bloco de codigo da etiqueta, macro execução.
@type function
@author	  Lucas.Brustolin
@since	  01/10/2019
@version 12.1.27
/*/
Static Function ExecBloco(cMemoCod,cErro,nModo)
	...
	aLinha := StrTokArr(cMemoCod,CRLF)
	...
	If !Empty(__cErroBlock)
		...
		cErro           := __cErroBlock
        MsgAlert("Erro na fórmula: " + cErro, "Erro !!")
		__cErroBlock    := ""
		lRet            := .F.
	Else
		cPnlCmdI := cRetM
...
Tratamento de erro do bloco de codigo.
@type function
@author	  Lucas.Brustolin
@since	  01/10/2019
@version 12.1.27
...
Rotina executada quando o campo recebe foco. Chama janela para selecao de imagem.
@type function
@author	  Lucas.Brustolin
@since	  01/10/2019
@version 12.1.27
...
Exibe a janela para a escolher a imagem do logo da etiqueta
@type function
@author	  Lucas.Brustolin
@since	  01/10/2019
@version 12.1.27
...
// ---------------------------------------------+
	If !ExistDir(cFolderSrv)
		If ( MakeDir(cFolderSrv) <> 0 )
			cFolderSrv := ""
			MsgAlert("Nóo foi possóvel criar o diretório padróo de imagens. Erro: " + cValToChar( FError() ) )
		EndIf
	EndIf
...
// ---------------------------------------------+
	If !Empty( cImagem )
		...
		If (lRet := File( cImagem ) )
			...
			EndIf
		Else
			FWAlertHelp("Arquivo de imagem não localizado!","Selecione uma imagem valida.","ZebraCfg")
		EndIf
	EndIf
...
/*/{Protheus.doc} WebPreview 
Adiciona o componente para exibição da pagina Web. (Preview)
@type function
@author	  Lucas.Brustolin
@since	  01/10/2019
@version 12.1.27
...
Insere codigo ZPL na pagina web Preview
@type function
@author	  Lucas.Brustolin
@since	  01/10/2019
@version 12.1.27
...
Faz a troca das imagens ao navegar pelas linha do Browse
@type function
@author	  Lucas.Brustolin
@since	  01/10/2019
@version 12.1.27
...
Checa a existencia das pastas\arquivos 
@type function
@author	  Lucas.Brustolin
@since	  01/10/2019
@version 12.1.27
	...
	For nI := 1 To Len( aDiretorio )
		...
		If !ExistDir( cPath )
			If ( MakeDir(cPath) <> 0 )
				...
				If ( lRet )
					cErro   := "Nao foi possivel criar o diretorio padrao. Erro: " + cValToChar( FError() ) + CRLF
				EndIf

				cErro   += Lower(cPath) + CRLF
				lRet    := .F.
			EndIf
		EndIf
	Next
		...
		For nI := 1 To Len(aFile)
			...
			If File(cArquivo)
				If !( CpyS2T( cArquivo, cPath, .T.) )
					...
					cErro   := "Não foi possível copiar o arquivo: " + cArquivo + " para: " + cPath + " . Verifique se voce possui acesso de escrita"
					lRet    := .F.
					Exit
				EndIf
			Else
				cErro   := "Não foi localizar o arquivo: "+ cArquivo + " . Contate o administrador do sistema"
				lRet    := .F.
				Exit
			EndIf
		Next
...
Executa aóóo no click da Folder. Carrega pagina web ao clicar na folder corresponde.
@type function
@author	  Lucas.Brustolin
@since	  01/10/2019
@version 12.1.27
...
Static Function ValidFolder(cFolderID, nOldSheet, nSelSheet)
	...
	// ------------------------------------------------------------------------+
	//  Apóo ao clicar na pasta PreView - Carrega pagina com o novo codigo ZPL |
	// -------------------------------------------------------------------------+
	If( nSelSheet == 3 .And. .NOT. lZPLView )

		Help(" ",1,"ZEBRACFg",,"O Preview só estó disponivel para modelos de impressora Zebra",3,1)
		lRet := .F.
...
Grava os modelos de etiqueta 000001,000002 e 000003. 
@type function
@author	  Lucas.Brustolin
@since	  01/10/2019
@version 12.1.27
/*/
Static Function GravaModel(cPK, aData, cError)
	...
	For nI := 1 To Len(aData)
		...
		If PDI->( !DbSeek(xFilial("PDI") + aData[nI] ) )
			...
			If ( lRet .And. !Empty(cData) )
				...
				If lRet
					If oModel:lModify // Verifico se o modelo sofreu alguma alteraóóo
...
@description		: Verifica se o modelo de impressora configurado ó ZEBRA.
@author	  Lucas.Brustolin
@since	  22/11/2019
@version 12.1.27
/*/
