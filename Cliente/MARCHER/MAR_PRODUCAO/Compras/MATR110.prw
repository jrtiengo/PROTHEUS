...
/*/
User Function _Matr110( cAlias, nReg, nOpcx )            
...
//alert(cAlias+', '+cvaltochar(nReg)+', '+cvaltochar(nOpcx))  
//alert(FunName())
...
If lTRepInUse
	...
	oReport:= ReportDef(nReg, nOpcx) //aqui
	oReport:PrintDialog()
Else
	//alert('2')
...
Local cMsgPC1 := GetMv('LE_MSGPC1')
Local cMsgPC2 := GetMv('LE_MSGPC2')
Local cMsgPC3 := GetMv('LE_MSGPC3')
Local cMsgPC4 := GetMv('LE_MSGPC4')
	...
	//Incluida validação para os pedidos de compras por item do pedido  (IP/alçada)			
		...
		If !lNewAlc
			...
			If SC7->C7_TIPO == 1 .OR. SC7->C7_TIPO == 3
				...
				if( alltrim(cMsgPC1) <> '')
					oReport:PrintText('NOTAS: '+cMsgPC1,,050 )
//					oReport:PrintText(cMsgPC1,,100 ) //"NOTA: So aceitaremos a mercadoria se na sua Nota Fiscal constar o numero do nosso Pedido de Compras."
				else
				...
				if( alltrim(cMsgPC2) <> '')
					oReport:PrintText(cMsgPC2,,150 ) 
				EndIf
				if( alltrim(cMsgPC3) <> '')
					oReport:PrintText(cMsgPC3,,150 ) 
				EndIf
				if( alltrim(cMsgPC4) <> '')
					oReport:PrintText(cMsgPC4,,200 ) 
				EndIf
...
_cAliasX3 := GetNextAlias()
OpenSXs(Nil,Nil,Nil,Nil,cEmpAnt,_cAliasX3,"SX3",Nil,.F.)
lOpen := Select(_cAliasX3) > 0
if (lOpen)
	dbSelectArea(_cAliasX3)
	(_cAliasX3)->(dbSetOrder(2)) //campo
	(_cAliasX3)->(dbSeek("C7_CC"))
	if (Found() )
		if ((_cAliasX3)->X3_TAMANHO == 9 )
			DifColCC := 11
			Tamanho   := "M"
		else
			lComp	  := .F.   // C.Custo c/ tamanho maior que 9, sempre PAISAGEM
...
If dbSeek(xFilial("SC7")+cPedido+cItemDe+Alltrim(cSequen))
	...
	While !Eof() .AND. SC7->C7_FILIAL+SC7->C7_NUM == xFilial("SC7")+cPedido .AND. ;
		...
		If (lOpen)
			...
			(_cSX32)->(dbSetOrder(1))
			(_cSX32)->(dbSeek("SC7"))
			While ( !(_cSX32)->(Eof()) .And. (_cSX32)->X3_ARQUIVO == "SC7" )
				cValid	:= StrTran(UPPER((_cSX32)->X3_VALID)," ","")
				cValid	:= StrTran(cValid,"'",'"')
				if ("MAFISREF" $ cValid)
					nPosRef  := AT('MAFISREF("',cValid) + 10
					cRefCols := Substr(cValid,nPosRef,AT('","MT120",',cValid)-nPosRef )
					// Carrega os valores direto do SC7.           
					MaFisLoad(cRefCols,&("SC7->"+ (_cSX32)->X3_CAMPO),nItem)
				endif
				(_cSX32)->(dbSkip())
			EndDo
			(_cSX32)->(DBCloseArea())
