...
User Function MARA090()
	...
	Private _dataIni      	:= 	DataValida(dDatabase-30,.F.)
	...
	DEFINE MSDIALOG oDlgOP TITLE "Gerenciamento de Ordens de Produção" FROM C(178),C(181) TO C(380),C(600) PIXEL
	...
	@ C(050),C(005) Say "Emissão de      " Size C(050),C(020) COLOR CLR_BLACK PIXEL OF oDlgOP
...
Static Function ImpProducao( cCliente, cLoja, cProduto, cProducao, cPedido, cAtendimento )
	...
	DEFINE MSDIALOG oDlg TITLE "Impressão Ordens de Produção" FROM 180,210 TO 700,1320 PIXEL
	...
	cParametros += 'Emissão De: '+DtoC(_dataIni)+' Até: '+DtoC(_dataFim)+'  '
	...
	@ 12,05 LISTBOX oList  FIELDS HEADER "" ,"Nº O.Produção", "Produto", "Descrição Produto",;
										"Emissao","Entrega Prevista","Entrega Real","Quantidade Original","Saldo a Entregar","Tipo OP","Recurso";
	...
	nCol += 060
	@ 245,nCol BUTTON "Estrutura X Saldo"  	Size 50,12 ACTION ( ChkEstrXQtd('BTN',Nil,Nil), EstrutXSaldo() ) 	Of oDlg PIXEL
	nCol += 060
...
Static Function SQLxListBox(oDlg)
	...
	cSql += "					SELECT TABSB1.* , 	ISNULL(SC5.C5_CLIENTE,'') AS 'C5_CLIENTE', ISNULL(SC5.C5_LOJACLI,'') AS 'C5_LOJACLI'," 	+ chr(13)
	cSql += "                  						ISNULL(SC5.C5_VEND1,'')   AS 'C5_VEND1'  , ISNULL(SA1.A1_NOME,'')    AS 'A1_NOME'  , " 	+ chr(13)
	cSql += "                  						ISNULL(SA1.A1_CGC,'')     AS 'A1_CGC'    , ISNULL(SA1.A1_MUN,'')     AS 'A1_MUN'   , " 	+ chr(13)
	cSql += "                  						ISNULL(SA1.A1_EST,'')     AS 'A1_EST'    " + chr(13)
	cSql += "                  	FROM (  " + chr(13) + chr(13)

	cSql += "							SELECT TABSC2.* , SB1.B1_DESC, SB1.B1_ESPECIF " + chr(13)
	cSql += "       					FROM ( "+chr(13)
	cSql += "       								SELECT 	SC2.C2_FILIAL , SC2.C2_NUM    , SC2.C2_ITEM   , SC2.C2_PRODUTO, " + chr(13)
	cSql += "												SC2.C2_LOCAL  , SC2.C2_QUANT  , SC2.C2_QUJE   , SC2.C2_UM     , " + chr(13)
	cSql += "												CASE WHEN SC2.C2_DATPRI  = '' THEN NULL    ELSE CAST(SC2.C2_DATPRI AS DATETIME) END as C2_DATPRI,	"+ chr(13)
	cSql += "												CASE WHEN SC2.C2_DATPRF  = '' THEN NULL    ELSE CAST(SC2.C2_DATPRF AS DATETIME)	END as C2_DATPRF,	"+ chr(13)
	cSql += "												CASE WHEN SC2.C2_EMISSAO = '' THEN NULL    ELSE CAST(SC2.C2_EMISSAO AS DATETIME)END as C2_EMISSAO, 	"+ chr(13)
	cSql += "												SC2.C2_CC, SC2.C2_STATUS , SC2.C2_SEQUEN, SC2.C2_ITEMGRD, SC2.C2_REVISAO, SC2.C2_ROTEIRO, SC2.C2_OBS, SC2.C2_PEDIDO, SC2.C2_TPOP, SC2.C2_RECURSO 	"+ chr(13)
	cSql += "										FROM " + RetSqlName("SC2") + " SC2 " + chr(13)
	cSql += "			  							WHERE	SC2.C2_FILIAL = '"+xFilial("SC2")+"' 	"+ chr(13)
	cSql += "			  							AND SC2.D_E_L_E_T_  =  ' '         				"+ chr(13)
	cSql += "										AND (SC2.C2_EMISSAO BETWEEN '"+DTOS(_dataIni)+"' AND '"+DTOS(_dataFim)+"' )  "  + chr(13)
	//cSql += "										AND SC2.C2_QUJE = 0 "  	+ chr(13)
	...
	EndIf
	cSql += "			 					) AS TABSC2, " + RetSqlName("SB1") + " SB1 " + chr(13) + chr(13)

	cSql += "							WHERE TABSC2.C2_PRODUTO   = SB1.B1_COD  " + chr(13)
	cSql += "							  AND SB1.D_E_L_E_T_ = ' ' "+ chr(13)+ chr(13)

	cSql += "			 			) AS TABSB1		LEFT JOIN " + RetSqlName("SC5") + " SC5 "+ chr(13)
	cSql += "										ON SC5.C5_NUM  	  =  TABSB1.C2_PEDIDO "+ chr(13)
	cSql += "										AND SC5.C5_FILIAL =  TABSB1.C2_FILIAL "+ chr(13)
	cSql += "										AND SC5.D_E_L_E_T_ = ' ' "+ chr(13)+ chr(13)

	cSql += "										LEFT JOIN " + RetSqlName("SA1") + " SA1 "+ chr(13)
	cSql += "										ON SA1.A1_COD  	=  	SC5.C5_CLIENTE  "+ chr(13)
	cSql += "										AND SA1.A1_LOJA	= 	SC5.C5_LOJACLI  "+ chr(13)
	cSql += "										AND SA1.D_E_L_E_T_ = ' ' "+ chr(13)+ chr(13)

	cSql += "			   ) AS TABFINAL " + chr(13)+ chr(13)
			

	If !Empty(cCliente)		// Filtra pelo Cliente
		cSql += " WHERE TABFINAL.C5_CLIENTE = '" + Alltrim(cCliente) + "' AND TABFINAL.C5_LOJACLI = '" + Alltrim(cLoja) + "'"  + chr(13)
	Endif

	cSql += " ORDER BY TABFINAL.C2_NUM, TABFINAL.C2_DATPRI " + chr(13)
	...
	While T_PRODUCAO->(!EOF())
		...
		xNomeTransp := ""
		If .NOT. T_FRETE->( EOF())
			xTransp     := T_FRETE->C5_TRANSP
			xNomeTransp := T_FRETE->A4_NOME
		EndIf
		...
		xCodVend  := ""
		xVendedor := ""
		If .NOT. T_VENDEDOR->( EOF())
			xCodVend  := T_VENDEDOR->A3_COD
			xVendedor := T_VENDEDOR->A3_NOME
		EndIf
		...
		xQtdPedido := If( .NOT. T_QUANTIDADE->( EOF()), T_QUANTIDADE->C6_QTDVEN , "")
		...
		Do Case
			Case AllTrim(T_PRODUCAO->C2_STATUS) == 'U'
				cStatusOP	:=	'Suspensa'
			Case AllTrim(T_PRODUCAO->C2_STATUS) == 'S'
				cStatusOP	:=	'Sacramentada'
			Case AllTrim(T_PRODUCAO->C2_STATUS) == 'N'
				cStatusOP	:=	'Normal'		
			OtherWise
				cStatusOP	:=	''
		EndCase
	...
	DbUseArea( .T., "TOPCONN", TcGenQry(,,_cEmp), "EMP", .T., .T. )

	_aEmpenho := {}
	DbSelectArea("EMP")
	DbGoTop()
	Do While !Eof()
			//																					0 AS 'D4_RESERV'  0 AS 'D4_RESPED'
		...
		If SB2->( DbSeek( cFilSB2 + EMP->D4_COD + EMP->D4_LOCAL, .F.) )

			nX := Len(_aEmpenho)
			// SB2->B2_QATU
			// SB2->B2_SALPEDI				// Qtde prevista p/ entrar

			//nQtdSaldo := SaldoSB2()		// SaldoSB2() == (SB2->B2_QATU - SB2->B2_QEMP)
			...
			If (SB2->B2_QATU - EMP->D4_QUANT) >= 0
				// SE EU RESERVAR O SALDO ELE NAO FICA NEGATIVO
				_aEmpenho[nX][06] := EMP->D4_QUANT						//	EMP->D4_RESERV
			Else
				// TENTO RESERVAR AGORA O SALDO DE PEDIDOS DE COMPRA
				If (SB2->B2_SALPEDI - EMP->D4_QUANT) >= 0				//	EMP->D4_RESPED
					// SE EU RESERVAR O SALDO ELE NAO FICA NEGATIVO
					_aEmpenho[nX][07] := EMP->D4_QUANT
				Else
					//SE FICOU NEGATIVO, PRECISA COMPRAR... ALIMENTO A POSIçãO 8 DO ARRAY
					_aEmpenho[nX][08] += EMP->D4_QUANT
				EndIf
			EndIf
...
Static Function I_PRODUCAO(aLista)
	...
	For nContar = 1 to Len(aLista)
		...
		If aLista[nContar,1]
			...
			//cDescCC	:= AllTrim(Posicione("CTT",1,xFilial("CTT")+SC2->C2_CC,"CTT_DESC01"))
			//oPrint:Say( _nLin, 0110, "Centro de Custo : " + AllTrim(SC2->C2_CC)+' '+cDescCC , oFont10x)
			//_cValid := ""
			//{SB1->B1_TIPE,SB1->B1_PRVALID,SB1->B1_DESC,SB1->B1_CODBAR}
			IF SB1->B1_TIPE == "H"
				_cValid := " HORA(s)"
			...
			Do While !Eof() .and. SD4->D4_OP = _cOP .AND. SD4->D4_FILIAL == cFilSD4
				...
				IF RIGHT(TRIM(SD4->D4_OP),3) == "001"
					//AADD(xITEM,{SD4->D4_COD,SD4->D4_OP,SD4->D4_OPORIG,SD4->D4_QTDEORI})
					
					oPrint:Say( _nLin, 0100, SD4->D4_COD           , oFont10y)
					oPrint:Say( _nLin, 0400, LEFT(SB1->B1_DESC,30) , oFont10y)
					
					IF LEFT(SD4->D4_COD,1) <> "3"
						oPrint:Say( _nLin, 1200, TRANSFORM((SD4->D4_QUANT/SC2->C2_QUANT)*100, "999.9999" )+"%" , oFont10y)
						TOTPER += ((SD4->D4_QUANT/SC2->C2_QUANT)*100)
						TOTQTD += SD4->D4_QUANT
					ENDIF
				...
				IF dDatabase >= SG1->G1_INI .and. dDatabase <= SG1->G1_FIM
					...
					IF EOF()
						...
						If SB1->B1_FANTASM == 'S'
							...
							IF LEFT(SD4->D4_COD,1) <> "3"
								oPrint:Say( _nLin, 1200, TRANSFORM((SG1->G1_QUANT/_nQb)*100, "999.9999" )+"%" , oFont10y)
								TOTPER += (SG1->G1_QUANT/_nQb)*100
			...
			_nLin += 25
			oPrint:Say( _nLin, 1200, TRANSFORM(TOTPER, "@E 999.9999"   )+"%" , oFont10x)
			...
			Do While xFilial("SG2")+SC2->C2_PRODUTO+SC2->C2_ROTEIRO == SG2->(G2_FILIAL+G2_PRODUTO+G2_CODIGO) //G2_OPERAC)
			...
			For _nContaSac := 1 to nVias
				...
				Do While !Eof() .and. SD4->D4_OP == _cOP .AND. SD4->D4_FILIAL == cFilSD4
					...
					IF RIGHT(TRIM(SD4->D4_OP),3) == "001"
						...
						IF LEFT(SD4->D4_COD,1) <> "3"
							oPrint:Say( _nLin, 0100, SD4->D4_COD           , oFont12b) //oFont10y)
							oPrint:Say( _nLin, 0400, LEFT(SB1->B1_DESC,30) , oFont12b) //oFont10y)
						
							oPrint:Say( _nLin, 1280, TRANSFORM((SD4->D4_QUANT/SC2->C2_QUANT)*100, "999.9999" )+"%" , oFont12b) //oFont10y)
							TOTPER += ((SD4->D4_QUANT/SC2->C2_QUANT)*100)
							TOTQTD += SD4->D4_QUANT //*_nPercx
		
							oPrint:Say( _nLin, 1580,TRANSFORM(SD4->D4_QUANT, "@E 99,999.9999" ) + " " + SB1->B1_UM , oFont12b) //oFont10y)
							oPrint:Say( _nLin, 1880, "____________" , oFont12b) //oFont10y)
				...
				oPrint:Say( _nLin, 0090, "(*) Estrutura sem empenho " , oFont08x)
				_nLin += 25
				oPrint:Say( _nLin, 1280, TRANSFORM(TOTPER, "@E 999.9999"   )+"%" , oFont10x)
...
Static Function I_Consulta(aLista)
	...
	For _n := 1 To Len(aLista)  
		...
		oPrint:Say( _nLin, 2430, cDtEmissao, oFont08)										//	EMISSAO
