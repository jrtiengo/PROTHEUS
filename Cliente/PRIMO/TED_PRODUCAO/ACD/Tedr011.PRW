...
Static Function ProcessI(Cabec1,Cabec2,Titulo,nLin)
    ...
    Cabec1 := "Fil.  Documento + Série  Tipo  Emissão     Valor Bruto      Cliente                                Transportador                    Placa  UF Des.  UF Ori.  Mens. Nota"
    ...
    While TMP->(!EOF())
        ...
        @nLin,006  PSAY TMP->F2_DOC + " - " + TMP->F2_SERIE
        @nLin,026  PSAY TMP->F2_TIPO
        @nLin,031  PSAY DtoC(StoD(TMP->F2_EMISSAO))
        @nLin,047  PSAY Transform(TMP->F2_VALBRUT,PesqPict("SF2","F2_VALBRUT",10))   
        if (TMP->F2_TIPO=="B" .or. TMP->F2_TIPO=="D")
            @nLin,060  PSAY TMP->F2_CLIENTE + "-" + TMP->F2_LOJA + " - " + Left(Posicione("SA2",1,xFilial("SA2") + TMP->F2_CLIENTE + TMP->F2_LOJA,"SA2->A2_NOME"),22) 
        else
            @nLin,060  PSAY TMP->F2_CLIENTE + "-" + TMP->F2_LOJA + " - " + Left(Posicione("SA1",1,xFilial("SA1") + TMP->F2_CLIENTE + TMP->F2_LOJA,"SA1->A1_NOME"),22) 
        endif
        
        @nLin,099  PSAY TMP->F2_TRANSP + " - "+Left(Posicione("SA4",1,xFilial("SA4")+TMP->F2_TRANSP,"SA4->A4_NOME"),22)
