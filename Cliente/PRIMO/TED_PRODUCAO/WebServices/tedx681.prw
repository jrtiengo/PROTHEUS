		...
		*/
		...
		Else
			...
			CASE !OPComEmp(cNumOP)
				...
				lOk := .F.
				cDescErro:="Critica: OF "+ AllTrim(STR(oJson:H6_OFINFOB)) +". (OP Protheus " + cNumOP + ") Não teve os empenhos/OPs intermediarias Criadas. Corrija a Estrutura do Produto. Exclua a OP. Re-Integre apontamento da OF."
				SetRestFault( 112, EncodeUTF8(cDescErro, "cp1252") )
				u_LogInteg(cRotina,'Erro',"SH6",alltrim(str(oJson:H6_OFINFOB)),'',cDescErro,time())
