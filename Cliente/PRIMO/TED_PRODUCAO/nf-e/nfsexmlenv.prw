		...
		If DbSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)	
           ...
           If cMvMsgTrib $ "1-3"
			...
			While !(cAliasSD2)->(Eof()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				ElseIf SuperGetMV("MV_JESCJUR",, .F.) // Integração com módulo SIGAPFS
				...
				If lAglutina
					If Len(aProd) > 0	
						If lUsaSF3	
							If Empty(Alltrim(SF3->F3_TRIBMUN))
								dbselectArea("SF3")
								dbsetOrder(5)//F3_FILIAL+F3_SERIE+F3_NFISCAL+F3_CLIEFOR+F3_LOJA+F3_IDENTFT
								DbSeek(xFilial("SF3")+(cAliasSD2)->D2_SERIE+(cAliasSD2)->D2_DOC+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA)
							EndIf	
							nX := aScan(aProd,{|x| x[24] == (cAliasSD2)->D2_CODISS .And. x[23] == IIF(SF3->(FieldPos("F3_TRIBMUN"))<>0,SF3->F3_TRIBMUN,"")})
						Else
...
static function prestacao( cMunPrest, cDescMunP, aDest, cMunPSIAFI )
	...
	Else
		...
		If SuperGetMV("MV_JESCJUR",, .F.) // Integração com módulo SIGAPFS
