...
//IF Altera == .t. .and. l103Class == .T. //.and. ogetdados:obrowse:nat == 1 .and. cFLAG == 0//Alteração,ópcao Classificacao, 1a linha Acols
	nCodOper:=Aviso("Classificação ...","Deseja Replicar a conta para todos os Itens da NF?",{"Sim","Não"},1)
	IF nCodOper == 1      // 1==Sim
		lRet := fRepCTA()
		MsgRun("Replicando a Conta, aguarde...","Replicação de Conta",{||  fRepCTA() })
	EndIf
//EndIF
...
**/
Static Function fRepCTA()

Local nX := 0
Local nCodOper:=0
Local cField := "D1_CONTA"
...
Local nPosD1CF   := gdFieldPos("D1_CF", aHeader )
Local nLin          := ogetdados:obrowse:nat
procregua( Len(Acols) )

	For nlin := 1 To Len(aCols) // ira partir da 2a. linha

		aCols[Nlin , nPosD1CTA] := M->D1_CONTA 
