...
If P_LDSRSALD .And. !(SRA->RA_CATFUNC == "S" .AND. SRA->RA_TIPOPGT == "S" )
	If !EMPTY(ACODFOL[1399,1])
		n_X1 += GPER140Sum(1,1,"1399",,2,1)
	EndIf
	n_X1  := n_X1 / (SRA->RA_HRSMES/30)
EndIf
...
Else
	...
	For nY := 1 to Len(aPer)
		...
		If !Empty(aVbAux)
			//Posiciona na SRR para identificar se a verba È informada.
			DbSelectArea("SRR")
			SRR->(DbSetOrder(RetOrder("SRR","RR_FILIAL+RR_MAT+RR_PERIODO+RR_ROTEIR+RR_SEMANA+RR_PD+RR_CC+RR_SEQ+DTOS(RR_DATA)")))
			If SRR->(DbSeek(SRG->(RG_FILIAL+RG_MAT+RG_PERIODO+RG_ROTEIR+RG_SEMANA)+aVbAux[ny][1]))
...
oPrint:say (nL+nTip+nPD,665,Iif(Alltrim(SRG->RG_TPAVISO) $ "N", "",  DtoC(SRG->RG_DTAVISO)), oFont10)
...
oPrint:say (nL+nTip+nPD+10,972+30,Iif(Alltrim(SRG->RG_TPAVISO) $ "N", "", DtoC(SRG->RG_DTAVISO)), oFont10)
