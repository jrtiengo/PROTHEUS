 ...
 * Início   : 23junho2012
 * Descrição: Emissao de Indicador dos Custos dos Veiculos SIRTEC - GERAL - CONTA ANO MES
 */
User Function RINDCVGA()
...
Private cString, Titulo, cDesc1, cDesc2, cDesc3, cQtde ,vUnit, vTotal, cPedido ,Tamanho, aReturn, NomeProg, nLastKey, cQtdeV1, cQtdeV2, ;
		...
		cQtdeaV1, cQtdeaV2, cQtdeaVo, cCliente, cQtdeMC, cQtdeFC, cQtdeOC, vTotalC, qLinhas, cPerg := PadR('RINDCVGA',10)
...
@ 052,020 Say 'GERAL -EMPRESA - ANO '       Size 175, 15
...
aAdd( aArq , { 'ANOMES'		, 'C', 06, 0 } )
aAdd( aArq , { 'VALORD'		, 'N', 15, 2 } )
aAdd( aArq , { 'VALORC'		, 'N', 15, 2 } )
aAdd( aArq , { 'DESCEND'	, 'N', 15, 2 } )
...
oTempTab:AddIndex("01", { aArq[1]+aArq[2] } ) //EMPRESA+ANOMES
oTempTab:Create()
...
While !eof() ;
   ...
   IF  CT2->CT2_DC == '4'
    ...
    IF MV_PAR03 == 1
        ...
        IF !Alltrim(CT2->CT2_CREDIT)$cCTAS  .AND. CT2->CT2_DC == '2'  .OR. Alltrim(CT2->CT2_CREDIT)$'3122301002'  .AND. CT2_CT2_DC == '2'
			dbSelectArea( 'CT2' )
			dbSkip()
			Loop
		EndIf
	...
	EndIf
     ...
     cANOMES := SUBSTR(DTOS(CT2->CT2_DATA),1,4)+SUBSTR(DTOS(CT2->CT2_DATA),5,2)
    ...
    IF dbSelectArea( 'CT1' )
		...
		// Estava sendo feito um dbseek na tabela _cTRB, sem posicionar na mesma. Mauro - Solutio. 23/01/2022.
		dbSelectArea(_cTRB)
		If ( !DBSEEK('S I R T E C'+PadR(cANOMES,6), .F.))
			(_cTRB)->(DBAPPEND())
			(_cTRB)->EMPRESA     := 'S I R T E C'
			(_cTRB)->ANOMES      := PadR(cANOMES,6)
		EndIf
    ENDIF
		...
		EndIf
		    ...
		    While SD1->( !Eof() ) .and. SD1->D1_FILIAL  == SE5->E5_FILIAL ;
		          ...
		          IF Alltrim(SD1->D1_CONTA)$cCTAS
					...
					cANOMES  := Subst(DTOS(SE5->E5_DATA),1,4) + SUBSTR(DTOS(SE5->E5_DATA),5,2)

                    If cFlagLeo == 0  .and.  cANOMES == '201205'
                            Valor := Valor + 0.005
                            cFlagLeo := 1
                    EndIF

				    IF  (_cTRB)->( !DBSEEK('S I R T E C'+PadR(cANOMES,6), .F.))
					    (_cTRB)->(DBAPPEND())
		       			(_cTRB)->EMPRESA     := 'S I R T E C'
			   			(_cTRB)->ANOMES      := PadR(cANOMES,6)
		    		ENDIF
...
while !EOF()
    ...
    IF  qLinhas == 0
		...
		oprn:say(0450,0200,'        A N O M E S     '  ,oFont5,100)
		oprn:say(0450,1500,'        V a l o r       % Partic. '  ,oFont5,100)
    EndIF
    IF cCTA <> (_cTRB)->ANOMES //(_cTRB)->CONTA
       cLinha := cLinha + 050
		oprn:say(cLinha,0000,Repl("_",1800),oFont5,100)
       cLinha := cLinha + 100
		oprn:say(cLinha,0000,'                       '+SUBSTR((_cTRB)->ANOMES,1,4)+'  /  '+SUBSTR((_cTRB)->ANOMES,5,2)+' - ' ,oFont5,100)
    	oprn:say(cLinha,01620, Transform( (_cTRB)->VALORD-(_cTRB)->VALORC , '@E 9999,999.99' ),oFont6,100)
    	nPerc :=   (((_cTRB)->VALORD-(_cTRB)->VALORC)*100)/nVALORT
    	oprn:say(cLinha,01920, Transform( nPerc , '@E 999.999999' ),oFont6,100)
        cCTA:= (_cTRB)->ANOMES //(_cTRB)->CONTA

		If mv_par04 == 1
           IF mv_par05 == 2
	        	AAdd(aDados, {(_cTRB)->ANOMES,(_cTRB)->VALORD-(_cTRB)->VALORC,MV_PAR01,MV_PAR02}) //adiciona aos dados
           EndIF
        EndIF
...
If  qLinhas == 0
       ...
       cLinha := cLinha + 100
		oprn:say(0450,0200,'        A N O M E S     '  ,oFont5,100)
		oprn:say(0450,1500,'        V a l o r       % Partic. '  ,oFont5,100)
		oprn:say(cLinha,0800, (_cTRB)->CT1DESC,oFont5,100)
	    cLinha := cLinha + 050
		oprn:say(cLinha,0000,Repl("_",1800),oFont5,100)
        cCTA:= (_cTRB)->ANOMES //(_cTRB)->CONTA
	    cLinha := cLinha + 050
...
oprn:say(cLinha,01500, Transform( vTotal , '@E 9,999,999,999.99' ),oFont5,100)
...
If mv_par04 == 1
  ...
  IF mv_par05 == 2
     aCabec := {"AnoMes","Valor","Data Inic","Data Final"} //Criação de um cabeçalho
  ElseIF mv_par03 == 2
  ...
  EndIF
    IF  mv_par03 == 1
		DlgToExcel({ {"ARRAY", "Indicadores Sirtec - Veículos - Regime de Competencia", aCabec, aDados} }) // utiiliza a função
    Else
		DlgToExcel({ {"ARRAY", "Indicadores Sirtec - Veículos - Regime de Caixa", aCabec, aDados} }) // utiiliza a função
    EndIF
EndIF
