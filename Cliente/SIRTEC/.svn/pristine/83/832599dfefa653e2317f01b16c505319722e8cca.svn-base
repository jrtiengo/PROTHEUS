...
User Function MTA105LIN()
  ...
  //Local _nCA		:= aScan(aHeader,{|x| Alltrim(x[2]) == "CP_XNUMCAP" })
    ...
    Local _nItemSA  := aScan(aHeader,{|x| Alltrim(x[2]) == "CP_ITEM"    })
    Local _nNumeSA  := aScan(aHeader,{|x| Alltrim(x[2]) == "CP_NUM"     })
	...
	Else
		...
		If !Empty(aCols[n][_nSEQFUNC])
		   ...
		   Else	                        
		      ...
		      kSAItem := Alltrim(aCols[n][_nNumeSA])  + "/" + Alltrim(aCols[n][_nItemSA])
			  ...
			  dbSetOrder(1)
     		  dbSeek(xFilial("ZZ4") + kEquipe)
      	      ...
      	      If ( ! Found() ) 
 			     MsgAlert("Problema na SA/Item (" + kSAItem + ") - Equipe: " + Alltrim(kEquipe) + " não está vinculada na tabela ZZ4 (Relaciona Técnico X Equipe)." ,"ATENÇÃO!")
				 _lRet	:= .F. 			
			  Else	
