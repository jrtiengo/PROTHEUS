		...
		While !(cAliasSD2)->(Eof()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
			...
			If ( Len(_aSX5) > 0 )
				If !lNFeDesc  .And. nCont == 1
					If  cCodmun == "3300704-3156700"
						cNatOper := If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(SubStr(_aSX5[1][4],1,55))),AllTrim(SubStr(_aSX5[1][4],1,55))) + cNatOper
	    			Else
						cNatOper += If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(SubStr(_aSX5[1][4],1,55))),AllTrim(SubStr(_aSX5[1][4],1,55)))    			
	    			EndIf
	    	    ElseIf nCont == 1 
	    	        If cCodmun == "3300704-3156700"
						cDescrNFSe := If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(SubStr(_aSX5[1][4],1,55))),AllTrim(SubStr(_aSX5[1][4],1,55))) + cNatOper
	    			Else
	...
	If DbSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
		...
		If SF1->(FieldPos("F1_INSS"))<>0 .and. SF1->F1_INSS>0
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				If (Len(_aSX5) > 0 )
					If (cCodmun == "3300704-3156700")
						cNatOper :=If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(SubStr(_aSX5[1][5],1,55))),AllTrim(SubStr(_aSX5[1][5],1,55))) + cNatOper
					Else
						cNatOper += If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(SubStr(_aSX5[1][5],1,55))),AllTrim(SubStr(_aSX5[1][5],1,55)))
                    EndIf
				EndIf
