...
cArqTxt   := GetSrvProfString("Startpath","") + "STCS002_R_" + MV_YCTPER + ".HTM"
//cArqTxt   := GetSrvProfString("Startpath","") + "STCS002_P_" + DtoS(Date()) + Time() + ".HTM"
...
ferase(cArqTxt)  // machado 19/08/20
Return
...
ConOut("STCS002 - " + DToS(Date()) + " - " + Time() + ": " + cMsg)
...
//Monta query
cQry:=EOL+" select 
cQry+=EOL+"   SE1.E1_FILIAL  as FILIAL
cQry+=EOL+"  ,SE1.E1_PREFIXO as PREFIXO
cQry+=EOL+"  ,SE1.E1_NUM     as NUMERO
cQry+=EOL+"  ,SE1.E1_PARCELA as PARCELA
cQry+=EOL+"  ,SE1.E1_TIPO    as TIPO
cQry+=EOL+"  ,SE1.E1_CLIENTE as CODIGO
cQry+=EOL+"  ,SE1.E1_LOJA    as LOJA
cQry+=EOL+"  ,SE1.E1_VENCTO  as VENCIMENTO
cQry+=EOL+"  ,SE1.E1_EMISSAO as EMISSAO
cQry+=EOL+"  ,SE1.E1_VALOR   as VALOR
cQry+=EOL+"  ,SE1.E1_NATUREZ as NATUREZA
cQry+=EOL+"  ,SED.ED_DESCRIC as DESCRICAO
cQry+=EOL+" from 
cQry+=EOL+"   !SE1! SE1
cQry+=EOL+"  ,!SED! SED
cQry+=EOL+" where 
cQry+=EOL+"      SE1.D_E_L_E_T_ = ''
cQry+=EOL+"  and SED.D_E_L_E_T_ = ''
cQry+=EOL+"  and SE1.E1_FILIAL  = !SE1.FILIAL!
cQry+=EOL+"  and SED.ED_FILIAL  = !SED.FILIAL!
cQry+=EOL+"  and SED.ED_CODIGO  = SE1.E1_NATUREZ
cQry+=EOL+"  and SE1.E1_SALDO > 0
cQry+=EOL+"  and SE1.E1_TIPO = 'PR '
cQry+=EOL+" order by 
cQry+=EOL+"  SE1.E1_FILIAL, SE1.E1_VENCREA

//Ajusta query
cQry := StrTran(cQry,'!SE1!'          ,RetSqlName('SE1'))
cQry := StrTran(cQry,'!SED!'          ,RetSqlName('SED'))
cQry := StrTran(cQry,'!SE1.FILIAL!'   ,ValToSql(xFilial('SE1')))
cQry := StrTran(cQry,'!SED.FILIAL!'   ,ValToSql(xFilial('SED')))

//Log de controle
//Memowrite('\STCS002-1.txt',cQry)
...
fWrite(nHdl,'			<TABLE WIDTH=100% height="250">'+cEOL)
fWrite(nHdl,'				<TR>'+cEOL)
fWrite(nHdl,'					<TH ALIGN="left" BGCOLOR="#202664"><FONT  SIZE="5" COLOR="#FFFFFF" FACE="verdana, arial, helvetica, times" >Processamento de Títulos Provisórios a Receber<BR></FONT></TH>'+cEOL)
fWrite(nHdl,'				</TR>'+cEOL)
fWrite(nHdl,'				<TR>'+cEOL)
fWrite(nHdl,'					<TD align="left" BGCOLOR="#FFFFFF"><FONT  SIZE="-3" COLOR="#201D84" FACE="verdana, arial, helvetica, times" ><BR>Voce esta recebendo o relatório de processamento da rotina de Provisão a Receber.<BR>A análise do relatório é necessária para compreensão dos saldos atuais.<BR></FONT></TD>'+cEOL)
fWrite(nHdl,'				</TR>'+cEOL)
fWrite(nHdl,'		</TABLE>'+cEOL)
fWrite(nHdl,'		<BR>'+cEOL)
...
cQry+=EOL+"  and SE1.E1_SALDO   > 0
cQry+=EOL+"  and SE1.E1_TIPO    = 'PR '
cQry+=EOL+"  and SE1.E1_NATUREZ like rtrim(!SE1.NATUREZA!) + '%'
cQry+=EOL+"  and SE1.E1_VENCREA between !SE1.DATA1! and !SE1.DATA2!
cQry+=EOL+"  and SE1.E1_PREFIXO+SE1.E1_NUM+SE1.E1_PARCELA <> !SE1.CHAVE!
cQry+=EOL+" order by 
cQry+=EOL+"   SE1.E1_FILIAL, SE1.E1_VENCREA
...
cQry := StrTran(cQry,'!SE1.FILIAL!'   ,ValToSql(xFilial('SE1')))
cQry := StrTran(cQry,'!SED.FILIAL!'   ,ValToSql(xFilial('SED')))
cQry := StrTran(cQry,'!SE1.CHAVE!'    ,ValToSql(cChave))
cQry := StrTran(cQry,'!SE1.NATUREZA!' ,ValToSql(cNatureza))
cQry := StrTran(cQry,'!SE1.DATA1!'    ,ValToSql(dInicial))
cQry := StrTran(cQry,'!SE1.DATA2!'    ,ValToSql(dFinal)) 

//Log de controle
//Memowrite('\STCS002-2.txt',cQry)
...
cQry+=EOL+"  and SED.ED_FILIAL  = !SED.FILIAL!
cQry+=EOL+"  and SED.ED_CODIGO  = SE1.E1_NATUREZ
cQry+=EOL+"  and SE1.E1_YATU    = ''
cQry+=EOL+"  and SE1.E1_VALOR   > 0
cQry+=EOL+"  and SE1.E1_TIPO    <> 'PR '
cQry+=EOL+"  and left(SE1.E1_TIPO,1) <> '-' 
cQry+=EOL+"  and SE1.E1_NATUREZ like rtrim(!SE1.NATUREZA!) + '%'
cQry+=EOL+"  and SE1.E1_VENCREA between !SE1.DATA1! and !SE1.DATA2!
cQry+=EOL+" order by 
cQry+=EOL+"   SE1.E1_FILIAL, SE1.E1_VENCREA
...
For i:=1 To Len(aTitulos)
	If SE1->(DbSeek(aTitulos[i,1]+aTitulos[i,2]+aTitulos[i,3]+aTitulos[i,4]+aTitulos[i,5]))
		Reclock("SE1",.F.)
			SE1->E1_VALOR := aTitulos[i,8]
			SE1->E1_SALDO := aTitulos[i,8]
			SE1->E1_VLCRUZ:= aTitulos[i,8]
		SE1->(MsUnlock())       
...
For i:=1 To Len(aOrigem)
	If SE1->(DbSeek(aOrigem[i,1]+aOrigem[i,2]+aOrigem[i,3]+aOrigem[i,4]+aOrigem[i,5]))
		Reclock("SE1",.F.)
			SE1->E1_YATU := 'S' 
		SE1->(MsUnlock())
...
Local EOL       := "<br />"
Local cTexto    := ""
Local MV_YMSPER := Alltrim(GetMv("MV_YMSPERR"))       
Local aInfo     := {} 
Local cInfRem,cInfDes,cInfAst,cInfMen,cInfAne,cInfFor

IF EMPTY(MV_YMSPER)
   Return   // MACHADO 19/08/20
EndIf   

cTexto:='<TABLE WIDTH=100% height="250">
cTexto+='	<TR>
cTexto+='		<TH ALIGN="left" BGCOLOR="#202664"><FONT  SIZE="5" COLOR="#FFFFFF" FACE="verdana, arial, helvetica, times" >Processamento de Títulos Provisórios a Receber<BR/></FONT></TH>
cTexto+='	</TR>
cTexto+='	<TR>
cTexto+='		<TD align="left" BGCOLOR="#FFFFFF"><FONT  SIZE="-3" COLOR="#201D84" FACE="verdana, arial, helvetica, times" >Voce esta recebendo o relatório de processamento da rotina de Provisão a Receber.<BR/>A análise do relatório é necessária para compreensão dos saldos atuais.</FONT></TD>
cTexto+='	</TR>
cTexto+='	<TR>
cTexto+='		<TD align="left" BGCOLOR="#FFFFFF"><FONT  SIZE="2" COLOR="#000000" FACE="verdana, arial, helvetica, times" >Data: ' +DtoC(Date())+ '<BR/>Hora: ' +Time()+ '<BR/><BR/>Descrição: <BR/> Esta mensagem informa que a rotina de processamento de títulos provisórios a receber foi executada.<BR />Os dados do processamento estão gravados no arquivo anexado.</FONT></TD>
cTexto+='	</TR>
cTexto+='</TABLE>
...
cInfRem := ''
cInfDes := MV_YMSPER
cInfAst := "Processamento dos títulos provisórios a receber"
cInfMen := cTexto
cInfAne := cAnexo
cInfFor := 'S'
...
cQry+=EOL+" where
cQry+=EOL+"      D_E_L_E_T_ = ''
cQry+=EOL+"  and E1_FILIAL  = !SE1.FILIAL!
cQry+=EOL+"  and E1_TIPO    = !SE1.TIPO!
cQry+=EOL+"  and E1_VENCTO  < !SE1.DATA!
...
cQry := StrTran(cQry,'!SE1!'          ,RetSqlName('SE1'))
cQry := StrTran(cQry,'!SE1.FILIAL!'   ,ValToSql(xFilial('SE1')))
cQry := StrTran(cQry,'!SE1.TIPO!'     ,ValToSql(cTipoPR))
cQry := StrTran(cQry,'!SE1.DATA!'     ,ValToSql(dDeleta))

//Log de controle
//Memowrite('\STCS002-4.txt',cQry)
...
aRet:= {"Excluídos os títulos a receber, tipo " + cTipoPr + " e com data de vencimento menor que " + DtoC(dDeleta) + "."}
