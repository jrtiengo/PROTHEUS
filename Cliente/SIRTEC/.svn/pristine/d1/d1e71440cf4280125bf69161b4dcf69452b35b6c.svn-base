...
User Function EqXEpc()
    ...
    // leef - machado - ch 51088 - grid deve ser somente leitura
	lRetMod2 := Modelo2(cTitulo, aCab, aRoda, aGrid, 6, cLinhaOk, cTudoOk,{},,,9999,,,.T.)
	...
	If (lOpen)
		...
		If (Found())
  			While ( !(_cSX3)->(Eof()) .And. &("(_cSX3)->X3_ARQUIVO") == cAlias )
				...
				If &("X3Uso((_cSX3)->X3_USADO)") .AND. &("(_cSX3)->X3_BROWSE") == 'S' //.AND. SX3->X3_CONTEXT == 'R'				
