...
************************
CUSTOMIZADA A DESCRICAO DOS SERVICOS!
*************************
/*/
//-----------------------------------------------------------------------
	...
	If cTipo == "1" .And. Empty(cMotCancela)
		...
		If DbSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)	
           ...
           If cMvMsgTrib $ "1-3"
			...
			If  SED->(FieldPos("ED_RECIRRF"))<>0  .and. ( SED->ED_RECIRRF == "2" .Or. Empty(SED->ED_RECIRRF) ) .or. SED->(FieldPos("ED_RECIRRF"))<>0  .and. (SED->ED_RECIRRF == "3"  .and. SA1->A1_RECIRRF  == "2")
				lRecIrrf:= .F.
			EndIf
				...
				Else
					...
					Else
						...
						Else
							...
							If ((cAliasSD2)->D2_ORIGLAN $ "LO")
								cMunPrest := SM0->M0_CODMUN
								cDescMunP := Alltrim(SM0->M0_CIDENT)
							Elseif ((cAliasSD2)->D2_ORIGLAN $ "VD")
								cMunPrest := aDest[07]
								If Empty(cMunPrest)
									cMunPrest := SM0->M0_CODMUN
					...
					If lSC5 .And. SC5->(FieldPos("C5_MUNPRES")) > 0 .And. Empty(SC5->C5_MUNPRES) .And. SM0->M0_CODMUN == "3509502"
		...
		//---  Customizacao Totvs TRS--- desabilitada descricao acima
			cnatoper += Alltrim(&(SC5->C5_YMSG01)) + " "
			cnatoper += Alltrim(&(SC5->C5_YMSG02)) + " "
			cnatoper += Alltrim(&(SC5->C5_YMSG03)) + " "
			cnatoper += Alltrim(&(SC5->C5_YMSG04)) + " "
			cnatoper += Alltrim(&(SC5->C5_YMSG05)) + " "
			cnatoper += Alltrim(&(SC5->C5_YMSG06)) + " "
			cnatoper += Alltrim(&(SC5->C5_YMSG07)) + " "
			cnatoper += Alltrim(&(SC5->C5_YMSG08)) + " "
			cnatoper += Alltrim(&(SC5->C5_YMSG09)) + " "
			cnatoper += Alltrim(&(SC5->C5_YMSG10)) + " "
			cnatoper += Alltrim(&(SC5->C5_YMSG11)) + " "
			cnatoper += Alltrim(&(SC5->C5_YMSG12)) + " "
			cnatoper := Left(cnatoper,2000)
		//----
				...
				If !lAglutina .Or. Len(aProd) == 0
					...
					aadd(aProd,	{Len(aProd)+1,;
								...
								(cAliasSD2)->D2_VALFRE,;
								a410Arred( IIF(!(cAliasSD2)->D2_TIPO$"IP",IIF(SM0->M0_CODMUN == "3550308",(cAliasSD2)->D2_PRCVEN * (cAliasSD2)->D2_QUANT,(cAliasSD2)->D2_TOTAL),0), FuCamArren(nCamPrcv,nCamQuan,nCamTot) ),; // Valor Liquido
								a410Arred( FunValTot((cAliasSD2)->D2_TIPO,(cAliasSD2)->D2_PRCVEN, (cAliasSD2)->D2_QUANT, (cAliasSD2)->D2_TOTAL, (cAliasSD2)->D2_DESCON, (cAliasSD2)->D2_DESCZFR, (cAliasSD2)->D2_VALICM), FuCamArren(nCamPrcv,nCamQuan,nCamTot) )+Iif(lAglutina,((cAliasSD2)->D2_DESCON+(cAliasSD2)->D2_DESCZFR),0),; //Valor Total
		...
		if !empty(aNota)
			if len (aProd) > 0
				...
				cString	+= servicos( aProd, aISSQN, aRetido, cNatOper, lNFeDesc, cDiscrNFSe, aCST, aDest[22], SM0->M0_CODMUN, cF4Agreg ,nDescon,cFntCtrb,@aLeiTrp,lRecIrrf )
				cString	+= valores( aISSQN, aRetido, aTotal, aDest, SM0->M0_CODMUN,aLeiTrp,lRecIrrf )
...
static function assina( aDeduz, aNota, aProd, aTotal, aDest )
	...
	Else  
		cAssinatura += AllTrim("00077777777777")//consumidor não identificado DSFNET
	Endif
...
static function servicos( aProd, aISSQN, aRetido, cNatOper, lNFeDesc, cDiscrNFSe,aCST, cTpPessoa, cCodMun, cF4Agreg, nDescon, cFntCtrb,aLeiTrp,lRecIrrf)	
	...
	cNFESERV := GetNewPar("MV_NFESERV","1") 
	cCMPUSR  := GetMV("MV_CMPUSR")
	...
	for nX := 1 to len( aProd )
		...
		If ( SC6->(FieldPos("C6_DESCRI")) > 0 .And. Len(aProd[nX]) > 40 .And. !Empty(aProd[nX][41]) ) .And. (!lNFeDesc .And. !cNFESERV == "1" .And. !Empty(cCMPUSR) )
