...
// Data......: 04/11/2019                                                             ##
// Objetivo..: Programa de manutenção do cadastro de Serviços                         ##
// #####################################################################################
...
User Function SOLTPCC81()
   ...
   DEFINE MSDIALOG oDlg TITLE "Cadastro de Serviços - GPM" FROM C(178),C(181) TO C(639),C(575) PIXEL
   ...
   // Desenha o Browse
   oBrowse := TCBrowse():New( 005 , 005, 245, 260,,{'Código', 'Descrição dos Serviços', 'Ativo' },{20,50,50,50},oDlg,,,,,{||},,,,,,,.F.,,.T.,,.F.,,, )
...
Static Function XMancontratos(kOperacao, k_codigo, k_descricao, k_Ativo)
   ...
   DEFINE MSDIALOG oDlgMan TITLE "Cadastro de Serviços - GPM" FROM C(178),C(181) TO C(274),C(658) PIXEL

   @ C(002),C(005) Say "Código Serviço"       Size C(040),C(008) COLOR CLR_BLACK PIXEL OF oDlgMan
   @ C(002),C(069) Say "Descrição do Serviço" Size C(150),C(008) COLOR CLR_BLACK PIXEL OF oDlgMan
...
Static Function XSalvaCadastro( koperacao, kCodigo, kDescricao, lAtivo)
   ...
   If Empty(Alltrim(kCodigo))
      MsgAlert("Código do Serviço não informado. Verifique!", "ATENÇÃO!")
      Return(.T.)
   Endif
   
   If Empty(Alltrim(kDescricao))
      MsgAlert("Descrição do Serviço não foi informado. Verifique!", "ATENÇÃO!")
      Return(.T.)
   Endif
   ...
   If kOperacao == "I"                                                               
	  ...
	  If DbSeek(cFilAnt + kCodigo)
	     MsgAlert("Serviço já está cadastrado. Verifique!", "ATENÇÃO!")
	     Return(.T.)
	  Endif
   ...
   If kOperacao == "E"                                                               
      ...
      If MsgYesNo("Deseja realmente excluir este cadastro")
	     ...
	     DbSetOrder(1)    
	     If !DbSeek(cFilAnt + kCodigo)
	        MsgAlert("Serviço a ser excluído não foi localizado. Verifique!", "ATENÇÃO!")
	        Return(.T.)
   	     Else
            Reclock("Z30",.F.)
            dbDelete()
