...
Static Function XMancontratos(kOperacao, k_codigo, k_descricao, k_Ativo)
   ...
   @ C(002),C(069) Say "Descrição do Centro de Serviço" Size C(150),C(008) COLOR CLR_BLACK PIXEL OF oDlgMan
...
Static Function XSalvaCadastro( koperacao, kCodigo, kDescricao, lAtivo)
   ...
   If kOperacao == "E"                                                               
      ...
      If MsgYesNo("Deseja realmente excluir este cadastro")
	     ...
	     DbSetOrder(1)    
	     If !DbSeek(cFilAnt + kCodigo)
	        MsgAlert("Centro de Serviço a ser excluído não foi localizado. Verifique!", "ATENÇÃO!")
	        Return(.T.)
   	     Else
            Reclock("Z28",.F.)
            dbDelete()
