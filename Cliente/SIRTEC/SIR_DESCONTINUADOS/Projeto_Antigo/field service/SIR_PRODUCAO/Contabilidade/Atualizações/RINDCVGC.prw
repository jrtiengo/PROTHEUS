...
Private cString, Titulo, cDesc1, cDesc2, cDesc3, cQtde ,vUnit, vTotal, cPedido ,Tamanho, aReturn, NomeProg, nLastKey, cQtdeV1, cQtdeV2, ;
		...
		cQtdeaV1, cQtdeaV2, cQtdeaVo, cCliente, cQtdeMC, cQtdeFC, cQtdeOC, vTotalC, cCCusto, qLinhas, cPerg := PadR('RINDCVGC',10)
...
@ 052,020 Say 'GERAL -EMPRESA - Centro Custo'       Size 175, 15
...
While !eof() ;
   ...
   EndIF
    ...
    IF (_cTRB)->( !DBSEEK(PadR(cCustoC,20), .F.))
       (_cTRB)->(DBAPPEND())
	   (_cTRB)->CCUSTO      := PadR(cCustoC,20)
    ENDIF
		...
		EndIf
		    ...
		    While SD1->( !Eof() ) .and. SD1->D1_FILIAL  == SE5->E5_FILIAL ;
		          ...
		          IF Alltrim(SD1->D1_CONTA)$cCTAS
				    ...
				    IF  (_cTRB)->( !DBSEEK(PadR(SD1->D1_CC,20), .F.))
					    (_cTRB)->(DBAPPEND())
			   			(_cTRB)->CCUSTO       := PadR(SD1->D1_CC,20)
		    		ENDIF
