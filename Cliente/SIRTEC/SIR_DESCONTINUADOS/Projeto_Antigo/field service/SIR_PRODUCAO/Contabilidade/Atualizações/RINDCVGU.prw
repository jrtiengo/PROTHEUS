 ...
 * Início   : 09junho2012
 * Descrição: Emissao de Indicador dos Custos dos Veiculos SIRTEC - GERAL - UNIDADE
 */
User Function RINDCVGU()
...
Private cString, Titulo, cDesc1, cDesc2, cDesc3, cQtde ,vUnit, vTotal, cPedido ,Tamanho, aReturn, NomeProg, nLastKey, cQtdeV1, cQtdeV2, ;
		...
		cQtdeaV1, cQtdeaV2, cQtdeaVo, cCliente, cQtdeMC, cQtdeFC, cQtdeOC, vTotalC, cCCusto, qLinhas, cPerg := PadR('RINDCVGU',10)
...
@ 052,020 Say 'GERAL -EMPRESA - UNIDADE'       Size 175, 15
...
oTempTab:AddIndex("01", { aArq[1] } ) //CCUSTO
oTempTab:AddIndex("02", { aArq[5] } ) //DESCEN
oTempTab:Create()
...
While !eof() ;
   ...
   EndIF
	...
	cCustoC := IIF (SUBSTR(CT2->CT2_CREDIT,1,1) == '3', PadR(CT2->CT2_CCC,20),PadR(CT2->CT2_CCD,20))

	dbSelectArea( 'CTT' )
	dbSetOrder( 1 )
	dbSeek( xFilial('CTT') + PadR(cCustoC,20), .F. )

    IF (_cTRB)->( !DBSEEK(CTT->CTT_CCSUP, .F.))
       (_cTRB)->(DBAPPEND())
	   (_cTRB)->CCUSTO      := CTT->CTT_CCSUP
    ENDIF
	...
	//		    	   				(_cTRB)->CTTDESC := CTT->CTT_DESC01
		    ...
		    IF SE5->E5_NATUREZ == PadR('202072003',10) .OR. SE5->E5_NATUREZ == PadR('202072006',10) .OR. SE5->E5_NATUREZ == PadR('201081001',10)
				...
				IF SE2->E2_ORIGEM == PadR('FINA050',8)
					...
					cConta := SED->ED_CONTA
	            	cCustoC := PadR(SE5->E5_CCD,20)
]
					dbSelectArea( 'CTT' )
					dbSetOrder( 1 )
					dbSeek( xFilial('CTT') + PadR(cCustoC,20), .F. )
		            cCustoSUP := CTT->CTT_CCSUP
					...
					IF SEZ->( dbSeek( SE5->E5_FILIAL + SE5->E5_PREFIXO + SE5->E5_NUMERO + SE5->E5_PARCELA + SE5->E5_TIPO + SE5->E5_CLIFOR + SE5->E5_LOJA, .f. ) )
					    ...
					    While SEZ->( !Eof() ) .and. SEZ->EZ_FILIAL  == SE5->E5_FILIAL ;
						    ...
						    IF PadR(cConta,20) == PadR('3112102013',20)  .OR. PadR(cConta,20) == PadR('3112102014',20)  .OR. PadR(cConta,20) == PadR('3112102015',20)
							    ...
							    cCustoC :=  SEZ->EZ_CCUSTO
								dbSelectArea( 'CTT' )
								dbSetOrder( 1 )
								dbSeek( xFilial('CTT') + PadR(cCustoC,20), .F. )
		        			    cCustoSUP := CTT->CTT_CCSUP

							    IF (_cTRB)->( !DBSEEK(PadR(cCustoSUP,20), .F.))
	    						   (_cTRB)->(DBAPPEND())
								   (_cTRB)->CCUSTO      := PadR(cCustoSUP,20)
							    ENDIF
...
while !EOF()
    ...
    IF  qLinhas == 0
        ...
        If  mv_par03 == 1
    		oprn:say(0300,0500,' G  E  R  A  L : ' + 'S I R T E C'+'     -    '+ dtoc(MV_PAR01)+ ' Até '+ dtoc(MV_PAR02) + '     -    Regime de Competência' ,oFont5,100)
        Else
    		oprn:say(0300,0500,' G  E  R  A  L : ' + 'S I R T E C'+'     -    '+ dtoc(MV_PAR01)+ ' Até '+ dtoc(MV_PAR02) + '     -    Regime de Caixa' ,oFont5,100)
        EndIF
		oprn:say(0350,0000,Repl("_",1800),oFont55,100)
		oprn:say(0450,0200,' C E N T R O   D E   C U S T O    '  ,oFont5,100)
		oprn:say(0450,1750,'        V a l o r       % Partic. '  ,oFont5,100)

    EndIF
    IF PadR(cCCusto,12) <> PadR((_cTRB)->CCUSTO,12)
       cLinha := cLinha + 050
		oprn:say(cLinha,0000,Repl("_",1800),oFont5,100)
       cLinha := cLinha + 100
		oprn:say(cLinha,0000,'  '+(_cTRB)->CCUSTO ,oFont5,100)
		...
		dbSelectArea( 'CTT' )
		dbSetOrder( 1 )
	    IF !EMPTY((_cTRB)->CCUSTO)
			IF dbSeek( xFilial('CTT') + (_cTRB)->CCUSTO, .F. )
	    	   (_cTRB)->CTTDESC := CTT->CTT_DESC01
	   		EndIF
	    EndIF
		oprn:say(cLinha,0500, (_cTRB)->CTTDESC,oFont5,100)
        cCCusto := PadR((_cTRB)->CCUSTO,20)

		If mv_par04 == 1
           IF mv_par05 == 2
	        	AAdd(aDados, {(_cTRB)->CCUSTO,(_cTRB)->CTTDESC,(_cTRB)->VALORD-(_cTRB)->VALORC,MV_PAR01,MV_PAR02}) //adiciona aos dados
           EndIF
        EndIF

    EndIF
    IF PadR(cCCusto,20) == PadR((_cTRB)->CCUSTO,20)
...
If  qLinhas == 0
        ...
        EndIF
		oprn:say(0350,0000,Repl("_",1800),oFont55,100)
		oprn:say(0450,0200,' C E N T R O   D E   C U S T O                                                                                           V a l o r       % Partic. '  ,oFont5,100)
EndIF
