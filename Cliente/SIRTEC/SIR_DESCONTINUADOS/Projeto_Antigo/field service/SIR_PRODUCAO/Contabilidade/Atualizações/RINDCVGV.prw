 ...
 * Início   : 10junho2012
 * Descrição: Emissao de Indicador dos Custos dos Veiculos SIRTEC - GERAL - VEICULO
 */
User Function RINDCVGV()
...
Private cString, Titulo, cDesc1, cDesc2, cDesc3, cQtde ,vUnit, vTotal, cPedido ,Tamanho, aReturn, NomeProg, nLastKey, cQtdeV1, cQtdeV2, ;
		...
		cQtdeaV1, cQtdeaV2, cQtdeaVo, cCliente, cQtdeMC, cQtdeFC, cQtdeOC, vTotalC, cCCusto, qLinhas, cPerg := PadR('RINDCVGV',10)
...
@ 052,020 Say 'GERAL -EMPRESA - Veiculos'       Size 175, 15
...
aArq := {}
aAdd( aArq , { 'ITEM'		, 'C', 09, 0 } )
aAdd( aArq , { 'CTDDESC'	, 'C', 20, 0 } )
aAdd( aArq , { 'VALORD'		, 'N', 10, 0 } )
aAdd( aArq , { 'VALORC'		, 'N', 10, 2 } )
aAdd( aArq , { 'DESCEN'		, 'N', 15, 2 } )
aAdd( aArq , { 'HISTORICO'	, 'C', 60, 2 } )
...
oTempTab:AddIndex("01", { aArq[1] } ) //ITEM
...
While !eof() ;
   ...
   IF  CT2->CT2_DC == '4'
    ...
    IF (_cTRB)->( !DBSEEK(PadR(cITEM,9), .F.))
       (_cTRB)->(DBAPPEND())
...
If mv_par04 == 1
  ...
  EndIF
    IF  mv_par03 == 1
		DlgToExcel({ {"ARRAY", "RINDCVGV - Indicadores Sirtec - Veículos - Regime de Competencia", aCabec, aDados} }) // utiiliza a função
    Else
		DlgToExcel({ {"ARRAY", "RINDCVGV - Indicadores Sirtec - Veículos - Regime de Caixa", aCabec, aDados} }) // utiiliza a função
    EndIF
EndIF
