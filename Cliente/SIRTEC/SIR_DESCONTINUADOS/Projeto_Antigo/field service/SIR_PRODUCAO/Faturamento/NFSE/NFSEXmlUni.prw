...
User function nfseXMLUni( cCodMun, cTipo, dDtEmiss, cSerie, cNota, cClieFor, cLoja, cMotCancela, aAIDF )
	...
	Private cTPABISS :=GetMv("MV_TPABISS")
		...
		If DbSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
			...
			If !Empty(SF2->F2_DUPL)	
				...
				While !Eof() .And. xFilial("SE1") == (cAliasSE1)->E1_FILIAL .And.;
					...
					If 	(cAliasSE1)->E1_TIPO = MVNOTAFIS .OR. ((cAliasSE1)->E1_ORIGEM = 'LOJA701' .AND. (cAliasSE1)->E1_TIPO $ cWhere)
						...
						If cTPABISS == "2" .AND. SF2->F2_RECISS == "1"   //GetMv("MV_TPABISS") //1-ABATE VALOR LIQ / 2-GERA TITULO ISS 
			...
			Else
			 	if IsBlind()
					...
					FWLogMsg("INFO", , "INTEGRATION", FunName() , "SENDER", , "Configure o Parametro MV_XMLOBRA {,,,,,,,,,,,,,,}  15 posicoes") 
			 	Else
			...
			While !(cAliasSD2)->(Eof()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				_aSX5 := {}
				_aSX5 := FWGetSX5( "60",RetFldProd(SB1->B1_COD,"B1_CODISS"),"pt-br") 
				If (Len(_aSX5) > 0)
					If (!lNFeDesc )
						If nCont == 1
							cNatOper   += If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(SubStr(_aSX5[1][4],1,55))),AllTrim(SubStr(_aSX5[1][4],1,55)))
						EndIf
					ElseIf nCont == 1
						cDescrNFSe := If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(SubStr(_aSX5[1][4],1,55))),AllTrim(SubStr(_aSX5[1][4],1,55)))
					EndIf
				EndIf
...
Static Function assina( aDeduz, aNota, aProd, aTotal, aDest )
	Local cAssinatura	:= ""
	Local cMVCODREG		:= superGetMV( "MV_OPTSIMP" ,.T., "3" ) //-- Codigo regime tributario emitente da Nf-e = 1-Simples Nacional; 2-Simples Nacional - Excesso desub-limite de receita bruta; 3- Regime Nacional
	Local nDeduz		:= 0
	Local nX			:= 0
...
Static Function servicos( aProd, aISSQN, aRetido, cNatOper, lNFeDesc, cDiscrNFSe,aCST, cTpPessoa, cCodMun, cF4Agreg, nDescon   )	
	...
	For nX := 1 To Len( aProd )
		...
		If ( SC6->(FieldPos("C6_DESCRI")) > 0 .And. Len(aProd[nX]) > 40 .And. !Empty(aProd[nX][41]) ) .And. (!lNFeDesc .And. !cNFESERV == "1" .And. !Empty(CMPUSR) )
