...
Local aEmpNFe  := {}
Local aInfo   := {}
Local aTables := {"SA1","SA2","SF1","SD1","SF2","SD2","CTT","ZNF", "SF4","SB6","SB1","CT1","SE2",'SX6'}//seta as tabelas que serão abertas no rpcsetenv
Local nI := 0
Local nAbax := 0
...
If Empty(cIdAbx)
	...
	Next nI
       		Conout('---------------------------------------------------------------------------------------------')
       		Conout('Função MTUFOPRO Iniciada - TESTE LEEF ! ')
       		Conout('---------------------------------------------------------------------------------------------')
   ...
   Enddo		
	...
	RESET ENVIRONMENT//RpcClearEnv() //RESET ENVIRONMENT //->[CRITICA] - Remover o RESET ENVIRONMENT desse ponto	   	
	...
	For nAbax:=1 to Len(aEmpSmar)
		...
		If cEmpABax = aEmpSmar[nAbax][1]
	  	...
	  	Else
		   ...
		   PREPARE ENVIRONMENT EMPRESA aEmpSmar[nAbax][1] FILIAL aEmpSmar[nAbax][2] MODULO "COM"		   		 
		Endif
			
		If CHKFILE("ZNF", .F.) 
       		Conout('---------------------------------------------------------------------------------------------')
       		Conout('Função MTUFOPRO entrando MimpNFS  - TESTE LEEF ! ')
       		Conout('---------------------------------------------------------------------------------------------')
			U_MImpNFs(aEmpSmar[nAbax][1],aEmpSmar[nAbax][2])			   
		Else
		  	Conout('EMPRESA SEM ZNF' + aEmpSmar[nAbax][1] )
		Endif
...
User Function MImpNFs(cEmpMa,cFilMa)
******************************************************************************
	...
	Conout('---------------------------------------------------------------------------------------------')
 	...
 	If !Empty((cAlias)->ZNF_DOC+(cAlias)->ZNF_SERIE+(cAlias)->ZNF_FORNEC+  (cAlias)->ZNF_LOJA)
		/* Função Gera NF de Entrada dentro do sistema Protheus*/
       		Conout('---------------------------------------------------------------------------------------------')
       		Conout('Função MTUFOPRO entrando FSGeraNFE('+cAlias+')  - TESTE LEEF ! ')
       		Conout('---------------------------------------------------------------------------------------------')
		FSGeraNFE(cAlias)
	 	Conout('---------------------------------------------------------------------------------------------')
...
Static Function FSGeraNFE(cPrefix)
	...
	Private cBancAba     := '' //Informações referentes aos dados bancários. 
	Private cAgenAba     := '' // Estas variáveis poderão ser utilizadas nos pontos de entradas 
   Private cContAba     := ''	// no momento de gerar os títulos a pagar.  
   Private cTipFret     := '' //Tipo do Frete
	...
	Do While (cPrefix)->(!Eof())
		...
		If !lImport		                                 
         //Trocado Log para mostrar só uma vez no console.log, da forma antiga a nota era grava para cada item da mesma.          //Leonardo Vasco Viana 21/07/2017
			Conout('MTUFOPRO - IMPORTANDO NOTA  '+ (cPrefix)->ZNF_DOC+'-'+ (cPrefix)->ZNF_SERIE+'-'+(cPrefix)->ZNF_FORNEC+'-'+(cPrefix)->ZNF_LOJA)
			
			If (cPrefix)->ZNF_ESPEC == 'NFE' 
				cEspecie	:= 'SPED' 
			Else 
			...
			nTotItem    := 1  
			nValFreA     := (cPrefix)->ZNF_FRETE	 //Valor total do Frete
			...
			If Alltrim(cEspecie) == 'CTE'       
				...
				cUsaCampo  := AllTrim(Posicione("SX3",2,"ZNF_TPFRET","X3_CAMPO"))		
				If !Empty(cUsaCampo) 
			  		Aadd(_aCabSF1,{"F1_TPFRETE"    ,Alltrim((cPrefix)->ZNF_TPFRET)			   			,Nil})     //Enviar tipo de Frete para o ERP. 
			  	Endif 
         ...
         cUsaCampo  := AllTrim(Posicione("SX3",2,"F1_USUSMAR","X3_CAMPO"))      
         If !Empty(cUsaCampo)
				Aadd(_aCabSF1,{"F1_USUSMAR"  ,(cPrefix)->ZNF_USERLG			   			,Nil})                 
         Endif
         
			cUsaCampo  := AllTrim(Posicione("SX3",2,"F1_ZUSER","X3_CAMPO"))      //Mesma função do campo F1_USUSMAR, porem seguindo o padrão Z de campos customizados
         If !Empty(cUsaCampo)
         ...
         Endif
			...
			If Alltrim(cEspecie) = 'CTE'  
				...
				//Envio dos novos campos de Municipio e Estado de Origem e Destino do Transporte. 				//Leonardo Vasco 26/06/2018
				cUsaCampo  := AllTrim(Posicione("SX3",2,"F1_MUORITR","X3_CAMPO"))		
				If !Empty(cUsaCampo) 
			  		Aadd(_aCabSF1,{"F1_MUORITR"    ,(cPrefix)->ZNF_MUORIT			   			,Nil}) 	      
			  	Endif	
				
				cUsaCampo  := AllTrim(Posicione("SX3",2,"F1_UFORITR","X3_CAMPO"))		
				If !Empty(cUsaCampo) 
			  		Aadd(_aCabSF1,{"F1_UFORITR"    ,(cPrefix)->ZNF_UFORIT			   			,Nil})    
			  	Endif	    
			  	
			  	cUsaCampo  := AllTrim(Posicione("SX3",2,"F1_MUDESTR","X3_CAMPO"))		
				If !Empty(cUsaCampo) 
			  		Aadd(_aCabSF1,{"F1_MUDESTR"    ,(cPrefix)->ZNF_MUDEST			   			,Nil})      
			  	Endif 
				
				cUsaCampo  := AllTrim(Posicione("SX3",2,"F1_UFDESTR","X3_CAMPO"))		
				If !Empty(cUsaCampo) 
			  		Aadd(_aCabSF1,{"F1_UFDESTR"    ,(cPrefix)->ZNF_UFDEST			   			,Nil}) 
			  	Endif 				
	 		
	 			cUsaCampo  := AllTrim(Posicione("SX3",2,"ZNF_MODAL","X3_CAMPO")) 
				If !Empty(cUsaCampo)
					Aadd(_aCabSF1,{"F1_MODAL"    ,Alltrim((cPrefix)->ZNF_MODAL)		,Nil})    
      	        Endif 
	 				 			
			Endif
         
		 cUsaCampo  := AllTrim(Posicione("SX3",2,"F1_DTCPISS","X3_CAMPO"))    //  Inclusai 11.03.2019    //Data competencia do Serviço prestado.
         If !Empty(cUsaCampo)
            AAdd(aCabec, {"F1_DTCPISS",    Stod((cPrefix)->ZNF_EMISSA) 	  			,Nil})
         Endif
         ...
         //Caso parametro não exista, será considerado .T. e pegará a data de emissão da nota fiscal.
			...
			If (cPrefix)->ZNF_ESPEC = 'NFSE' .or. (cPrefix)->ZNF_ESPEC = 'NFPS'// Incluído tratamento de Fornecedor de ISS e Loja ISS
			...
			cUsaCampo  := AllTrim(Posicione("SX3",2,"ZNF_BANCO","X3_CAMPO"))      
			If !Empty(cUsaCampo)
				cBancAba  := Alltrim((cPrefix)->ZNF_BANCO)
			Endif			
			
			cUsaCampo  := AllTrim(Posicione("SX3",2,"ZNF_AGENC","X3_CAMPO")) 
			If !Empty(cUsaCampo)
				cAgenAba	:= Alltrim((cPrefix)->ZNF_AGENC)
			Endif			                                 
			
			cUsaCampo  := AllTrim(Posicione("SX3",2,"ZNF_CONTAB","X3_CAMPO")) 
			If !Empty(cUsaCampo)
				cContAba := Alltrim((cPrefix)->ZNF_CONTAB)				                       
			Endif			
			//       
	   ...
	   SC7->(dbSeek(xFilial('SC7')+(cPrefix)->ZNF_PEDIDO+Alltrim((cPrefix)->ZNF_ITEMPC)))
		...
		/**********************************************************************************************************************
		Autor: Leonardo Vasco Viana 
		Data: 02/12/2019 
		Descrição: Não serão inseridos no EXECAUTO nenhuma informação relacionado a impostos, pois por premissa a TES deverá realizar todos os calculos automaticamente.
		O envio dos impostos estava causando inconsistências em alguns impostos, como por premissa a nota deve ser registrada sem intervenção manual, o ÁBAX não mais
		enviará estas informações.
		Será enviado somente a base, aliquota e valor do ISS, que incide somente sobre as notas de serviços.
		...
		*************************************************************************/
		...
		//Campo D1_ABATISS só será enviado se existir seus correspondentes na tabela ZNF
		cUsaAbTIs  := AllTrim(Posicione("SX3",2,"ZNF_ABATIS","X3_CAMPO")) 	
      	If !Empty(cUsaAbTIs)  
			If (cPrefix)->ZNF_ABATIS > 0
				Aadd(_aLinha,{"D1_ABATISS" 	, (cPrefix)->ZNF_ABATIS , Nil})
			Endif
      	Endif
      	
		
		cUsaAbTIn  := AllTrim(Posicione("SX3",2,"ZNF_ABATIN","X3_CAMPO"))
		If !Empty(cUsaAbTIn)  
			If (cPrefix)->ZNF_ABATIN > 0
				Aadd(_aLinha,{"D1_ABATINS" 	, (cPrefix)->ZNF_ABATIN , Nil})
			Endif
      	Endif	
		
		cUsaAbTAl  := AllTrim(Posicione("SX3",2,"ZNF_ABATAL","X3_CAMPO"))				
		If !Empty(cUsaAbTAl)  
			If (cPrefix)->ZNF_ABATAL > 0
				Aadd(_aLinha,{"D1_ABATALM" 	, (cPrefix)->ZNF_ABATAL , Nil})
			Endif
		Endif	 
	
		//Campo D1_DCIPSC só será enviado 
		cUsaDCSC  := AllTrim(Posicione("SX3",2,"D1_DCIPSC","X3_CAMPO"))
      If !Empty(Alltrim((cPrefix)->ZNF_CFOP))      
        	If (cEstSM == 'SC') .and. !Empty(cUsaDCSC) 
         		Aadd(_aLinha,{"D1_DCIPSC" 	, (cPrefix)->ZNF_CFOP , Nil})
        	Endif
      Endif		
      ...
      If (cPrefix)->ZNF_AVLINS > 0	
			Aadd(_aLinha,{"D1_AVLINSS" 	, (cPrefix)->ZNF_AVLINS ,Nil})			
		Endif        
...
If Empty(cIdAbx)
	...
	For n:=1 to Len(aEmpSmar)
		...
		If CHKFILE("ZNF", .F.)
			U_MImpCTE(aEmpSmar[n][1],aEmpSmar[n][2])			   
		Else
...
Else	
	...
	U_MImpCTE(cEmpAbx,cFilAbx)			   
Endif

Return 

User Function MImpCTE(cEmpMa,cFilMa)
******************************************************************************
* Função para importar  notas para o Protheus. Foi desmembrado a função devido
* a empresas que possuem muitas empresas e filiais.
******************************************************************************
   ...
   Private cUsaCampo     := ""
	...
	Do While (cPrefix)->(!Eof())
		...
		If !lImport      
		  ...
		  aIteAbax := U_Busca_NFs((cPrefix)->ZNF_FILIAL,(cPrefix)->ZNF_DOCCTE,(cPrefix)->ZNF_SERCTE,(cPrefix)->ZNF_FORCTE,(cPrefix)->ZNF_LOJCTE ) // Busca Notas Fiscais
	      ...
	      SA2->(dbSeek(xFilial('SA2')+(cPrefix)->ZNF_FORNEC+(cPrefix)->ZNF_LOJA ))			
			...
			AAdd(aCabec, {"F1_UFORITR",    (cPrefix)->ZNF_UFORIT}) // 24 Estado Origem do Transporte			
			AAdd(aCabec, {"F1_MUORITR",    (cPrefix)->ZNF_MUORIT}) // 25 Município Origem do Transporte			
			AAdd(aCabec, {"F1_UFDESTR",    (cPrefix)->ZNF_UFDEST}) // 26 Estado Destino do Transporte		
			AAdd(aCabec, {"F1_MUDESTR",    (cPrefix)->ZNF_MUDEST}) // 27 Município Destino do Transporte
			//AAdd(aCabec, {"F1_DTCPISS",    Stod((cPrefix)->ZNF_EMISSA)}) // Data competencia do ISS  Fata Inclus'ao 11.3.2019
			cUsaCampo  := AllTrim(Posicione("SX3",2,"ZNF_MODAL","X3_CAMPO")) 
			If !Empty(cUsaCampo)
				Aadd(aCabec,{"F1_MODAL",	Alltrim((cPrefix)->ZNF_MODAL)})  //Modal do Transporte.  
				cModal := Alltrim((cPrefix)->ZNF_MODAL)
    		Endif
    		
    		cUsaCampo  := AllTrim(Posicione("SX3",2,"ZNF_TPFRET","X3_CAMPO"))		
			
			If !Empty(cUsaCampo) 
				Aadd(aCabec,{"F1_TPFRETE"    ,Alltrim((cPrefix)->ZNF_TPFRET)		   			,Nil})     //Enviar tipo de Frete para o ERP.     				
				cTipFret   := (cPrefix)->ZNF_TPFRET
			Endif 
		   ...
		   cDTCPiss := Stod((cPrefix)->ZNF_EMISSAO)
...
********************************************************************************
	...
	Else // Tratamento para Oracle
		...
		If cStatus = '2' 
			cQryExc :="UPDATE "+RetSqlName("SF1")+""+CHR(13)+CHR(10)             
			cUsaCampo  := AllTrim(Posicione("SX3",2,"F1_ZUSER","X3_CAMPO"))      
		 	If !Empty(cUsaCampo)
