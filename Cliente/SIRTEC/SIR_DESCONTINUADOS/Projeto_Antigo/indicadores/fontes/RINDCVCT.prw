 ...
 * Início   : 12junho2012
 * Descrição: Emissao de Indicador dos Custos dos Veiculos SIRTEC - GERAL - CONTA / CC
 */
User Function RINDCVCT()
...
Private cString, Titulo, cDesc1, cDesc2, cDesc3, cQtde ,vUnit, vTotal, cPedido ,Tamanho, aReturn, NomeProg, nLastKey, cQtdeV1, cQtdeV2, ;
		...
		cQtdeaV1, cQtdeaV2, cQtdeaVo, cCliente, cQtdeMC, cQtdeFC, cQtdeOC, vTotalC, cCCusto, qLinhas, cPerg := PadR('RINDCVCT',10)
...
@ 052,020 Say 'CONTA - Centro Custo'       Size 175, 15
...
oTempTab:AddIndex("01", { aArq[1]+aArq[3] } ) //CONTA+CCUSTO
oTempTab:AddIndex("02", { aArq[7] } ) //DESCEN
...
While !eof() ;
   ...
   EndIF
    ...
    IF MV_PAR03 == 1  .OR. MV_PAR06 == PadR('3122301002',20)
	    ...
	    IF (_cTRB)->( !DBSEEK(PadR(cConta,20)+PadR(cCustoC,20), .F.))
	       (_cTRB)->(DBAPPEND())
		    ...
		    IF PadR(MV_PAR06,20) == PadR('3112102013',20) .or. PadR(MV_PAR06,20) == PadR('3112102014',20) .or. PadR(MV_PAR06,20) == PadR('3112102015',20)   // PadR(MV_PAR06,20) == PadR('3121103025',20)
				...
				IF SE2->E2_ORIGEM == PadR('FINA050',8)
					...
					IF SEZ->( dbSeek( SE5->E5_FILIAL + SE5->E5_PREFIXO + SE5->E5_NUMERO + SE5->E5_PARCELA + SE5->E5_TIPO + SE5->E5_CLIFOR + SE5->E5_LOJA, .f. ) )
						...
						End
	                ElseIF SE5->E5_NATUREZ == PadR('202072003',10) .AND. PadR(MV_PAR06,20) == PadR('3112102015',20) .or. SE5->E5_NATUREZ == PadR('202072006',10) .AND. PadR(MV_PAR06,20) == PadR('3112102014',20) .or. SE5->E5_NATUREZ == PadR('201081001',10) .AND. PadR(MV_PAR06,20) == PadR('3112102013',20)   // PadR(MV_PAR06,20) == PadR('3121103025',20)
...
while !EOF()
    ...
    IF  qLinhas == 0
	    ...
	    EndIF
        If  mv_par03 == 1
			oprn:say(0300,0600,' C O N T A : ' + ALLTRIM((_cTRB)->CONTA)+' - '+ALLTRIM((_cTRB)->CT1DESC)+'     -    '+ dtoc(MV_PAR01)+ ' Até '+ dtoc(MV_PAR02) + '     -    Regime de Competência' ,oFont5,100)
        Else
			oprn:say(0300,0600,' C O N T A : ' + ALLTRIM((_cTRB)->CONTA)+' - '+ALLTRIM((_cTRB)->CT1DESC)+'     -    '+ dtoc(MV_PAR01)+ ' Até '+ dtoc(MV_PAR02) + '     -    Regime de Caixa' ,oFont5,100)
        EndIF

		oprn:say(0350,0000,Repl("_",1800),oFont55,100)
		oprn:say(0450,0200,' C E N T R O   D E   C U S T O '  ,oFont5,100)
		oprn:say(0450,1700,' V a l o r       % Partic. '  ,oFont5,100)
    EndIF
       ...
       cLinha := cLinha + 100
	    ...
	    IF !EMPTY((_cTRB)->CCUSTO)
			IF dbSeek( xFilial('CTT') + (_cTRB)->CCUSTO, .F. )
	   		...
	   		EndIF
	    EndIF
		oprn:say(cLinha,0600, (_cTRB)->CTTDESC,oFont5,100)
        cCCusto := PadR((_cTRB)->CCUSTO,12)
    EndIF
    IF PadR(cCCusto,12) == PadR((_cTRB)->CCUSTO,12)
...
oprn:say(cLinha,01870, Transform( vTotal , '@E 9999,999.99' ),oFont5,100)
