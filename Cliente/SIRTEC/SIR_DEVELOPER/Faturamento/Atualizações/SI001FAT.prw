	...
	For i := 1 to Len(aDados)
		...
		If Len(aDados[i]) >= 15
			...
			Else			
				...
				// #31266 Não sobrepor a informação do campo, caso a leitura do arquivo esteja vazia. Mauro - Solutio. 17/12/2021.
				If !Empty(Alltrim(aDados[i,9]))
					aadd(aCabec,{"C5_YMSG01"  , StrTran(aDados[i,9],'"""','"')  ,Nil})	
				EndIf
				If !Empty(Alltrim(aDados[i,17]))
					aadd(aCabec,{"C5_YMSG02"  , StrTran(aDados[i,17],'"""','"')  ,Nil})	
				EndIf
				If !Empty(Alltrim(aDados[i,18]))
					aadd(aCabec,{"C5_YMSG03"  , StrTran(aDados[i,18],'"""','"')  ,Nil})	
				EndIf
				If !Empty(Alltrim(aDados[i,19]))
					aadd(aCabec,{"C5_YMSG04"  , StrTran(aDados[i,19],'"""','"')  ,Nil})	
				EndIf
				If !Empty(Alltrim(aDados[i,20]))
					aadd(aCabec,{"C5_YMSG05"  , StrTran(aDados[i,20],'"""','"')  ,Nil})	
				EndIf
				If !Empty(Alltrim(aDados[i,21]))
					aadd(aCabec,{"C5_YMSG06"  , StrTran(aDados[i,21],'"""','"')  ,Nil})	
				EndIf
				If !Empty(Alltrim(aDados[i,22]))
					aadd(aCabec,{"C5_YMSG07"  , StrTran(aDados[i,22],'"""','"')  ,Nil})	
				EndIf
				If !Empty(Alltrim(aDados[i,23]))
					aadd(aCabec,{"C5_YMSG08"  , StrTran(aDados[i,23],'"""','"')  ,Nil})	
				EndIf
				If !Empty(Alltrim(aDados[i,24]))
					aadd(aCabec,{"C5_YMSG09"  , StrTran(aDados[i,24],'"""','"')  ,Nil})	
				EndIf
				If !Empty(Alltrim(aDados[i,25]))
					aadd(aCabec,{"C5_YMSG10"  , StrTran(aDados[i,25],'"""','"')  ,Nil})	
				EndIf
				If !Empty(Alltrim(aDados[i,26]))
					aadd(aCabec,{"C5_YMSG11"  , StrTran(aDados[i,26],'"""','"')  ,Nil})	
				EndIf
				If !Empty(Alltrim(aDados[i,27]))
					aadd(aCabec,{"C5_YMSG12"  , StrTran(aDados[i,27],'"""','"')  ,Nil})	
				EndIf
			...
			If lOk
				...
				// Adicionado em 12-01-2019 - Bruno
				aadd(aLinha,{"C6_NUMOS"    , _cNumOS    ,    Nil})
				aadd(aItens, aLinha)
				_nIteAtu ++
				If i == Len(aDados)				
...
Static Function fBuscaItens(cNumPed)
	...
	If SC6->(dbSeek(xFilial("SC6") + cNumPed))
		...
		aLinha := {}
			...
			// Adicionado em 12-01-2019 - Bruno
			aadd(aLinha,{"C6_NUMOS"    , SC6->C6_NUMOS     ,    Nil})			
			//aadd(aLinha,{"C6_CC" ,   "A0102020101030201",        Nil})
			aadd(aItens, aLinha)
			cDeleta := "S"
			
			If !Empty(SC6->C6_NUMOS)
				_cNumOS := SC6->C6_NUMOS
				//alert(_cNumOS)
			EndIf
...
Static Function fImportPed(aCab,aItens,cNumPed)
	...
	If lOk		
		...
		If !lMsErroAuto
			...
			//If MsgYesNo("Pedido de venda "+ cNumPed +" atualizado com sucesso. Deseja gerar a NF deste pedido?",_cRotina)
...
Static Function fGeraNF(cNumPed)	
	...
	If Len(_aPvlNfs) > 0
		If lFat				
			...
			// #31266 Tratamento para série. Quando RS, a série deve ser a 3. Manter como 1 para as demais empresas. Mauro - Solutio. 17/12/2021.
			If cEmpAnt == "01"
				cSerie  := "3  "
			Else
				cSerie  := "1  "
