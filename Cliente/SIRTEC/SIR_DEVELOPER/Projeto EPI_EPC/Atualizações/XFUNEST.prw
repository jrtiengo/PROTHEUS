...
// Local cMat     := M->CP_SEQFUNC
   ...
   // Pesquisa o Centro de Custo do Funcionário informado.
   If !Empty(cMat) 
		...
		*/
		...
		If cMat == cMat_
			...
			cQuery := " SELECT RE_CCP, RE_DATA "
			cQuery += " FROM "+ RETSQLNAME("SRE") +" SRE "
			cQuery += " WHERE SRE.RE_EMPP = '"+ cEmpAnt +"' "
			cQuery += " AND SRE.RE_FILIALP = '"+ cFilAnt +"' "
			...
			Do While !EOF("T_SRE")
				If STOD(T_SRE->RE_DATA) <= da105data
