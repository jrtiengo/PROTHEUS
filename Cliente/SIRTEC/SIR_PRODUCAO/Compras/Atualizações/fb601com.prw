...
If !Empty(aFilsCalc)                                                                                  
	...
	For nForFilial := 1 to Len(aFilsCalc)
		...
		If aFilsCalc[nForFilial,1]
			...
			ProcRegua(RecCount())
			/*
			#31702 Alteracao da query. Estava dando erro, quando a variavel lArqSBZ vinha como F. 
			Criado tratamento para nao incluir SBZ.R_E_C_N_O_ nesse caso. Mauro - Solutio. 02/02/2022.
			*/
			#IFDEF TOP
				If ( TcSrvType()!="AS/400" )
					lQuery := .T.
					cQuery := "SELECT SB1.*,SB1.R_E_C_N_O_ SB1RECNO "
					
					If lArqSBZ
						cQuery += " ,SBZ.R_E_C_N_O_ SBZRECNO "
					EndIf
					...
					cQuery += "SB1.B1_CONTRAT<>'S' AND SB1.B1_TIPO<>'BN' AND "   // Alterado por Assis em 21/10/20 11:27
					cQuery += "SB1.D_E_L_E_T_ = ' ' "					
					cQuery += " ORDER BY SB1.B1_COD"
					
					cQuery := ChangeQuery(cQuery)
					memowrit("C:\temp\QRY_FB601COM.TXT",cQuery)   // ASSIS - RETIRAR 
			...
			While Eval(bWhile)
				...
				IncProc( OemToAnsi("Imprimindo") + ": " + aFilsCalc[ nForFilial, 3 ] )

				If IsProdMod((cAliasSB1)->B1_COD)
					//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
					//³ Filtra produtos MOD				³
					//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
					memowrit("C:\temp\FB601COM_produto_mod_283.TXT",(cAliasSB1)->B1_COD)
				   	(cAliasSB1)->(dbSkip())
			   		Loop
				EndIf
				...
				If !Empty(aReturn[7])
					If !&(aReturn[7])
         				memowrit("C:\temp\FB601COM_return7_290.TXT",(cAliasSB1)->B1_COD)
						(cAliasSB1)->(dbSkip())
						Loop
					Endif   
				EndIf
				If !lQuery
					...
					If (cAliasSB1)->B1_GRUPO < mv_par03 .Or. (cAliasSB1)->B1_GRUPO > mv_par04 .Or.;
        				...
        				memowrit("C:\temp\FB601COM_no_not_query_302.TXT",(cAliasSB1)->B1_COD)
					...
					If (cAliasSB1)->B1_LOCPAD < mv_par07 .Or. (cAliasSB1)->B1_LOCPAD > mv_par08
        				memowrit("C:\temp\FB601COM_B1_LOCPAD_312.TXT",(cAliasSB1)->B1_COD)
						dbSkip()
						Loop
					EndIf
				EndIf
				...
				While !Eof() .And. SB2->B2_FILIAL + SB2->B2_COD == xFilial("SB2") + (cAliasSB1)->B1_COD
					If SB2->B2_LOCAL < mv_par16 .OR. SB2->B2_LOCAL > mv_par17
        				memowrit("C:\temp\FB601COM_B2_LOCAL_328.TXT",(cAliasSB1)->B1_COD)
						dbSkip()
						Loop
					EndIf
					...
					If SB2->B2_LOCAL == cLocCQ .And. mv_par11 == 2
        				memowrit("C:\temp\FB601COM_B2_LOCAL_CQ_336.TXT",(cAliasSB1)->B1_COD)
						dbSkip()
						Loop
					EndIf
			 ...
			 if mv_par20 <> 1
					...
					While !Eof() .And. SB2->B2_FILIAL + SB2->B2_COD == xFilial("SB2")+(cAliasSB1)->B1_COD
						...
						SB2->(dbSkip())
					EndDo
       				memowrit("C:\temp\FB601COM_LOG_437.TXT",alltrim((cAliasSB1)->B1_COD) + "qatu: "  + alltrim(str(_nValAtu)) + ;
