//{IX} ================================================================================
//{IX} 000026 = User Function MT131WF( nOpcao, oProcess )
//{IX} 000073 = User Function COTIniciar( _aDados, oProcess )
//{IX} 000248 = User Function COTTimeOut( oProcess )
//{IX} 000268 = User Function COTRetorno( oProcess )
//{IX} 000386 = User Function COTNotificar(_cFilial, _cCotacao, _cFornece, _cLoja, _Solict, _DtRet, _Cd_Pgto, _Tp_Frete)
//{IX} 000478 = Static Function _TlForn( _cNome, _cCotacao, cEdit1, cEdit2, cMemo )
//{IX} ================================================================================
#INCLUDE "PROTHEUS.CH"
#INCLUDE "TOPCONN.CH"
...
//ฑฑบPrograma  ณMT131WF   บAutor  ณJose Vergani        บ Data ณ  04/12/07   บฑฑ
...
//฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿
//*/
User Function MT131WF( nOpcao, oProcess )
 
   // Assis 25/09/20 17:01
   //LOCAL aRet  := ParamIXB[1]
         //  Elemento [1] do Vetor que armazena o n๚mero das Solicita็๕es de Compras que estใo relacionadas na gera็ใo de cota็๕es. 
         //  Representa a primeira SC selecionada. (Uma SC relacionada a cota็ใo)
   //LOCAL aRet2 := ParamIXB[2]  
         // Vetor contendo todas as Solicita็๕es de Compras que estใo relacionadas na gera็ใo das cota็๕es 
      ...
      While !Eof() .and. (SC8->C8_FILIAL+SC8->C8_NUM == xFilial("SC8")+_aDados[_nLinha])
         ...
         If _xFornece <> SC8->C8_FORNECE
            ...
            If _aTela[1,1]
               ...
               oProcess:bReturn := "U_MT131WF(1)"
               oProcess:bTimeOut := {{"U_MT131WF(2)",30, 0, 5 }}
...
User Function COTRetorno( oProcess )
   ...
   For _nLinha := 1 to Len( _aItens )
      ...
      if Val(StrTran(AllTrim(_aPrecos[_nLinha]), ',', '.')) >  0  // leef chamado 48882
         ...
         Do While !Eof() .And. SC8->C8_NUM == _cCotacao .And. SC8->C8_FORNECE == _cFornece ;
            ...
            SC8->C8_ALIIPI   := Val(StrTran(AllTrim(_aIpi[_nLinha]), ',', '.'))
            ...
            //ConOut('Fazendo Cota็ใo de Compra "'+_cCotacao+'" do Fornecedor "'+_cFornece+"/"+_cLoja+'".  ITEM ' +str(_nLinha,3) + ' prd:' +SC8->C8_PRODUTO+ ' Vindo html Condpagto. = '+ _cCondicao +            ' IPI _aIpi[_nLinha]->' + _aIpi[_nLinha] )
            MsUnlock("SC8")
            DbSelectArea("SC8")
            DbSkip()
         EndDo
