 ...
 * Início   : 03junho2012
 * Descrição: Emissao de Indicador dos Custos dos Veiculos SIRTEC - GERAL - CONTA
 */
User Function RINDCVG()
...
Private cString, Titulo, cDesc1, cDesc2, cDesc3, cQtde ,vUnit, vTotal, cPedido ,Tamanho, aReturn, NomeProg, nLastKey, cQtdeV1, cQtdeV2, ;
		...
		cQtdeaV1, cQtdeaV2, cQtdeaVo, cCliente, cQtdeMC, cQtdeFC, cQtdeOC, vTotalC, qLinhas, cPerg := PadR('RINDCVG',10)
...
@ 052,020 Say 'GERAL -EMPRESA'       Size 175, 15
...
aArq := {}
aAdd( aArq , { 'EMPRESA'	, 'C', 11, 0 } )
aAdd( aArq , { 'CTTDESC'	, 'C', 40, 0 } )
aAdd( aArq , { 'CONTA'		, 'C', 20, 0 } )
aAdd( aArq , { 'CT1DESC'	, 'C', 40, 0 } )
aAdd( aArq , { 'VALORD'		, 'N', 15, 2 } )
aAdd( aArq , { 'VALORC'		, 'N', 15, 2 } )
...
oTempTab:AddIndex("01", { aArq[1]+aArq[3] } ) //EMPRESA+CONTA
oTempTab:AddIndex("02", { aArq[5] } ) //VALORD
oTempTab:AddIndex("03", { aArq[7] } ) //DESCEN
...
cCTAS := Alltrim(GetMV("SV_CTASIND"))
...
While !eof() ;
   ...
   IF  CT2->CT2_DC == '4'
    ...
    IF MV_PAR03 == 1
        IF !Alltrim(CT2->CT2_DEBITO)$cCTAS .AND. CT2->CT2_DC == '1'  .OR. Alltrim(CT2->CT2_DEBITO)$'1520900007'  .AND. CT2->CT2_DC == '1'
			dbSelectArea( 'CT2' )
			dbSkip()
			Loop
		EndIf

        IF !Alltrim(CT2->CT2_CREDIT)$cCTAS  .AND. CT2->CT2_DC == '2'  .OR. Alltrim(CT2->CT2_CREDIT)$'3122301002'  .AND. CT2->CT2_DC == '2'
			dbSelectArea( 'CT2' )
			dbSkip()
			Loop
		EndIf

        IF !Alltrim(CT2->CT2_DEBITO)$cCTAS .AND. !Alltrim(CT2->CT2_CREDIT)$cCTAS  .AND. CT2->CT2_DC == '3'
			dbSelectArea( 'CT2' )
			dbSkip()
			Loop
		EndIf
    ...
    Else
    	...
    	IF CT2->CT2_DEBITO <> PadR('3122301002',20)
			dbSelectArea( 'CT2' )
			dbSkip()
			Loop
		EndIf
    ...
    IF CT2->CT2_DC == '1'  .AND. SUBSTR(CT2->CT2_DEBITO,1,1) <> '3'
		dbSelectArea( 'CT2' )
		dbSkip()
		Loop
	EndIf

    IF  SUBSTR(CT2->CT2_CREDIT,1,5) == '21101'  .AND. CT2->CT2_DC == '2'
		dbSelectArea( 'CT2' )
		dbSkip()
		Loop
	EndIf
    ...
    IF (_cTRB)->( !DBSEEK('S I R T E C'+PadR(cConta,20), .F.))
       (_cTRB)->(DBAPPEND())
       (_cTRB)->EMPRESA     := 'S I R T E C'
...
IF MV_PAR03 == 2
	...
	While !eof() .and. SE5->E5_FILIAL == xFilial( 'SE5' ) ;
		...
		EndIf
		    ...
		    While SD1->( !Eof() ) .and. SD1->D1_FILIAL  == SE5->E5_FILIAL ;
		          ...
		          IF Alltrim(SD1->D1_CONTA)$cCTAS

                   IF SD1->D1_DOC == PadR('38468',9)
                      valor := valor
                   EndIF
				    ...
				    IF  (_cTRB)->( !DBSEEK('S I R T E C'+PadR(SD1->D1_CONTA,20), .F.))
					    (_cTRB)->(DBAPPEND())
		       			(_cTRB)->EMPRESA     := 'S I R T E C'
			   			(_cTRB)->CONTA       := PadR(SD1->D1_CONTA,20)
		    		ENDIF
		       		(_cTRB)->VALORD      := (_cTRB)->VALORD + Valor //SD1->D1_TOTAL
				    (_cTRB)->DESCEN := (10000000 - (_cTRB)->VALORD)
	...
	//		    	   				(_cTRB)->CTTDESC := CTT->CTT_DESC01
		    ...
		    IF SE5->E5_NATUREZ == PadR('202072003',10) .OR. SE5->E5_NATUREZ == PadR('202072006',10) .OR. SE5->E5_NATUREZ == PadR('201081001',10)
				...
				IF SE2->E2_ORIGEM == PadR('FINA050',8)
					...
					IF SEZ->( dbSeek( SE5->E5_FILIAL + SE5->E5_PREFIXO + SE5->E5_NUMERO + SE5->E5_PARCELA + SE5->E5_TIPO + SE5->E5_CLIFOR + SE5->E5_LOJA, .f. ) )
					    ...
					    While SEZ->( !Eof() ) .and. SEZ->EZ_FILIAL  == SE5->E5_FILIAL ;
						    ...
						    IF PadR(cConta,20) == PadR('3112102013',20)  .OR. PadR(cConta,20) == PadR('3112102014',20)  .OR. PadR(cConta,20) == PadR('3112102015',20)
...
while !EOF()
    ...
    IF  qLinhas == 0
        ...
        If  mv_par03 == 1
    		oprn:say(0300,0500,' G  E  R  A  L : ' + (_cTRB)->EMPRESA+'     -    '+ dtoc(MV_PAR01)+ ' Até '+ dtoc(MV_PAR02) + '     -    Regime de Competência' ,oFont5,100)
        Else
    		oprn:say(0300,0500,' G  E  R  A  L : ' + (_cTRB)->EMPRESA+'     -    '+ dtoc(MV_PAR01)+ ' Até '+ dtoc(MV_PAR02) + '     -    Regime de Caixa' ,oFont5,100)
        EndIF

		oprn:say(0350,0000,Repl("_",1800),oFont55,100)
		oprn:say(0450,0200,'        C O N T A       '  ,oFont5,100)
		oprn:say(0450,1750,'        V a l o r       % Partic. '  ,oFont5,100)
    EndIF
    IF cCTA <> (_cTRB)->CONTA
       cLinha := cLinha + 050
		oprn:say(cLinha,0000,Repl("_",1800),oFont5,100)
       cLinha := cLinha + 100
		oprn:say(cLinha,0000,'  '+(_cTRB)->CONTA ,oFont5,100)
    	oprn:say(cLinha,01820, Transform( (_cTRB)->VALORD-(_cTRB)->VALORC , '@E 9999,999.99' ),oFont6,100)
    	nPerc :=   (((_cTRB)->VALORD-(_cTRB)->VALORC)*100)/nVALORT
        oprn:say(cLinha,02120, Transform( nPerc , '@E 999.999999' ),oFont6,100)
 		dbSelectArea( 'CT1' )
		dbSetOrder( 1 )
	    IF !EMPTY((_cTRB)->CONTA)
			IF dbSeek( xFilial('CT1') + (_cTRB)->CONTA, .F. )
	   		...
	   		EndIF
	    EndIF
		oprn:say(cLinha,0400, (_cTRB)->CT1DESC,oFont5,100)
        cCTA:= (_cTRB)->CONTA

		If mv_par04 == 1
           IF mv_par05 == 2
	        	AAdd(aDados, {(_cTRB)->CONTA,(_cTRB)->CT1DESC,(_cTRB)->VALORD-(_cTRB)->VALORC,MV_PAR01,MV_PAR02}) //adiciona aos dados
           EndIF
        EndIF
...
If  qLinhas == 0
		...
		oprn:say(0050,0700,'I N D I C A D O R  -  C U S T O S   V E I C U L O S  -  S  I  R  T  E  C  - ',oFont55,100)
	   	oprn:say(0180,1250,"SÃO BORJA,   " + Right(dtos(dDataBase),2) + '   de   ' + MesExtenso(str(month(dDataBase),2)) + '   de    ' + Str(Year(dDataBase),4) + '.'  ,oFont4,100)
		oprn:say(0200,0000,Repl("_",1800),oFont55,100)
		oprn:say(0300,0800,' G  E  R  A  L : ' + (_cTRB)->EMPRESA+' '+ dtoc(MV_PAR01)+ ' '+ dtoc(MV_PAR02) ,oFont5,100)
		oprn:say(cLinha,0000,Repl("_",1800),oFont5,100)
       cLinha := cLinha + 100
		oprn:say(cLinha,0000,'         Conta: '+(_cTRB)->CONTA+' - ' ,oFont5,100)
		dbSelectArea( 'CT1' )
		dbSetOrder( 1 )
	    IF !EMPTY((_cTRB)->CONTA)
			IF dbSeek( xFilial('CT1') + (_cTRB)->CONTA, .F. )
	   		...
	   		EndIF
	    EndIF
		oprn:say(cLinha,0500, (_cTRB)->CT1DESC,oFont5,100)
	    cLinha := cLinha + 050
		oprn:say(cLinha,0000,Repl("_",1800),oFont5,100)
        cCTA:= (_cTRB)->CONTA
	    cLinha := cLinha + 050
...
oprn:say(cLinha,01400, 'Total:',oFont5,100)
oprn:say(cLinha,01770, Transform( vTotal , '@E 9999,999.99' ),oFont5,100)
oprn:say(cLinha,02100, '100.00%',oFont5,100)
...
IF  MV_PAR04 == 1  .AND.  MV_PAR05 == 1
	...
	while !EOF()
		IF  (_cTRB3)->E5NUMERO <> PadR(cNOTA,9) .OR. (_cTRB3)->E5NUMERO == PadR(cNOTA,9) .AND. (_cTRB3)->E5CLIFOR <> PadR(cCLIFOR,6) .or. (_cTRB3)->E5NUMERO == PadR(cNOTA,9) .AND. (_cTRB3)->E5CLIFOR == PadR(cCLIFOR,6) .and. (_cTRB3)->E5DATA <> dDataE5
            IF cFlagL == 1
			  	AAdd(aDados, {'','','','','','','TOTAL ','',nTotalE5,'','',nTotalD1,nDifer,'','','','','','','',''}) //adiciona aos dados
            EndIF
			nTotalE5 := 0
			nTotalD1 := 0
			nDifer   := 0
		  	AAdd(aDados, {(_cTRB3)->E5DATA,(_cTRB3)->E5BENEF,(_cTRB3)->E5PREFIXO,(_cTRB3)->E5NUMERO,(_cTRB3)->E5PARCELA,(_cTRB3)->E5TIPODOC,(_cTRB3)->E5CLIFOR,(_cTRB3)->E5LOJA,(_cTRB3)->E5VALOR,(_cTRB3)->D1DOC,(_cTRB3)->D1ITEM,(_cTRB3)->D1VALOR,' ',(_cTRB3)->D1COD,(_cTRB3)->D1DESC,(_cTRB3)->D1CONTA,(_cTRB3)->D1ITEMCTA,(_cTRB3)->D1CC,(_cTRB3)->CTTDESC,(_cTRB3)->D1TIPO,(_cTRB3)->D1DTDIGIT}) //adiciona aos dados
	    ELSE
		  	AAdd(aDados, {(_cTRB3)->E5DATA,(_cTRB3)->E5BENEF,(_cTRB3)->E5PREFIXO,(_cTRB3)->E5NUMERO,(_cTRB3)->E5PARCELA,(_cTRB3)->E5TIPODOC,(_cTRB3)->E5CLIFOR,(_cTRB3)->E5LOJA,' ',(_cTRB3)->D1DOC,(_cTRB3)->D1ITEM,(_cTRB3)->D1VALOR,' ',(_cTRB3)->D1COD,(_cTRB3)->D1DESC,(_cTRB3)->D1CONTA,(_cTRB3)->D1ITEMCTA,(_cTRB3)->D1CC,(_cTRB3)->CTTDESC,(_cTRB3)->D1TIPO,(_cTRB3)->D1DTDIGIT}) //adiciona aos dados
	    ENDIF
		    IF  mv_par03 == 2
				cCLIFOR := (_cTRB3)->E5CLIFOR
 				cNOTA   := (_cTRB3)->E5NUMERO
            	dDATAE5 := (_cTRB3)->E5DATA
            EndIf

        cFlagL  := 1
        nTotalE5 := (_cTRB3)->E5VALOR
        nTotalD1 := nTotalD1 + (_cTRB3)->D1VALOR
        nDifer   := nTotalE5 - nTotalD1
...
If mv_par04 == 1
  ...
  IF mv_par05 == 2
     aCabec := {"Conta","Desc. Conta","Valor","Data Inic","Data Final"} //Criação de um cabeçalho
  ElseIF mv_par03 == 2
	aCabec := {"DATA BAIXA","BENEF","PREFIXO","NUMERO","PARCELA","TIPODOC","CLIFOR","LOJA","VALOR BX","DOCUMEN","ITEM"," VALOR ITEM ","DIFERENCA","CODPROD","DESCPROD"," CONTA"," ITEMCTA"," CCUSTO","CTTDESC"," TIPO"," DTDIGIT"} //Criação de um cabeçalho
  ElseIF mv_par03 == 1
  ...
  EndIF
    IF  mv_par03 == 1
		DlgToExcel({ {"ARRAY", "RINDCVG - Indicadores Sirtec - Veículos - Regime de Competencia", aCabec, aDados} }) // utiiliza a função
    Else
		DlgToExcel({ {"ARRAY", "RINDCVG - Indicadores Sirtec - Veículos - Regime de Caixa", aCabec, aDados} }) // utiiliza a função
    EndIF
EndIF
