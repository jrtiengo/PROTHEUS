	...
	If lOk
		...
		If !Empty( aMarcadas )
			If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
				...
				If lAuto
					If lOk
						MsgInfo( "Atualização realizada.", "UPDSZL" )
					Else
						MsgStop( "Atualização não realizada.", "UPDSZL" )
					EndIf
					dbCloseAll()
				Else
					If lOk
...
//
	...
	'Rotina'																, ; //X3_TITSPA
	'Rotina'																, ; //X3_TITENG
	'Rotina que está sendo exe'												, ; //X3_DESCRIC
	'Rotina que está sendo exe'												, ; //X3_DESCSPA
	'Rotina que está sendo exe'												, ; //X3_DESCENG
	'@!'																	, ; //X3_PICTURE
	''																		, ; //X3_VALID
	'x       x       x       x       x       x       x       x       x       x       x       x       x       x       x x', ; //X3_USADO
	''																		, ; //X3_RELACAO
