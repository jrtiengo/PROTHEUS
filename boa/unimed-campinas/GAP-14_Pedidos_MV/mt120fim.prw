...
// Exemplo de posicionamento (ajuste conforme sua l๓gica):
// SD1->(dbSetOrder(1))
// SD1->(dbSeek(xFilial("SD1") + <chave>))
If !Empty(SD1->D1_COD)
    Posicione('SB1', 1, FWxFilial('SB1') + SD1->D1_COD, "B1_UM")
Else
    // Trate o caso de SD1->D1_COD estar vazio ou SD1 nใo estar posicionado
    ConOut("SD1->D1_COD estแ vazio ou SD1 nใo estแ posicionado.")
EndIf
