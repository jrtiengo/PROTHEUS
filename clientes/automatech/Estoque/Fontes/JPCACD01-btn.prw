...
Else
	Do While .t.
		...
		IF GeraArq() // Gera Arquivo Temporario
			...
			@237,330 ComboBox oPedAtu Var cPedAtu ITEMS { "S=Sem Filtro","T=Total","P=Parcial"} SIZE 040,010 OF oDlgBrw Pixel
			DEFINE SBUTTON FROM 237,370 TYPE 01 ACTION ( AtuArq() ) OF oDlgBrw ENABLE Pixel
			@247,340 Say OemtoAnsi("Selecione Filtro") SIZE 40, 20 OF oDlgBrw PIXEL
						...
						Do While !Eof()
							...
							IF aLstBox[_n, 5] > 0
								...
								If dbSeek( xFilial("SC6") + SEPARA->C5_NUM + aLstBox[ _n, 1 ] )
									RecLock("SC6",.F.)
										If SC6->C6_TEMDOC == "S" .And. SC6->C6_STATUS != "09"
											SC6->C6_STATUS := "09" // Ag. Documenta็ใo cliente
											U_GrvLogSts( SC6->C6_FILIAL, SC6->C6_NUM, SC6->C6_ITEM, "09", "JPCACD01" ) // Gravo o log de atualiza็ใo de status na tabela ZZ0
...
If cJpcSep == "N"
	...
	If dbSeek( SC5->C5_FILIAL + SC5->C5_NUM )
		While !Eof() .And. SC5->C5_FILIAL + SC5->C5_NUM == SC6->C6_FILIAL + SC6->C6_NUM
			If !( SC6->C6_STATUS $ "081112" )
				RecLock("SC6",.F.)
					SC6->C6_STATUS := "08" // Ag. Sep. Estoque
					U_GrvLogSts( SC6->C6_FILIAL, SC6->C6_NUM, SC6->C6_ITEM, "08", "JPCACD01" ) // Gravo o log de atualiza็ใo de status na tabela ZZ0
				MsUnLock()
			EndIf
			SC6->( dbSkip() )
		End
...
Static Function FConfEmb()
	...
	DbSelectArea("SF2")
	DbSetOrder(1)
	DbSeek( xFilial("SF2") + cNumNF + cSerNF )
	...
	If Empty( AllTrim( SF2->F2_CONHECI ) )
 	...
 	Else
 		MsgAlert("Nota fiscal jแ expedida!")
 	EndIf
...
Static Function FGravaJ()
	...
	U_MailSts( SC5->C5_NUM, "E" ) // Envio de E-mail
