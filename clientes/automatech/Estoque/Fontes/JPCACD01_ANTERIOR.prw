...
ฑฑบUso       ณ Automatech - Solutio                                       บฑฑ
ฑฑฬออออออออออุออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
						...
						Do While !Eof()
						   ...
						   IF aLstBox[_n, 5] > 0
							  ...
							  IF _cTipoCod=="S"
								 ...
								 IF !lSBFLOC
									ALERT("SBF - Nใo encontrado produto/armaz้m/NS : "+ CONF->B1_COD +" | "+ CONF->B1_CODBAR +" | "+ aLstBox[_n, 9] )
									Exit
								 ENDIF
...
/*
                        // --------------------------------------------------------------------------------------------------------- //
                        // Realiza a verifica็ใo se o pedido separado ้ proveniente de um pedido de venda referente a L O C A ว ร O. //
                        // Em caso afirmativo, ้ alterado o Status do pedido de venda para 09 Aguardando cliente.                    //
                        // Somente serแ passado para 09 se o pedido de venda for de Loca็ใo e o status neste ponto do programa for   //
                        // = a 10 - Aguarando Faturamento.                                                                           //
                        // Esta altera็ใo foi necessแria pois o faturamento do pedido de remessa de mercadoria para loca็ใo somente  //
                        // poderแ ser efetuado ap๓s o faturamento do pedido referente a medi็ใo do contrato de loca็ใo.              //
                        // --------------------------------------------------------------------------------------------------------- //
                              ...
                              WHILE !T_PRODUTOS->( EOF() )
							     ...
							     If dbSeek( xFilial("SC6") + SEPARA->C5_NUM + T_PRODUTOS->C6_ITEM + T_PRODUTOS->C6_PRODUTO)
                                    If T_PRODUTOS->C6_STATUS == "10"
                                       ...
                                       // Gravo o log de atualiza็ใo de status na tabela ZZ0
...
Static Function FConfirma()
   ...
   // Verifica se o cliente do pedido de venda ้ GKN.
   // Se for, somente permite proceguir se o pedido de venda jแ foi faturado
...
Static Function FGrava()
    ...
    // Se cliente for GKN, abre tela de divisใo dos volumes por produtos
...
User Function VldLote(p1)
   ...
   If ! Empty(cCodLote)
	  ...
	  _cLocProd := aLstBox[_nPosAlst,9] // Jean Rehermann - 21/08/2014 - Pego o armaz้m do produto no item selecionado da listbox
	  ...
	  IF _cTipoCod == "S"
         ...
         cSql += "   AND BF_LOCAL   = '"+ _cLocProd +"'" // Jean Rehermann - 22/08/2014 - Ajustado para considerar o armaz้m do item do pedido
         ...
         If T_ARMAZEM->( EOF() )
			Alert("Produto " + Alltrim(_cCodProd) + " com Controle de Enderecamento, mas Numero de Serie lido nใo ้ desse produto ou nใo tem saldo neste armaz้m !")
			lRetorno := .F.
		 Else
			dbSelectArea("SBF")
...
//			DbSeek(xFilial("SB2")+SB1->B1_COD)
			DbSeek(xFilial("SB2")+SB1->B1_COD+_cLocProd) // Jean Rehermann - 21/08/2014 - Alterado para considerar o armaz้m no seek do SB2
			IF FOUND()
		 ...
		 If lRetorno
			...
			Else
                ...
                // Este teste foi colocado pois exsitem pedidos de etiquetas com quantidade entre 0,1 a 0,9.
                // Com esta verifica็ใo, o Sistema permite separar quantidades fracionadas.
...
For _n := 1 to Len(aLstBox)
	...
	IF p1 == "ATUAL"   // Atualiza o ListBox
		...
		IF ALLTRIM(aLstBox[_n, 2]) == ALLTRIM(cCodProd)
            ...
            // Altera็ใo realizada em 16/12/2014 por Harald Hans L๖schenkohl
            // Existem pedidos de etiquetas que sใo de quantidade 0.5 milheiros
            // Com o teste abaixo, nunca era possํvel de ser separado quantidades < que 1
            // IF _nQtd >= 1  
        	IF _nQtd <> 0  
				lRet := .t.
				aLstBox[_n, 5]+= _nQtd
...
Static Function E_Etiqueta(_Volumes, _Pedido, _cNumNF, _cSerNF)
   ...
   // Se cliente for GKN, envia para a tela de informa็ใo de volumes e impressใo de etiquetas
...
//     MSCBWRITE("~SD18" + chr(13))
       ...
       MSCBWRITE("^FO128,500^AGR,120,40^FD" + Strzero(nEt,2) + "/" + Strzero(nQtetq,2) + "^FS"  + chr(13))//Volume
...
/*       MSCBWRITE("^FT470,020^XGlogoe.GRF,1,1^FS" + chr(13))
       ...
       MSCBWRITE("^FO122,28^XGR:DB3,1,1^FS"  + chr(13))
       MSCBWRITE("^FO10,159^BY4,3.0^BCR,83,N,N,N,N^FD" + Alltrim(cNota) + _Serie + Alltrim(Strzero(nEt,2)) + "^FS" + chr(13))
       MSCBWRITE("^FO128,617^AGR,120,40^FD" + Strzero(nEt,2) + "/" + Strzero(nQtetq,2) + "^FS"  + chr(13))
       MSCBWRITE("^FO128,102^AGR,120,40^FD" + Alltrim(cNota) + "^FS"  + chr(13))
       MSCBWRITE("^FO256,675^ACR,18,10^FDVOLUMES:^FS"  + chr(13))
       MSCBWRITE("^FO238,690^ACR,18,10^FD^FS"  + chr(13))
...
//     MSCBWRITE("^FO442,50^ABR,11,7^FDTRANSP.:^FS"  + chr(13))
       ...
       If Empty(Alltrim(cFantasia))
          MSCBWRITE("^FO416,50^AUR,40,80^FD" + Alltrim(Substr(cTransporte,01,28)) + "^FS" + CHR(13))
       Else
          MSCBWRITE("^FO416,50^AUR,40,80^FD" + Alltrim(Substr(cFantasia,01,28))   + "^FS" + CHR(13))
       Endif
       ...
       // Altera็ใo da emissใo de etiquetas conforme solicita็ใo do Sr. Fernando (13/08/2014)
       // Altera็ใo realizada em fun็ใo de troca de layout de etiqueta
//       MSCBPRINTER("S600",cPorta)
//       MSCBCHKSTATUS(.F.)
//       MSCBBEGIN(2,6,) 
//       MSCBWRITE("^XA" + chr(13))
...
//--------------------------------------------------------------
   ...
   // Desenha o Browse
   oConsulta := TCBrowse():New( 145 , 005, 335, 070,,{'Item','Produto','Part Number','Descri็ใo dos Produtos', 'Und', 'Lacre','Qtd','Unitแrio','Total', 'TES', 'Cod.Fiscal', 'Entrega', 'Status', 'N.Fiscal', 'S้rie', 'Data' },{20,50,50,50},oDlgV,,,,,{||},,,,,,,.F.,,.T.,,.F.,,, )
