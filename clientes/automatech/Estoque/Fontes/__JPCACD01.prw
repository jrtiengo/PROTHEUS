...
//				Endif
			...
			ElseIf nOca == 3
				...
				IF U_MtaConf()
					...
					@ 004,450 SAY OemToAnsi("Nบ Pedido: ") SIZE 060,008 OF oDlgConf PIXEL
                    cCodPedido := SEPARA->C5_NUM
					@ 003,480 MSGET oCodPedido VAR cCodPedido PICTURE "@X" When lChumba SIZE 029,009 OF oDlgConf PIXEL

                    @ 003,325 BUTTON "Embalagem"   Size 40,12 ACTION (U_Embala(SEPARA->C5_NUM))   Of oDlgConf PIXEL
                    @ 003,370 BUTTON "Obs.Interna" Size 40,12 ACTION (OBSINTERNA(SEPARA->C5_NUM)) OF oDlgConf PIXEL

					DEFINE SBUTTON FROM 003,510 TYPE 15 OF oDlgConf PIXEL ENABLE ACTION( ABRE_PEDIDO( SEPARA->C5_NUM ) )
									...
									RecLock("SC6",.F.)
                                        ...
                                        If (SC6->C6_TEMDOC == "S" .OR. SC6->C6_LACRE = "S") .And. SC6->C6_STATUS != "09"
                                            ...
                                            If Alltrim(SC6->C6_LACRE) == "S"                                            
                                               __Lacre += CONF->B1_CODBAR + chr(13) + chr(10)
