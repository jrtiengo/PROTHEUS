      ...
      Do While .t.
		...
		IF GeraArq() // Gera Arquivo Temporario
           ...
           If nTVideo == 970
              ...
              @ C(002),C(002) Jpeg FILE "logoautoma.bmp"    Size C(150),C(026) PIXEL NOBORDER OF oDlgBrw
		      ...
		      @ 240,330 Say OemtoAnsi("Selecione Filtro") SIZE 40, 20 OF oDlgBrw PIXEL
		   ...
		   ElseIf nOca == 3
			  ...
			  IF U_MtaConf()
                 ...
                 If nTVideo == 970
				    ...
				    oGetDb2 := MsGetDB():New(20,40,260,150,1,"U_CFTDOK","U_CFTDOK","",.F., aAlter, ,.F., ,"CONF",Nil,Nil,Nil,oDlgConf)
...
// Função que realiza a verificação da informação do código ou nº de série do produto
User Function VldLote(p1)
   ...
   // Verifica se o nº de série informado já está contido na lista de separação
...
//			DbSeek(xFilial("SB2")+SB1->B1_COD)
		 ...
		 If lRetorno
			...
			Else
                ...
                // Verifica se quantidade é igual a zero.
...
// Função que crarrega o grid de separação
...
// HARALD * HARALD * HARALD * HARALD * HARALD * HARALD * HARALD * HARALD * HARALD * 
User Function Embarque()
   ...
   @ C(002),C(002) Jpeg FILE "logoautoma.bmp" Size C(146),C(030) PIXEL NOBORDER OF oDlgPEDa
   ...
   @ C(142),C(061) Button "Etiqueta"  Size C(037),C(012) PIXEL OF oDlgPEDa When lEditar ACTION( E_Etiqueta(cVolumes, cPedido, cNumNF, cSerNF) )
   @ C(142),C(100) Button "Gravar"    Size C(037),C(012) PIXEL OF oDlgPEDa When lEditar ACTION( FGravaJ() )
   @ C(142),C(139) Button "Abandonar" Size C(037),C(012) PIXEL OF oDlgPEDa When lEditar ACTION( oDlgPEDa:End() )
