...
User Function AUTOM164()
   ...
   // Regra ICMS incluída em 26/05/2014 - Se Cliente for Pessoa Física, não tem valor de ICMS
   If T_PEDIDO->A1_GRPTRIB == "002"
      nPIcms := _QnAliqIcm
   ...
   // Calcula o ICMS ST
//-------->   aRetDife := U_CalDifal(SC6->C6_FILIAL, SC6->C6_PRODUTO, SC6->C6_CLI, SC6->C6_LOJA, cPrcvenda, SC6->C6_TES, nFretecob )

// aRetDife := U_AlqCalcST( SC6->C6_PRODUTO, SC6->C6_CLI, SC6->C6_LOJA, cPrcvenda, SC6->C6_TES )

 
   aRetDife := U_AUTOM232( SC6->C6_FILIAL, SC6->C6_NUM, SC6->C6_ITEM, SC6->C6_PRODUTO, 0, "PV")
...
//   Endif   
   ...
   // Calcula o crédito adjudicado
//------------------------------------------->   cAdjudicado := CreAdjudicado( 0 )

   cAdjudicado := U_AUTOM231(SC6->C6_FILIAL, SC6->C6_NUM, SC6->C6_ITEM, SC6->C6_PRODUTO, 0, "PV")
   ...
   // Verifica qual tela será aberta
   If Select("T_ACESSO") > 0
      T_ACESSO->( dbCloseArea() )
   EndIf

   cSql := ""  
   cSql := "SELECT ZZ4_QUOT,"
   cSql += "       ZZ4_MEP1,"
   cSql += "       ZZ4_MEP2,"
   cSql += "       ZZ4_MEP3,"
   cSql += "       ZZ4_MPOA,"
   ...
   // Verifica se existe parametrizador configurado
   If T_ACESSO->( EOF() )
      MsgAlert("Atenção!" + chr(13) + chr(10) + chr(13) + chr(10) + "Parametrização de acesso a tela de margem inexistente." + chr(13) + chr(10) + "Entre em contato com o administrador do sistema.")
      Return(.T.)
   Endif
            
   // Verifica se usuário está contido na liberação de acessos 1 e 2
   nAcesso_01 := IIF(U_P_OCCURS(T_ACESSO->ZZ4_QUOT, Alltrim(__cUserID), 1) == 0, .F., .T.)

   If U_P_OCCURS(T_ACESSO->ZZ4_MUS1, Alltrim(__cUserID), 1) == 0
      nAcesso_02 := .F.
      If U_P_OCCURS(T_ACESSO->ZZ4_MUS2, Alltrim(__cUserID), 1) == 0
         nAcesso_02 := .F.
         If U_P_OCCURS(T_ACESSO->ZZ4_MUS3, Alltrim(__cUserID), 1) == 0
            nAcesso_02 := .F.
         Else
            nAcesso_02 := .T.            
         Endif
   ...
   If nAcesso_01 == .F. .And. nAcesso_02 == .F.
      MsgAlert("Atenção!" + chr(13) + chr(10) + chr(13) + chr(10) + "Parametrização de acesso a tela de margem inexistente." + chr(13) + chr(10) + "Entre em contato com o administrador do sistema.")
      Return(.T.)
   Endif
 
   // Acesso para usuários secundários
   If nAcesso_01 == .F. .And. nAcesso_02 == .T.

      // Verifica se usuário tem acesso a Empresa/Filial
      
      // Grupo Empresa 01- Automatech Sistemas de Automação Ltda
      If cEmpAnt == "01"
         
         If T_ACESSO->ZZ4_MEP1 == "T"
              
            Do Case 
               Case cFilAnt == "01"
                    lAbreTela := IIF(T_ACESSO->ZZ4_MPOA == "T", .T., .F.)
               Case cFilAnt == "02"
                    lAbreTela := IIF(T_ACESSO->ZZ4_MCXS == "T", .T., .F.)
               Case cFilAnt == "03"
                    lAbreTela := IIF(T_ACESSO->ZZ4_MPEL == "T", .T., .F.)
               Case cFilAnt == "04"
                    lAbreTela := IIF(T_ACESSO->ZZ4_MSUP == "T", .T., .F.)
               Case cFilAnt == "07"
                    lAbreTela := IIF(T_ACESSO->ZZ4_MSUP == "T", .T., .F.)
            EndCase        
      ...
      If cEmpAnt == "02"
         ...
         If T_ACESSO->ZZ4_MEP2 == "T"
              
            Do Case 
               Case cFilAnt == "01"
                    lAbreTela := IIF(T_ACESSO->ZZ4_MTIC == "T", .T., .F.)
            EndCase        
      ...
      If cEmpAnt == "03"
         ...
         If T_ACESSO->ZZ4_MEP3 == "T"
              
            Do Case 
               Case cFilAnt == "01"
                    lAbreTela := IIF(T_ACESSO->ZZ4_MATC == "T", .T., .F.)
            EndCase        
      ...
      If lAbretela == .F.
         MsgAlert("Atenção!" + chr(13) + chr(10) + chr(13) + chr(10) + "Usuário sem permissão para efetuar esta operação." + chr(13) + chr(10) + "Entre em contato com seu supervisor.")
         Return(.T.)
      Endif
  
      // Desenha a tela para os usuários secundários
      DEFINE MSDIALOG oDlg TITLE "Parâmetros Calculo Margem" FROM C(186),C(190) TO C(571),C(839) PIXEL

      @ C(002),C(002) Jpeg FILE "logoautoma.bmp"    Size C(134),C(026) PIXEL NOBORDER OF oDlg

      @ C(168),C(005) GET oMemo1 Var cMemo1 MEMO    Size C(314),C(001) PIXEL OF oDlg
      @ C(138),C(005) GET oMemo6 Var cMemo6 MEMO    Size C(314),C(001) PIXEL OF oDlg
      @ C(032),C(005) GET oMemo7 Var cMemo7 MEMO    Size C(314),C(001) PIXEL OF oDlg

      @ C(035),C(294) Say "Qtd Vda"                 Size C(023),C(008) COLOR CLR_BLACK PIXEL OF oDlg
      @ C(036),C(005) Say "Nº PV"                   Size C(018),C(008) COLOR CLR_BLACK PIXEL OF oDlg
      @ C(036),C(045) Say "Produto"                 Size C(020),C(008) COLOR CLR_BLACK PIXEL OF oDlg
      @ C(036),C(136) Say "Descrição do Produto"    Size C(054),C(008) COLOR CLR_BLACK PIXEL OF oDlg
      @ C(056),C(005) Say "Cliente"                 Size C(021),C(008) COLOR CLR_BLACK PIXEL OF oDlg
      @ C(056),C(161) Say "Ciade"                   Size C(015),C(008) COLOR CLR_BLACK PIXEL OF oDlg
      @ C(076),C(005) Say "Cond. de Pagtº"          Size C(037),C(008) COLOR CLR_BLACK PIXEL OF oDlg
      @ C(095),C(005) Say "Transportadora"          Size C(036),C(008) COLOR CLR_BLACK PIXEL OF oDlg
      @ C(095),C(291) Say "Tipo Frete"              Size C(025),C(008) COLOR CLR_BLACK PIXEL OF oDlg
      @ C(115),C(005) Say "T E S"                   Size C(021),C(008) COLOR CLR_BLACK PIXEL OF oDlg
      @ C(115),C(149) Say "Vendedor"                Size C(025),C(008) COLOR CLR_BLACK PIXEL OF oDlg
      @ C(143),C(124) Say "Magem do Item"           Size C(038),C(008) COLOR CLR_BLACK PIXEL OF oDlg
      @ C(143),C(175) Say "%"                       Size C(007),C(008) COLOR CLR_BLACK PIXEL OF oDlg

      @ C(045),C(005) MsGet oGet1  Var cPedido      Size C(034),C(009) COLOR CLR_BLACK Picture "@!" PIXEL OF oDlg When lChumba
      @ C(045),C(045) MsGet oGet2  Var cProduto     Size C(085),C(009) COLOR CLR_BLACK Picture "@!" PIXEL OF oDlg When lChumba
      @ C(045),C(136) MsGet oGet3  Var cDescricao   Size C(151),C(009) COLOR CLR_BLACK Picture "@!" PIXEL OF oDlg When lChumba
      @ C(045),C(294) MsGet oGet4  Var cQuantidade  Size C(024),C(009) COLOR CLR_BLACK Picture "@!" PIXEL OF oDlg When lChumba
      @ C(065),C(005) MsGet oGet39 Var __Cliente    Size C(150),C(009) COLOR CLR_BLACK Picture "@!" PIXEL OF oDlg When lChumba
      @ C(065),C(161) MsGet oGet46 Var __Cidade     Size C(157),C(009) COLOR CLR_BLACK Picture "@!" PIXEL OF oDlg When lChumba
      @ C(084),C(005) MsGet oGet9  Var __Condicao   Size C(313),C(009) COLOR CLR_BLACK Picture "@!" PIXEL OF oDlg When lChumba
      @ C(104),C(005) MsGet oGet21 Var __Transporte Size C(280),C(009) COLOR CLR_BLACK Picture "@!" PIXEL OF oDlg When lChumba
      @ C(104),C(291) MsGet oGet24 Var cTipoFrete   Size C(027),C(009) COLOR CLR_BLACK Picture "@!" PIXEL OF oDlg When lChumba
      @ C(124),C(005) MsGet oGet23 Var __TES        Size C(138),C(009) COLOR CLR_BLACK Picture "@!" PIXEL OF oDlg When lChumba
      @ C(124),C(149) MsGet oGet45 Var __Vendedor   Size C(169),C(009) COLOR CLR_BLACK Picture "@!" PIXEL OF oDlg When lChumba
      @ C(153),C(124) MsGet oGet18 Var vMargem      Size C(045),C(009) COLOR CLR_BLACK Picture "@!" PIXEL OF oDlg When lChumba
      @ C(153),C(175) MsGet oGet40 Var cMargem      Size C(022),C(009) COLOR CLR_BLACK Picture "@!" PIXEL OF oDlg When lChumba

      @ C(175),C(138) Button "Retornar" Size C(048),C(012) PIXEL OF oDlg
      
   Endif
   
   // Acesso para usuários primários
   If nAcesso_01 == .T. .And. nAcesso_02 == .F.

      // Desenha a Tela para os usuários principais
      ...
      @ C(022),C(189) Say "CiDade"                     Size C(015),C(008) COLOR CLR_BLACK PIXEL OF oDlg
   ...
   // @ C(127),C(122) Button "..."                     Size C(011),C(009) PIXEL OF oDlg
      ...
      @ C(182),C(171) Say "DIFAL"                      Size C(045),C(008) COLOR CLR_BLACK PIXEL OF oDlg
   ...
   // @ C(127),C(088) MsGet    oGet43     Var pAdjudicado  When lChumba   Size C(022),C(009) COLOR CLR_BLACK Picture ("@E 999.99")       PIXEL OF oDlg
      ...
      //////////   @ C(182),C(190) Button "Cáculo"                                     Size C(025),C(009) PIXEL OF oDlg ACTION( AbreDifal(cEmpAnt, SC6->C6_FILIAL, SC6->C6_NUM) )

      @ C(182),C(190) Button "Cáculo"                                     Size C(025),C(009) PIXEL OF oDlg ACTION( U_AUTOM232( SC6->C6_FILIAL, SC6->C6_NUM, SC6->C6_ITEM, SC6->C6_PRODUTO, 1, "PV") )
      ...
      // @ C(127),C(089) Button "Cálculo"    Size C(025),C(009) PIXEL OF oDlg ACTION( CreAdjudicado(1) )
      @ C(127),C(089) Button "Cálculo"    Size C(025),C(009) PIXEL OF oDlg ACTION( U_AUTOM231(SC6->C6_FILIAL, SC6->C6_NUM, SC6->C6_ITEM, SC6->C6_PRODUTO, 1, "PV") )
...
User Function AlqCalcST( cProd, cCli, cLoja, nValor, cTes )
...
Static Function TelaAdjudicado( xProduto, xCliente, xLoja, xUnitario, xTes )
   ...
   aRetDife := U_AlqCalcST( xProduto, xCliente, xLoja, xUnitario, xTes )
...
User Function CalDifal(D_Filial, D_Produto, D_Cliente, D_Loja, D_Unitarioi, D_TES, D_FRETE )
...
// ----------------------------------------------------------------------------------------------------
// Abre tela que mostra variáveis e cálculo do crédito adjudicado                                    --
// Parãmetro: Tipo onde 0 - Realiza somente o cálculo do crédito adjudicado                          --
//                      1 - Calcula o crédito adjudicado e abre tela para visualização das variáveis --
// ----------------------------------------------------------------------------------------------------
Static Function CreAdjudicado( _Tipo )

   Local lChumba     := .F.
   Local cSql        := ""

   Local cPedido     := SC6->C6_NUM
   Local cProduto    := Alltrim(SC6->C6_PRODUTO) + " - " + Alltrim(SC6->C6_DESCRI)
   Local cNFentrada  := ""
   Local cDataEntra  := ""
   Local cFornecedor := ""
   Local cUFEntrada  := ""
   Local cGProduto   := ""
   Local cNPedido	 := SC6->C6_NUM
   Local cDEmissao	 := Posicione("SC5",1,SC6->C6_FILIAL + SC6->C6_NUM, "C5_EMISSAO")
   Local cCliente	 := Alltrim(SC6->C6_CLI) + "." + Alltrim(SC6->C6_LOJA) + " - " + Alltrim(Posicione("SA1", 1, xFilial("SA1") + SC6->C6_CLI + SC6->C6_LOJA, "A1_NOME"))
   Local cUFVenda	 := Posicione("SA1", 1, xFilial("SA1") + SC6->C6_CLI + SC6->C6_LOJA, "A1_EST")
   Local cGCliente	 := Posicione("SA1", 1, xFilial("SA1") + SC6->C6_CLI + SC6->C6_LOJA, "A1_GRPTRIB")
   Local cCFOP  	 := SC6->C6_CF
   Local cBase	     := 0
   Local cQEntrada	 := 0
   Local cQVenda	 := 0
   ...
   cSql += "   AND SD1.D1_COD     = '" + Alltrim(SC6->C6_PRODUTO) + "'"
  	  ...
  	  If Alltrim(cUFEntrada) <> Alltrim(SM0->M0_ESTENT)      
         If Alltrim(cGProduto) <> "017"
     	    If Alltrim(cUFVenda) <> Alltrim(SM0->M0_ESTENT)               
               If Alltrim(Posicione("SA1", 1, xFilial("SA1") + SC6->C6_CLI + SC6->C6_LOJA, "A1_GRPTRIB")) <> "003"
   ...
   cSql += "   AND X5_CHAVE   = '" + Alltrim(Posicione("SA1", 1, xFilial("SA1") + SC6->C6_CLI + SC6->C6_LOJA, "A1_GRPTRIB")) + "'"
   cSql += "   AND D_E_L_E_T_ = ''"
