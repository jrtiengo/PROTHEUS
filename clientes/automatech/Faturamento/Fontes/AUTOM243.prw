...
// cFilAnt   := w_Filial
   ...
   If DbSeek( w_Filial + w_Oportunidade + w_Revisao) 
      ...
      If AD1_ZTIP == "2"
         ...
         U_AUTOM254(w_Filial, w_Oportunidade, w_Proposta, w_Revisao)
...
Static Function xAbrPComercial(_Operacao, _Oportunidade, _xFilial, _PorOnde, _aAlegenda, _PropostaK )
   ...
   If _Operacao == "I"
      ...
      // ########################################
      // Captura o nmero da proposta comercial ##
      // ########################################
      kk_Proposta := ADY->ADY_PROPOS      

      // ############################      
      // Abre a Proposta Comercial ##
      // ############################
      A600Mnt("ADY", ADY->( Recno() ), 4)
  
      // ################################################################
      // Pesquisa o nº do orçamento na tabela SCJ para pesquisa da SCK ##
      // ################################################################
      If Select("T_ORCAME") > 0
         T_ORCAME->( dbCloseArea() )
      EndIf

      cSql := ""
      cSql := "SELECT CJ_NUM" 
      cSql += "  FROM " + RetSqlName("SCJ") + "(NoLock)"
      cSql += " WHERE CJ_FILIAL  = '" + Alltrim(Substr(cComboBx5,01,02)) + "'"
      cSql += "   AND CJ_PROPOST = '" + Alltrim(kk_Proposta)             + "'"
      cSql += "   AND D_E_L_E_T_ = ''"

      cSql := ChangeQuery( cSql )
      dbUseArea( .T., "TOPCONN", TcGenQry(,,cSql), "T_ORCAME", .T., .T. )

      kk_Orcamento := T_ORCAME->CJ_NUM

      // ############################################
      // Pesquisa a última revisão para pesquisada ##
      // ############################################
      ...
      cSql += "  FROM " + RetSqlName("ADZ") + "(NoLock)"
      cSql += " WHERE ADZ_FILIAL = '" + Alltrim(Substr(cComboBx5,01,02)) + "'"
      cSql += "   AND ADZ_PROPOS = '" + Alltrim(kk_Proposta)             + "'"
      cSql += "   AND D_E_L_E_T_ = '' 
      ...
      k_Ultima_Revisao := T_REVISAO->ADZ_REVISA
      ...
      cSql += "   AND B.ADZ_REVISA = '" + Alltrim(k_Ultima_Revisao)        + "'"
      cSql += "   AND A.D_E_L_E_T_ = ''"
      ...
      If !T_COMISSAO->( EOF() )
         ...
         WHILE !T_COMISSAO->( EOF() )
            ...
            DbSelectArea("SCK")
            DbSetorder(1)
//          If DbSeek(T_COMISSAO->ADY_FILIAL + kk_Orcamento + T_COMISSAO->ADZ_ITEM + T_COMISSAO->ADZ_PRODUT)
            If DbSeek(T_COMISSAO->ADY_FILIAL + kk_Orcamento + T_COMISSAO->ADZ_ITEMOR + T_COMISSAO->ADZ_PRODUT)
               RecLock("SCK",.F.)            
               SCK->CK_COMIS1 := T_COMISSAO->ADZ_COMIS1
               SCK->CK_COMIS2 := T_COMISSAO->ADZ_COMIS2
               SCK->CK_ENTREG := Ctod(Substr(T_COMISSAO->ADZ_DTENTR,07,02) + "/" + Substr(T_COMISSAO->ADZ_DTENTR,05,02) + "/" + Substr(T_COMISSAO->ADZ_DTENTR,01,04))
               MsUnLock()            
            Endif
   ...
   If _Operacao == "A"
      ...
      If _PorOnde == "S"
         ...
         If Substr(cComboBx6,01,01) == "9"
            ...
            // até que o vendedor resolva as pendências apresentadas.                                         ##
            // #################################################################################################
         ...
         If Substr(cComboBx6,01,01) == "9"
            ...
            // ##########################################
            If Select("T_CPROPOSTA") > 0
               T_CPROPOSTA->( dbCloseArea() )
            EndIf
            ...
            cSql += "       ADY_OPORTU,"
      	    cSql += "       ADY_PROPOS "
            cSql += "  FROM " + RetSqlName("ADY") + " (NoLock)"
            cSql += " WHERE ADY_FILIAL = '" + Alltrim(Substr(cComboBx5,01,02)) + "'"
            cSql += "   AND ADY_OPORTU = '" + Alltrim(xOportunidade)           + "'"
            cSql += "   AND D_E_L_E_T_ = ''"

            cSql := ChangeQuery( cSql )
            dbUseArea( .T., "TOPCONN", TcGenQry(,,cSql), "T_CPROPOSTA", .T., .T. )

            kk_Proposta := T_CPROPOSTA->ADY_PROPOS
            ...
            // #####################################################################################################
            // Verifica se existe registro da oportunidade/proposta comercial na tabela SCJ (Orçamento)           ##
            ...
            If cCliente == "008752"
               ...
               cSql := "SELECT TOP(1) ADZ.ADZ_FILIAL,"
               cSql += "       ADZ.ADZ_PROPOS," 
       	       cSql += "       ADZ.ADZ_ORDC  ,"
    	       cSql += "       ADZ.ADZ_ORDA  ,"
    	       cSql += "       ADZ.ADZ_ORDS   "
               ...
               cSql += "   AND ADY.ADY_OPORTU = '" + Alltrim(xOportunidade)           + "'"
               cSql += " ORDER BY ADZ.ADZ_REVISA DESC"
      ...
      If _PorOnde == "A"
         ...
         // ##########################################
         // Captura o núumero da proposta comercial ##
         // ##########################################
         kk_Proposta := ADY->ADY_PROPOS

         // ############################
         // Abre a Propsota Comercial ##
         // ############################
         A600Mnt("ADY", ADY->( Recno() ), 4 )
         ...
         cSql += "       A.ADY_OPORTU,"
         cSql += "       B.*          "
         cSql += "  FROM " + RetSqlName("ADY") + " A (NoLock), " 
         cSql += "       " + RetSqlName("ADZ") + " B (NoLock)  " 
         cSql += " WHERE A.ADY_FILIAL = '" + Alltrim(Substr(cComboBx5,01,02)) + "'"
         cSql += "   AND A.ADY_OPORTU = '" + Alltrim(xOportunidade)           + "'"
         cSql += "   AND B.ADZ_REVISA = '" + Alltrim(k_Ultima_Revisao)        + "'" 
         cSql += "   AND A.D_E_L_E_T_ = ''"
         ...
         If !T_COMISSAO->( EOF() )
            ...
            WHILE !T_COMISSAO->( EOF() )
               ...
               If DbSeek(T_COMISSAO->ADY_FILIAL + kk_Orcamento + T_COMISSAO->ADZ_ITEM + T_COMISSAO->ADZ_PRODUT)
                  ...
                  SCK->CK_COMIS2 := T_COMISSAO->ADZ_COMIS2
                  SCK->CK_ENTREG := Ctod(Substr(T_COMISSAO->ADZ_DTENTR,07,02) + "/" + Substr(T_COMISSAO->ADZ_DTENTR,05,02) + "/" + Substr(T_COMISSAO->ADZ_DTENTR,01,04))
                  SCK->CK_PRUNIT := 0 && Zera o preço unitário vindo da tabela de preço para evitar problema de dar desconto em nota fiscal
                  MsUnLock()            
               Endif
...
Static Function EBOLETOS(ac_filial, ac_pedido)
   ...
   Else   
      ...
      Else
         Do Case
            Case kTipo_Banco == "1"
                 U_SANTANDER(.T., cNota_Fiscal, cSerie_Fiscal )
            Case kTipo_Banco == "2"
