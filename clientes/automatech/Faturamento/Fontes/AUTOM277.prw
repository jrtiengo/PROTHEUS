...
User Function AUTOM277()
   ...
   DEFINE MSDIALOG oDlgP TITLE "Verificador Discionário de Dados" FROM C(178),C(181) TO C(543),C(524) PIXEL && 507
   ...
   @ C(163),C(003) GET oMemo3 Var cMemo3 MEMO Size C(164),C(001) PIXEL OF oDlgP
   ...
   @ C(138),C(007) Say "Tipo de arquivo a ser aberto"                      Size C(068),C(008) COLOR CLR_BLACK PIXEL OF oDlgP
   ...
   @ C(168),C(047) Button "Verificar" Size C(037),C(012) PIXEL OF oDlgP ACTION( RodaVerificao() )
   @ C(168),C(088) Button "Voltar"    Size C(037),C(012) PIXEL OF oDlgP ACTION( oDlgP:End() )
...
Static Function RodaVerificao()
   ...
   If Substr(cAbertura,01,02) == "00" 
      Msgalert("Tipo de Abertura de Arquivo não selecionado.")
      Return(.T.)
   Endif
...
Static Function GrVerificacao()
   ...
   Do Case
      ...
      Case Substr(cOrigem,01,02) == "04"

           If Substr(cAbertura,01,02) == "01"
              If Substr(cArquivos,01,02) == "01"
                 dbUseArea(.T., "DBFCDX", "SX2040.DBF", "DBF_ORIGEM", .T., .F.)
              Else
                 dbUseArea(.T., "DBFCDX", "SX2040.DBF", "DBF_TABELA", .T., .F.)
                 dbUseArea(.T., "DBFCDX", "SX3040.DBF", "DBF_ORIGEM", .T., .F.)
              Endif
           Else
              If Substr(cArquivos,01,02) == "01"
                 dbUseArea(.T., , "SX2010.DTC", "DBF_ORIGEM", .T., .F.)
              Else
                 ...
                 dbUseArea(.T., , "SX2040.DTC", "DBF_TABELA", .T., .F.)
                 dbUseArea(.T., , "SX3040.DTC", "DBF_ORIGEM", .T., .F.)                 
              Endif
           Endif
