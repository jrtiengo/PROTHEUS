...
// ------------------------------------------------------------------------------- ##
// Referencia: AUTOM580.PRW                                                        ##
// Parâmetros: Nenhum                                                              ##
// Tipo......: (X) Programa  ( ) Gatilho  ( ) Ponto de Entrada                     ##
// ------------------------------------------------------------------------------- ##
// Autor.....: Harald Hans Löschenkohl                                             ##
// Data......: 09/06/2017                                                          ##
// Objetivo..: Programa que gera a Danfe em disco para envio ao Cliente            ##
// ##################################################################################

User Function AUTOM580(kFilial, kDocumento, kSerie)
   ...
   // ######################################### 
   // Verifica se existe a pasta XMLDANFEBOL ##
   // #########################################
   If !ExistDir( "C:\XMLDANFEBOL" )
      ...
      If nRet != 0
         MsgAlert("Não foi possível criar a pasta XMLDANFEBOL. Entre em contato com o Administrador do Sistema. Erro: " + cValToChar( FError() ) )
         Return(.T.)
      Endif
   ...
   // #####################################################
   // Pesquisa dados da nota fiscal a ser gerada a DANFE ##
   // #####################################################
   dbSelectArea("SF3")
   DbSetOrder(6)
   If DbSeek(kFilial + kDocumento + kSerie)
   Else
      Return(.T.)
   Endif   
   ...
   If oDanfe == Nil
      lPreview := .T.
	  oDanfe   := FWMSPrinter():New('Danfe_' + Alltrim(SF3->F3_NFISCAL) + "_" + Alltrim(SF3->F3_SERIE),6,.F.,,.T.)
	  oDanfe:SetPortrait()
   ...
   // ############################################################
   // Envia para a função que gera a impressão da DANFE gráfica ##
   // ############################################################
   ...
   // #######################
   nHandle := FOPEN("C:\XMLDANFEBOL\Danfe_" + Alltrim(SF3->F3_NFISCAL) + "_" + Alltrim(SF3->F3_SERIE) + ".pdf" , FO_READWRITE + FO_SHARED )
   ...
   // ######################################################################################
   If File("C:\XMLDANFEBOL\DANFE_" + Alltrim(SF3->F3_NFISCAL) + "_" + Alltrim(SF3->F3_SERIE) + ".PDF")
      Ferase("C:\XMLDANFEBOL\DANFE_" + Alltrim(SF3->F3_NFISCAL) + "_" + Alltrim(SF3->F3_SERIE) + ".PDF")
   Endif

   // ######################
   // Grava o arquivo PDF ##
   // ######################
   kCaminho := "C:\XMLDANFEBOL\DANFE_" + Alltrim(SF3->F3_NFISCAL) + "_" + Alltrim(SF3->F3_SERIE) + ".PDF"
   nHdl := fCreate(kCaminho)
   fWrite (nHdl, kString ) 
   fClose(nHdl)

   // ############################################################
   // Enviar e-mail do Danfe.pdf para o e-mail do Cliente       ##
...
// ############################################
// Função que gera o objeto gráfico da DANFE ##
// ############################################
...
For i := 1 To Len(_aFiles)
    ...
    if Alltrim(vMail) <> ""
        ...
        // Colocar aqui a gravação da danfe na pasta desejada
...
Else
	Aviso('E-Mail','E-Mail NÃO pode ser enviado!',{"OK"},2,"Favor Verificar.",) 
EndIf
