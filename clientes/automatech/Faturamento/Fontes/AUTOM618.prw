...
// Data......: 18/08/2017                                                                 ##
// Objetivo..: Programa que gera automaticamente a liberação de pedido de venda dos tipos ##
...
//                         1 - Pedidos de Serviços (Assistência Técnica)                  ##
...
User Function AUTOM618(K_Filial, K_Pedido, K_Tipo)
   ...
   // ####################################################
   // Libera produtos/serviços na efetivação de OS (AT) ##
   // ####################################################
   If K_Tipo == 1
...
//     	     SC6->C6_STATUS := "03"
         ...
         // ##############################################################
         U_GrvLogSts(xFilial("SC6"),SC6->C6_NUM, SC6->C6_ITEM, "10", "AUTOM618 (AT)") // Gravo o log de atualização de status na tabela ZZ0
   ...
   If K_Tipo == 2
      ...
      WHILE !T_PRODUTOS->( EOF() )
         ...
         // Atualiza a tabela ZZ0 (Tabela de Status de Pedido de Venda) ##
         // ##############################################################
         U_GrvLogSts(xFilial("SC6"),SC6->C6_NUM, SC6->C6_ITEM, "10", "AUTOM618 (CONSERTO)") // Gravo o log de atualização de status na tabela ZZ0
   ...
   If K_Tipo == 3
      ...
      WHILE !T_PRODUTOS->( EOF() )
         ...
         // #####################################################################
         // Atualiza o campo C5_LIBEROK para liberar o pedido de Intermediação ##
         // #####################################################################
         dbSelectArea("SC5")
         DBSetOrder(1)
         DbSeek ( T_PRODUTOS->C6_FILIAL + T_PRODUTOS->C6_NUM )
         ...
         // Atualiza a tabela ZZ0 (Tabela de Status de Pedido de Venda) ##
         // ##############################################################
         U_GrvLogSts(xFilial("SC6"),SC6->C6_NUM, SC6->C6_ITEM, "10", "AUTOM618 (TES 766: DEV TROCA GARANTIA)") // Gravo o log de atualização de status na tabela ZZ0
