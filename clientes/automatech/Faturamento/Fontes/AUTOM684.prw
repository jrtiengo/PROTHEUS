...
//             dos a Clientes. Estes documentos serão gerados em pastas do  servi- ##
//             dor de aplicação, sendo XML e DANFE em uma pasta e boletos na  pas- ##
//             ta TREPORT do Protheus_Data (Exeigência do comando do Protheus).    ##
// Parâmetros: 0 - Programa é executado por scheduler                              ##
//             1 - Programa é executado pela chamada do menu                       ##
// Retorno...: .T.                                                                 ##
// ##################################################################################
...
User Function AUTOM684(kTipo, kEmpresa, kFilial)
   ...
   Private aTipoPesq := {"0 - Todos Doc. (Já gerados e não gerados)", "1 - Somente não gerados"}
   Private cComboBx1
   ...
   cTexto += "no Servidor da Aplicação.              " + chr(13) + chr(10) + chr(13) + chr(10)
   cTexto += "Isso é necessário  devido  a forma como" + chr(13) + chr(10)
   cTexto += "são gerados e salvos  estes  documentos" + chr(13) + chr(10)
   cTexto += "por este programa.                     " + chr(13) + chr(10) + chr(13) + chr(10)
   cTexto += "Utilize períodos curtos, pois este é um" + chr(13) + chr(10)
   cTexto += "procedimento de execução lenta.        "
   ...
   Else      
      DEFINE MSDIALOG oDlg TITLE "Geração de Documentos (DANFE, XML e Boletos)" FROM C(178),C(181) TO C(548),C(463) PIXEL
      ...
      @ C(037),C(005) Say "Emissão Inicial" Size C(037),C(008) COLOR CLR_BLACK PIXEL OF oDlg
      @ C(037),C(047) Say "Emissão Final"   Size C(035),C(008) COLOR CLR_BLACK PIXEL OF oDlg
...
// ################################################################################# 
// Função que gera dos documentos conforme data informada e parâmetro de pesquisa ##
// #################################################################################
...
//         MsgAlert("Salvar Em não foi indicado. Verifique!")
//         Return(.T.)
//      Endif
//      
//      If lLocal == .T. .Or. lServidor == .T.
//         MsgAlert("Indique apenas um local onde os arquivos serão salvos. Verifique!")
//         Return(.T.)
//      Endif

      MsgRun("Aguarde! Gerando Documentos para o período ...", "Gerador de Documentos",{|| xxAUTOMA684(kTipo, kEmpresa, kFilial) })
...
Static Function xxAUTOMA684(kTipo, kEmpresa, kFilial)
   ...
   // ###########################################################
   // Variáveis para a rotina de geração do XML dos documentos ##
   // ###########################################################
   Local lTemMarcado := .F.
   Local cCaminho    := ""
   Local cString     := ""
   ...
   Else
      If cDataIni == Ctod("  /  /    ")
         MsgAlert("Data inicial de pesquisa não informada. Verifique!")
         Return(.T.)
      Endif
       
      If cDataFim == Ctod("  /  /    ")
         MsgAlert("Data final de pesquisa não informada. Verifique!")
         Return(.T.)
      Endif
   Endif   
...
//   Endif   
   ...
   Else
      If !ExistDir("C:\XMLDANFEBOL")
         ...
         If nRet != 0
            MsgAlert("Não foi possível criar a pasta: C:\XMLDANFEBOL")
            Return(.T.)
         Endif
      Endif
   Endif   
   ...
   // ######################################################################
   // Verifica se existe a pasta RETASSDOCU. Caso não exista, será criada ##
   // ######################################################################
   If kTipo == 0
   ...
   Else
      If !ExistDir("C:\RETASSDOCU")
         ...
         If nRet != 0
            MsgAlert("Não foi possível criar a pasta: D:\Protheus11\Protheus_data\RETASSDOCU")         
            Return(.T.)
         Endif
      Endif
   Endif
   ...
   WHILE !T_NOTAS->( EOF() )
      ...
      // Parâmetros de envio                                        ##
      // Empresa   -> Código da Empresa ref. ao documento           ##
      // Filial    -> Código da Filial ref. ao documento            ##
      // Documento -> Nº da Nota Fiscal                             ##
      // Série     -> Nº de Série da Nota Fiscal                    ##
...
//    U_AUTOM651(cEmpAnt, T_NOTAS->F2_FILIAL, T_NOTAS->F2_DOC, T_NOTAS->F2_SERIE, T_NOTAS->F2_CLIENTE, T_NOTAS->F2_LOJA, kTipo)
   ...
   WHILE !T_NOTAS->( EOF() )
      ...
      // #######################################################################
      // Envia para o programa que gera o boleto bancário da nota fiscal lida ##
      // #######################################################################
   ...
   For nContar = 1 to Len(aDocumentos)
       ...
       // #####################################################
       // Verifica se o documento existe no diretório        ##
...
// ##############################################################
// Função que realiza a impressão da Danfe conforme parâmetros ##
// ##############################################################
Static Function XAUTOM651(kEmpresa, kFilial, kNota, kSerie, kCliente, kLoja, kTipo)
   ...
   If DbSeek( kFilial + kNota + kSerie)
      ...
      // #####################################################################################################################
      // Elimina o arquivo da Danfe do diretório para que o sistema não solicite se deseja gravar sobre o arquivo existente ##
      // #####################################################################################################################
      If kTipo == 0
...
// ################################################
// Função que gera o Objeto Gráfico de Impressão ##
// ################################################  
...
@param  aCabec     Array contendo as strings de cabeçalho das colunas
        aValores   Array contendo os valores que serão populados nas
                   colunas.
        oDanfe   Objeto de impressão instanciado para utilizar o método
...
@param  cTexto   Texto a ser medido.
        ...
        oDanfe Objeto de impressão instanciado.
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³ OKPROC   ºAutor  ³ Iranildo Cavalcante  º Data ³ 25/04/2012º±±
...
For i := 1 To Len(_aFiles)
    ...
    if Alltrim(vMail) <> ""
		...
		cTexto += "Segue Danfe Nº "+Alltrim(vNota)+"." 
		cTexto += '</font>'
		cTexto += '</b>'
		cTexto += '</p>'
		cTexto += '<hr>'
		...
		cTexto += '<h3 align="center" style="color: #ff0000;">>>>>>>>>>> ATENCÃO <<<<<<<<<<</h3>'
		cTexto += '<h3 align="left">RECOMENDACÃO IMPORTANTE:</h3>'
		cTexto += '<p align="justify">Prezado Cliente, favor conferir atentamente seus <b>DADOS CADASTRAIS</b> impressos na Danfe da NF-e, que acabam de chegar. Por determinacão da <b>SEFAZ - SECRETARIA DA FAZENDA</b>, após 1 dias de sua emissão, a Danfe da NF-e <b>NÃO PODERÃO SEREM CANCELADAS</b>.</p>'
		cTexto += '<p align="justify">Caso identifique algum dado inconsistente, entre em contato imediatamente com nossa Gerencia Comercial através do telefone: (85)3218-7065.</p>'
	
		cTexto += '<p align="LEFT"><b>'
		cTexto += 'Caso queira responder a este e-mail, gentileza encaminhar para o endereço:'
		cTexto += '<br>'
		cTexto += '<a href="mailto:gerencia.comercial@cogerh.com.br">gerencia.comercial@cogerh.com.br'
		cTexto += '</a></b></p>'
		cTexto += '<p align="LEFT">'
		cTexto += 'Cia. de Gestão dos Recusros Hídricos do Estado do Ceará - COGERH'
		cTexto += '<br>'
