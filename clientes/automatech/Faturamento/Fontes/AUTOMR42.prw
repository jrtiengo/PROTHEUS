...
User Function AUTOMR42()
   ...
   DEFINE MSDIALOG oDlg TITLE "Novo Formulário" FROM C(178),C(181) TO C(270),C(596) PIXEL

   @ C(005),C(006) Say "Arquivo de Clientes a ser importado" Size C(092),C(008) COLOR CLR_BLACK PIXEL OF oDlg

   @ C(014),C(006) MsGet oGet1 Var cCaminho Size C(192),C(009) COLOR CLR_BLACK Picture "@!" PIXEL OF oDlg

   @ C(027),C(119) Button "Importar" Size C(037),C(012) PIXEL OF oDlg ACTION ( ImpCadCli(cCaminho) )
...
Static Function ImpCadCli( cCaminho )
   ...
   For nContar = 1 to Len(xBuffer)
       ...
       Else
          ...
          _Separa  := Alltrim(StrTran(cCliente, chr(9), " "))
          _Cliente := Strzero(Int(Val(Alltrim(SubStr(cCliente,01,07)))),6)
          _Loja    := Strzero(Int(val(Alltrim(SubStr(_Separa,08)))),3)
