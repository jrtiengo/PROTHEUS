...
If cTipo == "1"
	...
	If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
		...
		If MsSeek(xFilial("SF3")+cClieFor+cLoja+cNota+cSerie)
			While !SF3->(Eof()) .And. cClieFor+cLoja+cNota+cSerie == SF3->F3_CLIEFOR+SF3->F3_LOJA+SF3->F3_NFISCAL+SF3->F3_SERIE
				...
				//Jesse - Solutio - Buscar o municipio de tributacao do SBZ - Inicio
				dbSelectArea("SBZ")
				dbSetOrder(1)
				MsSeek(xFilial("SBZ")+SD2->D2_COD)
								
				If SBZ->(FieldPos("BZ_TRIBMUN"))>0
					cTribMun:= SBZ->BZ_TRIBMUN
				EndIf
				//Jesse - Solutio - Buscar o municipio de tributacao do SBZ	 - fim			
		...
		ElseIf FunName() == "SPEDNFSE"
			...
			While !Eof() .And. xFilial("SF3") == SF3->F3_FILIAL .And.;
				...
				//If SC5->(FieldPos("C5_MUNPRES")) > 0 .And. !Empty(SC5->C5_MUNPRES) >>>>>>Comentado Michel Aoki
					//cMunPres  := SC5->C5_MUNPRES    >>>>>>Comentado Michel Aoki
					//cMunPres:= ConvType(aUF[aScan(aUF,{|x| x[1] == aDest[14]})][02]+cMunPres) >>>>>>Comentado Michel Aoki
					//cDescMunP := SC5->C5_DESCMUN >>>>>>Comentado Michel Aoki
				//Else >>>>>>Comentado Michel Aoki
					cMunPres:= aDest[13]
					cMunPres:= ConvType(aUF[aScan(aUF,{|x| x[1] == aDest[14]})][02]+cMunPres)
					cDescMunP := aDest[08]
			//	EndIf >>>>>>Comentado Michel Aoki
		...
		Else
			...
			#IFDEF TOP
				...
				BeginSql Alias cAliasSD2
						...
						D2_QUANT,D2_TOTAL,D2_DESCON,D2_VALFRE,D2_SEGURO,D2_PEDIDO,D2_ITEMPV,D2_DESPESA,D2_VALBRUT,D2_VALISS,D2_PRUNIT,D2_NUMSEQ,
					...
					If !Empty((cAliasSD2)->D2_NFORI)
						If (cAliasSD2)->D2_TIPO $ "DBN"
							...
							If ( MsSeek(xFilial("SD1")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI) ) .OR. ;
								( MsSeek(xFilial("SD1")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI) )
								dbSelectArea("SF1")
								dbSetOrder(1)
	...
	//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
				...
				cQry := "SELECT DB_NUMSERI FROM " + RetSQLName("SDB")+ " SDB(NOLOCK) "+CHR(13)
				cQry += "WHERE DB_FILIAL  = '"+ xFilial("SDB")+"' "+CHR(13)
				cQry += "AND DB_DOC  = '"+ (cAliasSD2)->D2_DOC+"' "+CHR(13)
				cQry += "AND DB_SERIE  = '"+ (cAliasSD2)->D2_SERIE+"' "+CHR(13) 
				cQry += "AND DB_NUMSEQ  = '"+ (cAliasSD2)->D2_NUMSEQ+"' "+CHR(13)
				cQry += "AND DB_PRODUTO  = '"+ (cAliasSD2)->D2_COD+"' "+CHR(13) 
				cQry += "AND DB_CLIFOR  = '"+ (cAliasSD2)->D2_CLIENTE+"' "+CHR(13) 
				cQry += "AND DB_LOJA  = '"+ (cAliasSD2)->D2_LOJA+"' "+CHR(13)
				cQry += "AND DB_ESTORNO = '' "+CHR(13)
				cQry += "AND D_E_L_E_T_  = '' "
				/*
				// Jean Rehermann 30-01-2012 - Alterado para corrigir impressão de numero de serie na nota fiscal
				// Esta alteração foi utilizada apenas em uma situação pontual de erro nos endereços.
				cQry := "SELECT DB_NUMSERI, DC_NUMSERI FROM "
				cQry += RetSQLName("SDB")+ " SDB(NOLOCK), "+CHR(13)
				cQry += RetSQLName("SDC")+ " SDC(NOLOCK), "+CHR(13)
				cQry += RetSQLName("SD2")+ " SD2(NOLOCK) "+CHR(13)
				cQry += "WHERE DB_FILIAL  = '"+ xFilial("SDB")+"' "+CHR(13)
				cQry += "AND DC_FILIAL  = '"+ xFilial("SDC")+"' "+CHR(13)
				cQry += "AND D2_FILIAL  = '"+ xFilial("SD2")+"' "+CHR(13)
				cQry += "AND DB_DOC  = '"+ (cAliasSD2)->D2_DOC+"' "+CHR(13)
				cQry += "AND DB_SERIE  = '"+ (cAliasSD2)->D2_SERIE+"' "+CHR(13) 
				cQry += "AND DB_NUMSEQ  = '"+ (cAliasSD2)->D2_NUMSEQ+"' "+CHR(13)
				cQry += "AND DB_PRODUTO  = '"+ (cAliasSD2)->D2_COD+"' "+CHR(13) 
				...
				cQry += "AND DB_LOJA  = '"+ (cAliasSD2)->D2_LOJA+"' "+CHR(13)
				cQry += "AND DB_ESTORNO = '' "+CHR(13)
				cQry += "AND DB_PRODUTO = DC_PRODUTO "+CHR(13)
				cQry += "AND D2_DOC = DB_DOC "+CHR(13)
				cQry += "AND D2_SERIE = DB_SERIE "+CHR(13)
				cQry += "AND DC_PEDIDO = D2_PEDIDO "+CHR(13)
				cQry += "AND DC_ITEM = D2_ITEM "+CHR(13)
				cQry += "AND SDB.D_E_L_E_T_ = '' "+CHR(13)
				cQry += "AND SDC.D_E_L_E_T_ = '' "+CHR(13)
				cQry += "AND SD2.D_E_L_E_T_ = '' "
				*/
				
				cQry := ChangeQuery(cQry)

				dbUseArea( .T., "TOPCONN", TcGenQry( ,,cQry ),"SDBTMP", .F., .T. )
				DbSelectArea("SDBTMP")
				DbGoTop()
				
				_aNumSer := {}
				do while SDBTMP->( !eof() ) 
					aadd(_aNumSer,{allTrim((cAliasSD2)->D2_ITEM),allTrim(SDBTMP->DB_NUMSERI)})
					DbSelectArea("SDBTMP")
					dbskip()
				enddo
				If Select("SDBTMP") <> 0
				...
				nCont := len(_aNumSer)
				for nLoop =1 to nCont
					if !"Num. Serie Item "+ _aNumSer[nLoop][1] $ cMensCli
						cMensCli += "Num. Serie Item "+ _aNumSer[nLoop][1]+": "
					endif
					if !_aNumSer[nLoop][2] $ cMensCli
						cMensCli += _aNumSer[nLoop][2]+iif(nLoop==nCont,"",",")
					endif 
				next
				if nCont > 0
					cMensCli += CHR(13)
				EndIf
					...
					cCodProd  := (cAliasSD2)->D2_COD	            
 			        cDescProd := IIF(Empty(SC6->C6_DESCRI), Alltrim(SB1->B1_DESC) + " " + Alltrim(SB1->B1_DAUX), Alltrim(SC6->C6_DESCRI) + " " + Alltrim(SB1->B1_DAUX))
					...
					If !Empty((cAliasSD2)->D2_IDENTB6) 
		         	...
		         	Else
                       ...
                       // Tarefa Nº 000622 - Imprime o código dos produtos do Cliente quando Filial 04 - Suprimentos
//                     If cFilant == "04"
				          SA7->(dbSetOrder(1)) 	         
					      If SA7->(MsSeek( xFilial("SA7") + (cAliasSD2)->(D2_CLIENTE+D2_LOJA+D2_COD) )) .and. !empty(SA7->A7_CODCLI) .and. !empty(SA7->A7_DESCCLI) 
					         cCodProd  := SA7->A7_CODCLI 
...
//					   Endif
					...
					aadd(aProd,	{Len(aProd)+1,;
						...
						IIF(SBZ->(FieldPos("BZ_TRIBMUN"))<>0,RetFldProd(SBZ->BZ_COD,"BZ_TRIBMUN"),""),;  //aProd[34]       //Jesse - Solutio - Pegar do SBZ
		...
		Else				
			...
			#IFDEF TOP
				...
				BeginSql Alias cAliasSD1			
						...
						D1_CF,D1_QUANT,D1_TOTAL,D1_VALDESC,D1_VALFRE,D1_SEGURO,D1_DESPESA,D1_CODISS,D1_VALISS,D1_VALIPI,D1_ICMSRET,D1_NUMSEQ, 
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				cQry := "SELECT DB_NUMSERI FROM " + RetSQLName("SDB")+ " SDB(NOLOCK) "+CHR(13)
				cQry += "WHERE DB_FILIAL  = '"+ xFilial("SDB")+"' "+CHR(13)
				cQry += "AND DB_DOC  = '"+ (cAliasSD1)->D1_DOC+"' "+CHR(13)
				cQry += "AND DB_SERIE  = '"+ (cAliasSD1)->D1_SERIE+"' "+CHR(13) 
				cQry += "AND DB_NUMSEQ  = '"+ (cAliasSD1)->D1_NUMSEQ+"' "+CHR(13)
				cQry += "AND DB_PRODUTO  = '"+ (cAliasSD1)->D1_COD+"' "+CHR(13) 
				cQry += "AND DB_CLIFOR  = '"+ (cAliasSD1)->D1_FORNECE+"' "+CHR(13) 
				cQry += "AND DB_LOJA  = '"+ (cAliasSD1)->D1_LOJA+"' "+CHR(13)
				cQry += "AND DB_ESTORNO = '' "+CHR(13)
				cQry += "AND D_E_L_E_T_  = '' "
				cQry := ChangeQuery(cQry)
				dbUseArea( .T., "TOPCONN", TcGenQry( ,,cQry ),"SDBTMP", .F., .T. )
				...
				_aNumSer := {}
				do while SDBTMP->( !eof() ) 
					aadd(_aNumSer,{allTrim((cAliasSD1)->D1_ITEM),allTrim(SDBTMP->DB_NUMSERI)})
					DbSelectArea("SDBTMP")
					dbskip()
				enddo
				If Select("SDBTMP") <> 0
		       ...
		       EndIf   		
				aadd(aProd,	{Len(aProd)+1,;  
					(cAliasSD1)->D1_COD,;
     				"",;
 					AllTrim( SB1->B1_DESC ) +" "+ AllTrim( SB1->B1_DAUX ),;					
					SB1->B1_POSIPI,;
					SB1->B1_EX_NCM,;
					(cAliasSD1)->D1_CF,;
					SB1->B1_UM,;
					...
					IIF(SBZ->(FieldPos("BZ_TRIBMUN"))<>0,RetFldProd(SBZ->BZ_COD,"BZ_TRIBMUN"),""),;		// [34]
					0,;		// [35]
					0,;		// [36]
					0,;		// [37]
					})
...
Do Case
	...
	Case (cVeramb >= "3.10" .and. (!Empty(aNfVinc) .Or. !Empty(aRefECF))) .and. (aNota[5] $ "D" .And. SF4->F4_AJUSTE <> "S")
   		...
   		EndIf
	Case (cVeramb >= "3.10" .and. (!Empty(aNfVinc) .Or. !Empty(aRefECF))) .and. (aNota[5] $ "N" .And. SF4->F4_PODER3 == "D")
		cTPNota:= "4" //Devolução de Mercadoria
	OtherWise 
  		cTPNota:= "1"
	EndCase
...
Static Function NfeItem(aProd,aICMS,aICMSST,aIPI,aPIS,aPISST,aCOFINS,aCOFINSST,aISSQN,aCST,aMed,aArma,aveicProd,aDI,aAdi,aExp,aPisAlqZ,aCofAlqZ,aAnfI,cTipo,cVerAmb,aComb,cMensFis,cCsosn,aPedCom,cF2Tipo,aICMSZFM,aDest,cIpiCst,aFCI,lIcmDevol,nVicmsDeson,nVIcmDif,cMunPres)
   ...
   // Verifica se o produto listado possui Pedido de Compra para impressão
...
msgalert("parei")
   ...
   If cTipo == "1"
      ...
      // Pesquisa se existe a informação do Nº do Pedido de Compra do Cliente bem como o sequencial do item do pedido de compra do cliente.
      If Select("T_DCOMPRAS") > 0
         T_DCOMPRAS->( dbCloseArea() )
      EndIf
      ...
      cSql += " WHERE SD2.D2_FILIAL  = '" + Alltrim(SF2->F2_FILIAL) + "'"
      cSql += "   AND SD2.D2_DOC     = '" + Alltrim(SF2->F2_DOC)    + "'"
      cSql += "   AND SD2.D2_SERIE   = '" + Alltrim(SF2->F2_SERIE)  + "'"
      cSql += "   AND SD2.D_E_L_E_T_ = ''
      cSql += "   AND SC6.C6_FILIAL  = SD2.D2_FILIAL
      cSql += "   AND SC6.C6_NUM     = SD2.D2_PEDIDO
      cSql += "   AND SC6.C6_ITEM    = SD2.D2_ITEMPV
      cSql += "   AND SC6.D_E_L_E_T_ = ''

      cSql := ChangeQuery( cSql )
      dbUseArea( .T., "TOPCONN", TcGenQry(,,cSql), "T_DCOMPRAS", .T., .T. )

      If !T_DCOMPRAS->( EOF() )

         If !Empty(Alltrim(T_DCOMPRAS->C6_ORDC))
            // cString += NfeTag('<xPed>', ConvType(T_DCOMPRAS->C6_ORDC,15)) + "</xPed>"

            cString += '<xPed>' + ConvType(T_DCOMPRAS->C6_ORDC,15) + "</xPed>"

            If !Empty(Alltrim(T_DCOMPRAS->C6_ORDS))
               // cString += NfeTag('<nItemPed>', ConvType(Strzero(Int(Val(T_DCOMPRAS->C6_ORDS)),6),06))   && + "</nItemPed>"
               cString += '<nItemPed>' + ConvType(Strzero(Int(Val(T_DCOMPRAS->C6_ORDS)),6),06) + "</nItemPed>"
            Endif
            
         Endif

//       If !Empty(Alltrim(aProd[26]))
//          __Ocompra := Alltrim(aProd[26]) + "/" + Alltrim(aProd[27])      
//          cString += NfeTag('<xPed>' ,ConvType(__Ocompra,15))
//       Endif   
