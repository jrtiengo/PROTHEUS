...
/*
				...
				cQry += "AND D_E_L_E_T_  = '' "
				/*
				// Jean Rehermann 30-01-2012 - Alterado para corrigir impress„o de numero de serie na nota fiscal
				// Esta alteraÁ„o foi utilizada apenas em uma situaÁ„o pontual de erro nos endereÁos.
...
*/
					...
					If !Empty((cAliasSD2)->D2_IDENTB6) 
		         	...
		         	Else
                       ...
                       // Tarefa N∫ 000622 - Imprime o cÛdigo dos produtos do Cliente quando Filial 04 - Suprimentos
                       If cFilant == "04"
				          SA7->(dbSetOrder(1)) 	         
					      If SA7->(MsSeek( xFilial("SA7") + (cAliasSD2)->(D2_CLIENTE+D2_LOJA+D2_COD) )) .and. !empty(SA7->A7_CODCLI) .and. !empty(SA7->A7_DESCCLI) 
					         cCodProd  := SA7->A7_CODCLI 
...
Do Case
	...
	/* Referente ao chamado TIDMJV que contempla, nota de transferÍncia de crÈdito / dÈbito */	   		
   	Case ( ( AllTrim( SF4->F4_CF ) $ cMVCfopTran ) .and. ( SF4->F4_SITTRIB == "90" ) .and.  ( SF4->F4_AJUSTE == "S" ) ) 
		cTPNota:= "3"
	Case (cVeramb >= "3.10" .and. !Empty(aNfVinc)) .and. (aNota[5] $ "D" .And. SF4->F4_AJUSTE <> "S")
...
Static Function NfeDest(aDest,cVerAmb,aTransp,aCST,lBrinde)
	...
	If cVerAmb >= "3.10"
		...
		Else
			If "ISENT" $ Upper(Alltrim(aDest[14]))
				//No caso de Contribuinte Isento de InscriÁ„o (indIEDest=2), n„o informar a tag IE do destinat·rio.
				//o campo IE do Destinat·rio n„o È mais informado com o literal ìISENTOî
				cIndicador := "2"
			Else
...
cString += '<indTot>'+aProd[24]+'</indTot>'
      // Verifica se o produto listado possui Pedido de Compra para impress„o
      If cTipo == "1"
...
If cVerAmb >= "3.10" .and. Len(aExp)>0 .and. ConvType(aExp[03][1]) == "I51"
		...
		If Len(aIcms)>0
			...
			Else
				If cVerAmb >= "3.10" .and. aCST[1] $ '51' .and. !Empty(aICMS[12]) .and. !lArt186
					...
					cString += NfeTag('<pDif>' ,ConvType(aICMS[12]/iIf(lIcmDevol,aICMS[07],0)*100,7,4))
	...
	Else
		...
		Else	//Caso nao possua a function, traz como padrao dados da homologacao realizada em 2011 Laudo POL2002011
			cMd5Master	:= IIF(LJAnalisaLeg(57)[1],"EC28FFF525A8851AC2A3EAE572CCD26F",; 	//Vers„o especial Credenciada no RJ via Laudo POL2002011
													"B300947472D4C482B759DDE8BC07B828") 	//Vers„o padr„o laudo POL2002011
		EndIf
    EndIf
