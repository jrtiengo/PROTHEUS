	...
	If cTipo == "1" .And. Empty(cMotCancela)
		...
		If DbSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)	
			...
			While !(cAliasSD2)->(Eof()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				//Jesse - Solutio - Este trecho possui customização e foi movido em de-para para fonte padrão - inicio
				If !lNFeDesc
				...
				Else 
					...
					EndIf
				EndIf
				//Jesse - Solutio - Este trecho possui customização e foi movido em de-para para fonte padrão - fim
				...
				If !lAglutina .Or. Len(aProd) == 0
					...
					If cFilAnt == "02"
					   xx_TribMun    := IIF(SBZ->(FieldPos("BZ_TRIBMUN"))<>0,SBZ->BZ_TRIBMUN,"")
					   xx_CodTribMun := SBZ->BZ_TRIBMUN
					Else
                       xx_TribMun    := IIF(SBZ->(FieldPos("BZ_TRIBMUN"))<>0,RetFldProd(SBZ->BZ_COD,"BZ_TRIBMUN"),"")
                       xx_CodTribMun := RetFldProd(SBZ->BZ_COD,"BZ_TRIBMUN")
                    Endif
					...
					aadd(aProd,	{Len(aProd)+1,;
    					        ...
    					        xx_CodTribMun,; //32 //Jesse - Solutio - Buscar o municipio de tributacao do SBZ
...
static function ident( aNota, aProd, aTotal, aDest, aISSQN, aAIDF, dDateCom )
	...
	cString += "<formpagto>"+aNota[10]+"</formpagto>"
...
static function prestacao( cMunPrest, cDescMunP, aDest, cMunPSIAFI )
	...
	if empty( cMunPSIAFI )
		...
		ElseIf  aDest[9] =='EX' .and. CC2->(DbSeek(xFilial("CC2")+ allTrim(SC5->C5_ESTPRES)+ cIbge)) 
		  	cMunPSIAFI	:=	alltrim(CC2->CC2_CDSIAF)
		...
		Endif
		//Jesse - Solutio - Customizacao não documentada = movida em de-para - inicio
		...
		Else
			cMunPSIAFI	:= "7535"
		EndIf			
		//Jesse - Solutio - Customizacao não documentada = movida em de-para - fim
	endif
...
static function servicos( aProd, aISSQN, aRetido, cNatOper, lNFeDesc, cDiscrNFSe,aCST, cTpPessoa, cCodMun, cF4Agreg, nDescon   )	
	...
	for nX := 1 to len( aProd )
		...
		If lMvEnteTrb
			...
			If lCrgTrib .And. cTpCliente == "F" .And. ( ( aProd[Nx][38] + aProd[Nx][39] + aProd[Nx][40] ) > 0 )
...
//Jesse - Solutio - Customizacao não documentada - movida para fonte padrão - inicio
...
TMP->(DbCloseArea())
Return(	_cRetorno)		
//Jesse - Solutio - Customizacao não documentada - movida para fonte padrão - fim

//Jesse - Solutio - Esta funcao é customizada, porém NÃO encontrei chamada dela no fonte. De qualquer forma, trouxe ela também
//Michel Aoki 
//Observação da nota
