	...
	If cTipo == "1" .And. Empty(cMotCancela)
		...
		If DbSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)	
			...
			While !(cAliasSD2)->(Eof()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				If lAglutina
					If Len(aProd) > 0			
						nX := aScan(aProd,{|x| x[24] == (cAliasSD2)->D2_CODISS .And. x[23] == IIF(SBZ->(FieldPos("BZ_TRIBMUN"))<>0,SBZ->BZ_TRIBMUN,"")}) //Jesse - Solutio - Buscar o municipio de tributacao do SBZ
						If nX > 0						
				...
				If !lAglutina .Or. Len(aProd) == 0
					...
					aadd(aProd,	{Len(aProd)+1,;
								...
								SF3->F3_ISSSUB,;
								SF3->F3_ISSMAT,;   
								IIF(SBZ->(FieldPos("BZ_TRIBMUN"))<>0,SBZ->BZ_TRIBMUN,""),; //Jesse - Solutio - Buscar o municipio de tributacao do SBZ
								SF3->F3_CODISS,;
								...
								SBZ->BZ_TRIBMUN,; //32 //Jesse - Solutio - Buscar o municipio de tributacao do SBZ
...
static function prestacao( cMunPrest, cDescMunP, aDest, cMunPSIAFI )
	...
	if Empty( cMunPSIAFI )           
		...
		If cEmpAnt == "01" .or.  cEmpAnt == "03"
	  		If cFilAnt == "01" .or. cFilAnt == "04" .Or. cFilAnt == "07"
