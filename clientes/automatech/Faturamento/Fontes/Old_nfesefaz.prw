...
If cTipo == "1"
	...
	If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
		...
		ElseIf FunName() == "SPEDNFSE"
            ...
            // ###########################################################################
            // Tarefa #3858 - Envio de XML para o SimFrete                              ##
            // Quando o frete da nota fiscal for = C (CIF), envia o XML para o SimFrete ##
            // ###########################################################################
            If SF2->F2_TPFRETE == "C"
   			   aadd(aDest,IIF(Empty(Alltrim(SA1->A1_EMAIL)), ";automatech@simfrete.com",  Alltrim(SA1->A1_EMAIL) + ";automatech@simfrete.com"))
   			Else
		...
		Else
			...
			If !SF2->F2_TIPO $ "DB" 
				...
				If !Upper(SA1->A1_EST) == "EX"                                                      				
					...
					Else
						aadd(aDest,VldIE(SA1->A1_INSCR,.T.))
			...
			Else
				...
				If !Upper(SA2->A2_EST) == "EX"				
					aadd(aDest,VldIE(SA2->A2_INSCR,.T.))
				Else
					aadd(aDest,"")							
				EndIf					
...
Do Case
    ...
    documento fiscal referenciado conforme  NT 2013/005 v 1.20.
   	*/
   	Case (aNota[5]) $ "D" .and. Empty(aNfVinc).and. Alltrim(SFT->FT_CFOP) $ "1201-1202-1410-1411-5921-6921-6411-2411"
   	      cTPNota:= "4" 
