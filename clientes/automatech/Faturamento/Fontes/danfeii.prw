...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
If nFaturas > 0
    // Verifica se Condição de Pagamento da Nota Fiscal é de Cartão.
    // Se for Cartão, imprime somente a descrição da Condição de Pagamento na Danfe.
	...
	For nX := 1 To 3
		...
		For nY := 1 To Min(9, nFaturas)
			Do Case
				Case nX == 1
					 If nFaturas > 1
                        If Substr(T_CONDICAO->E4_DESCRI,01,06) == "CARTAO"
                           If !lPrimeiro
   					          AAdd(aAux, AllTrim(T_CONDICAO->E4_DESCRI))
					          lPrimeiro := .T.
					       Else
   					          AAdd(aAux, "")
					       Endif   
...
// ---------------------------------------------------------------------------------------- //
// A ROTINA ABAIXO FOI COMENTADA POIS A MESMA FOI SUBSTITUIDA PELA ROTINA LOGO ABAIXO DESTA //
// ESTA ROTINA FOI SUBSTIBUIDA PARA QUE A DANFE SEJA IMPRESSA COM DOIS CANHOTOS.            //
// NÃO ESQUECER DE QUANDO ATUALIZAR ESTE PROGRAMA NAS FUTURAS IMPLEMENTAÇÕES, COPIAR  A RO- //
// TINA ABAIXO DESTA.                                                                       //
// ---------------------------------------------------------------------------------------- //
...
//	oDanfe:Say(nLinCalc,098, "Fone: "+IIf(Type("oEmitente:_EnderEmit:_Fone")=="U","",oEmitente:_EnderEmit:_Fone:Text),oFont08N:oFont)
        ...
        Do Case
           Case Alltrim(Upper(oEmitente:_EnderEmit:_xMun:Text)) == "PORTO ALEGRE"
	        oDanfe:Say(nLinCalc,098, "Fone: 51 3017-8300",oFont08N:oFont)
           Case Alltrim(Upper(oEmitente:_EnderEmit:_xMun:Text)) == "CAXIAS DO SUL"
   	        oDanfe:Say(nLinCalc,098, "Fone: 54 3227-2333",oFont08N:oFont)
           Case Alltrim(Upper(oEmitente:_EnderEmit:_xMun:Text)) == "PELOTAS"
   	        oDanfe:Say(nLinCalc,098, "Fone: 53 30262802",oFont08N:oFont)
        EndCase
Else
...
//		oDanfe:SayBitmap(042,000,cLogoD,095,096)
		oDanfe:SayBitmap(055,000,cLogoD,095,065)
	Else
//		oDanfe:SayBitmap(042,000,cLogo,095,096)
		oDanfe:SayBitmap(055,000,cLogo,095,065)
	EndIF
Endif
...
// Tarega nº 001253.00 - Impressão da base de cálculo ICMS ST conforme solicitação da Controladoria. Somente para NF do tipo I - Complemento de ICMS
If SD2->D2_TIPO == "I"
   oDanfe:Say(326,202,Str(SD2->D2_BRICMS,10,02),oFont08:oFont)   
Else
...
*/
   ...
   // --------------------------------------------------------- //
   // ROTINA QUE REALIZA A IMPRESSÃO DA DANFE COM DOIS CANHOTOS //
   // --------------------------------------------------------- // 

   // Definicao do Box - Recibo de entrega
   oDanfe:Box(000,000,010,501)
   oDanfe:Say(006, 002, "RECEBEMOS DE "+NoChar(oEmitente:_xNome:Text,lConverte)+" OS PRODUTOS CONSTANTES DA NOTA FISCAL INDICADA AO LADO", oFont07:oFont)
   oDanfe:Box(009,000,034,101)
   oDanfe:Say(017, 002, "DATA DE RECEBIMENTO", oFont07N:oFont)
   oDanfe:Box(009,100,034,500)
   oDanfe:Say(017, 102, "IDENTIFICAÇÃO E ASSINATURA DO RECEBEDOR", oFont07N:oFont)
   oDanfe:Box(000,500,034,603)
   oDanfe:Say(007, 542, "NF-e", oFont08N:oFont)
   oDanfe:Say(017, 510, "N. "+StrZero(Val(oIdent:_NNf:Text),9), oFont08:oFont)
   oDanfe:Say(027, 510, "SÉRIE "+oIdent:_Serie:Text, oFont08:oFont)

   // 2º Canhoto
   oDanfe:Box(040,000,050,501)
   oDanfe:Say(046, 002, "RECEBEMOS DE "+NoChar(oEmitente:_xNome:Text,lConverte)+" OS PRODUTOS CONSTANTES DA NOTA FISCAL INDICADA AO LADO", oFont07:oFont)
   oDanfe:Box(050,000,074,101)
   oDanfe:Say(058, 002, "DATA DE RECEBIMENTO", oFont07N:oFont)
   oDanfe:Box(050,100,074,500)
   oDanfe:Say(058, 102, "IDENTIFICAÇÃO E ASSINATURA DO RECEBEDOR", oFont07N:oFont)
   oDanfe:Box(040,500,074,603)
   oDanfe:Say(047, 542, "NF-e", oFont08N:oFont)
   oDanfe:Say(057, 510, "N. "+StrZero(Val(oIdent:_NNf:Text),9), oFont08:oFont)
   oDanfe:Say(067, 510, "SÉRIE "+oIdent:_Serie:Text, oFont08:oFont)

   // Quadro 1 IDENTIFICACAO DO EMITENTE
   oDanfe:Box(080,000,156,250)
   oDanfe:Say(089,098, "Identificação do emitente",oFont12N:oFont)
   nLinCalc	:=	098
   ...
   // Quadro 2
   oDanfe:Box(080,248,156,351)
   oDanfe:Say(093,275, "DANFE",oFont18N:oFont)
   oDanfe:Say(103,258, "DOCUMENTO AUXILIAR DA",oFont07:oFont)
   oDanfe:Say(113,258, "NOTA FISCAL ELETRÔNICA",oFont07:oFont)
   oDanfe:Say(123,266, "0-ENTRADA",oFont08:oFont)
   oDanfe:Say(133,266, "1-SAÍDA"  ,oFont08:oFont)
   oDanfe:Box(118,315,133,325)
   oDanfe:Say(128,318, oIdent:_TpNf:Text,oFont08N:oFont)
   oDanfe:Say(143,255,"N. "+StrZero(Val(oIdent:_NNf:Text),9),oFont10N:oFont)
   oDanfe:Say(153,255,"SÉRIE "+oIdent:_Serie:Text,oFont10N:oFont)
   oDanfe:Say(153,314,"FL. "+StrZero(nFolha,2)+"/"+StrZero(nFolhas,2),oFont10N:oFont)
   ...
   If nfolha==1
	  If lMv_Logod
		 oDanfe:SayBitmap(085,000,cLogoD,095,065)
   	  Else
  
         If cEmpant == "03"
            cLogo := "system\lgrl01.bmp"
         Endif
         
		 oDanfe:SayBitmap(085,000,cLogo,095,065)

	  EndIF
   Endif

   // Codigo de barra
   oDanfe:Box(080,350,113,603)
   oDanfe:Box(113,350,135,603)
   oDanfe:Say(132,355,TransForm(SubStr(oNF:_InfNfe:_ID:Text,4),"@r 9999 9999 9999 9999 9999 9999 9999 9999 9999 9999 9999"),oFont12N:oFont)
   oDanfe:Box(135,350,156,603)

   If nFolha == 1
      oDanfe:Say(123,355,"CHAVE DE ACESSO DA NF-E",oFont12N:oFont)
	  nFontSize := 28
	  oDanfe:Code128C(110,370,SubStr(oNF:_InfNfe:_ID:Text,4), nFontSize ) 
   EndIf
   ...
   If Empty(cCodAutDPEC)
	  If Empty(cChaveCont)
		 oDanfe:Say(143,355,"Consulta de autenticidade no portal nacional da NF-e",oFont12:oFont)
		 oDanfe:Say(153,355,"www.nfe.fazenda.gov.br/portal ou no site da SEFAZ Autorizada",oFont12:oFont)
	  Endif
   Endif
   ...
   // Quadro 4
   oDanfe:Box(161,000,184,603)
   oDanfe:Box(161,000,184,350)
   oDanfe:Say(170,002,"NATUREZA DA OPERAÇÃO",oFont08N:oFont)
   oDanfe:Say(180,002,oIdent:_NATOP:TEXT,oFont08:oFont)

   If !Empty(cCodAutDPEC)
	  oDanfe:Say(170,352,"NÚMERO DE REGISTRO DPEC",oFont08N:oFont)
   Endif

   If Empty(cCodAutDPEC) .And. (((Val(oNF:_INFNFE:_IDE:_SERIE:TEXT) >= 900).And.(oNFe:_NFE:_INFNFE:_IDE:_TPEMIS:TEXT)$"23") .Or. (oNFe:_NFE:_INFNFE:_IDE:_TPEMIS:TEXT)$"1")
	  oDanfe:Say(170,352,"PROTOCOLO DE AUTORIZAÇÃO DE USO",oFont08N:oFont)
   Endif

   If((oNFe:_NFE:_INFNFE:_IDE:_TPEMIS:TEXT)$"25")
	 oDanfe:Say(170,352,"DADOS DA NF-E",oFont08N:oFont)
   Endif

   oDanfe:Say(180,354,IIF(!Empty(cCodAutDPEC),cCodAutDPEC+" "+AllTrim(IIF(!Empty(dDtReceb),ConvDate(DTOS(dDtReceb)),Iif(oNF:_INFNFE:_VERSAO:TEXT >= "3.10",ConvDate(oNF:_InfNfe:_IDE:_DHEMI:Text),ConvDate(oNF:_InfNfe:_IDE:_DEMI:Text))))+" "+AllTrim(cDtHrRecCab),IIF(!Empty(cCodAutSef) .And. ((Val(oNF:_INFNFE:_IDE:_SERIE:TEXT) >= 900).And.(oNFe:_NFE:_INFNFE:_IDE:_TPEMIS:TEXT)$"23") .Or. (oNFe:_NFE:_INFNFE:_IDE:_TPEMIS:TEXT)$"1",cCodAutSef+" "+AllTrim(IIF(!Empty(dDtReceb),ConvDate(DTOS(dDtReceb)),Iif(oNF:_INFNFE:_VERSAO:TEXT >= "3.10",ConvDate(oNF:_InfNfe:_IDE:_DHEMI:Text),ConvDate(oNF:_InfNfe:_IDE:_DEMI:Text))))+" "+AllTrim(cDtHrRecCab),TransForm(cChaveCont,"@r 9999 9999 9999 9999 9999 9999 9999 9999 9999"))),oFont08:oFont)

   nFolha++

   // Quadro 5
   oDanfe:Box(184,000,201,603)
   oDanfe:Box(184,000,201,200)
   oDanfe:Box(184,200,201,400)
   oDanfe:Box(184,400,201,603)
   oDanfe:Say(192,002,"INSCRIÇÃO ESTADUAL",oFont08N:oFont)
   oDanfe:Say(200,002,IIf(Type("oEmitente:_IE:TEXT")<>"U",oEmitente:_IE:TEXT,""),oFont08:oFont)
   oDanfe:Say(192,205,"INSC.ESTADUAL DO SUBST.TRIB.",oFont08N:oFont)
   oDanfe:Say(200,205,IIf(Type("oEmitente:_IEST:TEXT")<>"U",oEmitente:_IEST:TEXT,""),oFont08:oFont)
   oDanfe:Say(192,405,"CNPJ",oFont08N:oFont)
   oDanfe:Say(200,405,TransForm(oEmitente:_CNPJ:TEXT,IIf(Len(oEmitente:_CNPJ:TEXT)<>14,"@r 999.999.999-99","@r 99.999.999/9999-99")),oFont08:oFont)
   ...
   oDanfe:Say(209,002,"DESTINATARIO/REMETENTE",oFont08N:oFont)
   oDanfe:Box(211,000,231,450)
   oDanfe:Say(219,002, "NOME/RAZÃO SOCIAL",oFont08N:oFont)
   oDanfe:Say(229,002,NoChar(oDestino:_XNome:TEXT,lConverte),oFont08:oFont)
   oDanfe:Box(211,280,231,500)
   oDanfe:Say(219,283,"CNPJ/CPF",oFont08N:oFont)
   oDanfe:Say(229,283,cAux,oFont08:oFont)

   oDanfe:Box(231,000,251,500)
   oDanfe:Box(231,000,251,260)
   oDanfe:Say(238,002,"ENDEREÇO",oFont08N:oFont)
   oDanfe:Say(248,002,aDest[01],oFont08:oFont)
   oDanfe:Box(231,230,251,380)
   oDanfe:Say(238,232,"BAIRRO/DISTRITO",oFont08N:oFont)
   oDanfe:Say(248,232,aDest[02],oFont08:oFont)
   oDanfe:Box(231,380,251,500)
   oDanfe:Say(238,382,"CEP",oFont08N:oFont)
   oDanfe:Say(248,382,aDest[03],oFont08:oFont)

   oDanfe:Box(250,000,271,500)
   oDanfe:Box(250,000,271,180)
   oDanfe:Say(259,002,"MUNICIPIO",oFont08N:oFont)
   oDanfe:Say(269,002,aDest[05],oFont08:oFont)
   oDanfe:Box(250,150,271,256)
   oDanfe:Say(259,152,"FONE/FAX",oFont08N:oFont)
   oDanfe:Say(269,152,aDest[06],oFont08:oFont)
   oDanfe:Box(250,255,271,341)
   oDanfe:Say(259,257,"UF",oFont08N:oFont)
   oDanfe:Say(269,257,aDest[07],oFont08:oFont)
   oDanfe:Box(250,340,271,500)
   oDanfe:Say(259,342,"INSCRIÇÃO ESTADUAL",oFont08N:oFont)
   oDanfe:Say(269,342,aDest[08],oFont08:oFont)

   oDanfe:Box(211,502,231,603)
   oDanfe:Say(219,504,"DATA DE EMISSÃO",oFont08N:oFont)
   oDanfe:Say(229,504,Iif(oNF:_INFNFE:_VERSAO:TEXT >= "3.10",ConvDate(oIdent:_DHEmi:TEXT),ConvDate(oIdent:_DEmi:TEXT)),oFont08:oFont)
   oDanfe:Box(231,502,251,603)
   oDanfe:Say(238,504,"DATA ENTRADA/SAÍDA",oFont08N:oFont)
   oDanfe:Say(247,504,Iif( Empty(aDest[4]),"",ConvDate(aDest[4]) ),oFont08:oFont)
   oDanfe:Box(250,502,271,603)
   oDanfe:Say(257,503,"HORA ENTRADA/SAÍDA",oFont08N:oFont)
   oDanfe:Say(266,503,aHrEnt[01],oFont08:oFont)
   ...
   // Desenha os quadro da Fatura
   oDanfe:Say(277,002,"FATURA",oFont08N:oFont)
   oDanfe:Box(279,000,304,068)
   oDanfe:Box(279,067,304,134)
   oDanfe:Box(279,134,304,202)
   oDanfe:Box(279,201,304,268)
   oDanfe:Box(279,268,304,335)
   oDanfe:Box(279,335,304,403)
   oDanfe:Box(279,402,304,469)
   oDanfe:Box(279,469,304,537)
   oDanfe:Box(279,536,304,603)

   // Imprime valores das Faturas
   nColuna := 002
   If Len(aFaturas) >0
      For nY := 1 To 9
	      oDanfe:Say(286,nColuna,aAux[1][nY][1],oFont08:oFont)
		  oDanfe:Say(294,nColuna,aAux[1][nY][2],oFont08:oFont)
		  oDanfe:Say(302,nColuna,aAux[1][nY][3],oFont08:oFont)
		  nColuna:= nColuna + 67
	  Next nY
   Endif

   // Calculo do imposto
   oDanfe:Say(313,002,"CALCULO DO IMPOSTO",oFont08N:oFont)

   oDanfe:Box(315,000,338,121)
   oDanfe:Say(324,002,"BASE DE CALCULO DO ICMS",oFont08N:oFont)
   If cMVCODREG $ "3" 
   	oDanfe:Say(334,002,aTotais[01],oFont08:oFont)
   ElseIf lImpSimpN
   	oDanfe:Say(334,002,aSimpNac[01],oFont08:oFont)	
   Endif

   oDanfe:Box(315,120,338,200)
   oDanfe:Say(324,125,"VALOR DO ICMS",oFont08N:oFont)
   If cMVCODREG $ "3" 
   	oDanfe:Say(334,125,aTotais[02],oFont08:oFont)
   ElseIf lImpSimpN
   	oDanfe:Say(334,125,aSimpNac[02],oFont08:oFont)
   Endif

   oDanfe:Box(315,199,338,360)
   oDanfe:Say(324,200,"BASE DE CALCULO DO ICMS SUBSTITUIÇÃO",oFont08N:oFont)

   // Tarega nº 001253.00 - Impressão da base de cálculo ICMS ST conforme solicitação da Controladoria. Somente para NF do tipo I - Complemento de ICMS
   If SD2->D2_TIPO == "I"
      oDanfe:Say(334,202,Str(SD2->D2_BRICMS,10,02),oFont08:oFont)   
   Else
      oDanfe:Say(334,202,aTotais[03],oFont08:oFont)
   Endif
                  
   oDanfe:Box(315,360,338,490)
   oDanfe:Say(324,363,"VALOR DO ICMS SUBSTITUIÇÃO",oFont08N:oFont)
   oDanfe:Say(334,363,aTotais[04],oFont08:oFont)

   oDanfe:Box(315,490,338,603)
   oDanfe:Say(324,491,"VALOR TOTAL DOS PRODUTOS",oFont08N:oFont)
   oDanfe:Say(334,491,aTotais[05],oFont08:oFont)

   oDanfe:Box(338,000,359,110)
   oDanfe:Say(347,002,"VALOR DO FRETE",oFont08N:oFont)
   oDanfe:Say(355,002,aTotais[06],oFont08:oFont)
   oDanfe:Box(338,100,359,190)
   oDanfe:Say(347,102,"VALOR DO SEGURO",oFont08N:oFont)
   oDanfe:Say(355,102,aTotais[07],oFont08:oFont)
   oDanfe:Box(338,190,359,290)
   oDanfe:Say(347,194,"DESCONTO",oFont08N:oFont)
   oDanfe:Say(355,194,aTotais[08],oFont08:oFont)
   oDanfe:Box(338,290,359,415)
   oDanfe:Say(347,295,"OUTRAS DESPESAS ACESSÓRIAS",oFont08N:oFont)
   oDanfe:Say(355,295,aTotais[09],oFont08:oFont)
   oDanfe:Box(338,414,359,500)
   oDanfe:Say(347,420,"VALOR DO IPI",oFont08N:oFont)
   oDanfe:Say(355,420,aTotais[10],oFont08:oFont)
   oDanfe:Box(338,500,359,603)
   oDanfe:Say(347,506,"VALOR TOTAL DA NOTA",oFont08N:oFont)
   oDanfe:Say(355,506,aTotais[11],oFont08:oFont)

   // Transportador/Volumes transportados
   oDanfe:Say(367,002,"TRANSPORTADOR/VOLUMES TRANSPORTADOS",oFont08N:oFont)
   oDanfe:Box(369,000,392,603)
   oDanfe:Say(378,002,"RAZÃO SOCIAL",oFont08N:oFont)
   oDanfe:Say(388,002,aTransp[01],oFont08:oFont)
   oDanfe:Box(369,245,392,315)
   oDanfe:Say(378,247,"FRETE POR CONTA",oFont08N:oFont)
   If cModFrete =="0"
   	  oDanfe:Say(388,247,"0-EMITENTE",oFont08:oFont)
   ElseIf cModFrete =="1"
   	  oDanfe:Say(388,247,"1-DEST/REM",oFont08:oFont)
   ElseIf cModFrete =="2"
   	  oDanfe:Say(388,247,"2-TERCEIROS",oFont08:oFont)
   ElseIf cModFrete =="9"
   	  oDanfe:Say(388,247,"9-SEM FRETE",oFont08:oFont)
   Else
   	  oDanfe:Say(388,247,"",oFont08:oFont)
   Endif

   oDanfe:Box(369,315,392,370)
   oDanfe:Say(378,317,"CÓDIGO ANTT",oFont08N:oFont)
   oDanfe:Say(388,319,aTransp[03],oFont08:oFont)
   oDanfe:Box(369,370,392,490)
   oDanfe:Say(378,375,"PLACA DO VEÍCULO",oFont08N:oFont)
   oDanfe:Say(388,375,aTransp[04],oFont08:oFont)
   oDanfe:Box(369,450,392,510)
   oDanfe:Say(378,452,"UF",oFont08N:oFont)
   oDanfe:Say(388,452,aTransp[05],oFont08:oFont)
   oDanfe:Box(369,510,392,603)
   oDanfe:Say(378,512,"CNPJ/CPF",oFont08N:oFont)
   oDanfe:Say(388,512,aTransp[06],oFont08:oFont)

   oDanfe:Box(391,000,415,603)
   oDanfe:Box(391,000,415,241)
   oDanfe:Say(399,002,"ENDEREÇO",oFont08N:oFont)
   oDanfe:Say(409,002,aTransp[07],oFont08:oFont)
   oDanfe:Box(391,240,415,341)
   oDanfe:Say(399,242,"MUNICIPIO",oFont08N:oFont)
   oDanfe:Say(409,242,aTransp[08],oFont08:oFont)
   oDanfe:Box(391,340,415,440)
   oDanfe:Say(399,342,"UF",oFont08N:oFont)
   oDanfe:Say(409,342,aTransp[09],oFont08:oFont)
   oDanfe:Box(391,440,415,603)
   oDanfe:Say(399,442,"INSCRIÇÃO ESTADUAL",oFont08N:oFont)
   oDanfe:Say(409,442,aTransp[10],oFont08:oFont)

   oDanfe:Box(411,000,432,603)
   oDanfe:Box(411,000,432,101)
   oDanfe:Say(420,002,"QUANTIDADE",oFont08N:oFont)
   oDanfe:Say(430,002,aTransp[11],oFont08:oFont)
   oDanfe:Box(411,100,432,200)
   oDanfe:Say(420,102,"ESPECIE",oFont08N:oFont)
   oDanfe:Say(430,102,Iif(!Empty(aTransp[12]),aTransp[12],Iif(Len(aEspVol)>0,aEspVol[1][1],"")),oFont08:oFont)
   oDanfe:Box(411,200,432,301)
   oDanfe:Say(420,202,"MARCA",oFont08N:oFont)
   oDanfe:Say(430,202,aTransp[13],oFont08:oFont)
   oDanfe:Box(411,300,432,400)
   oDanfe:Say(420,302,"NUMERAÇÃO",oFont08N:oFont)
   oDanfe:Say(430,302,aTransp[14],oFont08:oFont)
   oDanfe:Box(411,400,432,501)
   oDanfe:Say(420,402,"PESO BRUTO",oFont08N:oFont)
   oDanfe:Say(430,402,Iif(!Empty(aTransp[15]),aTransp[15],Iif(Len(aEspVol)>0 .And. Val(aEspVol[1][3])>0,Transform(Val(aEspVol[1][3]),"@E 999999.9999"),"")),oFont08:oFont)
   oDanfe:Box(411,500,432,603)
   oDanfe:Say(420,502,"PESO LIQUIDO",oFont08N:oFont)
   oDanfe:Say(430,502,Iif(!Empty(aTransp[16]),aTransp[16],Iif(Len(aEspVol)>0 .And. Val(aEspVol[1][2])>0,Transform(Val(aEspVol[1][2]),"@E 999999.9999"),"")),oFont08:oFont)
...
//				oDanfe:SayBitmap(000+nLinhavers,000,cLogoD,095,096)
				oDanfe:SayBitmap(000+nLinhavers,000,cLogoD,095,028)
			Else
//				oDanfe:SayBitmap(000+nLinhavers,000,cLogo,095,096)
				oDanfe:SayBitmap(000+nLinhavers,000,cLogo,095,028)
			EndIf
...
//			oDanfe:SayBitmap(000,000,cLogoD,095,096)
			oDanfe:SayBitmap(000,000,cLogoD,095,028)
		Else
//			oDanfe:SayBitmap(000,000,cLogo,095,096)
			oDanfe:SayBitmap(000,000,cLogo,095,028)
		EndIf
