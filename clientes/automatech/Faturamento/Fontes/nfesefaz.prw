...
//Declaração de Strings 
Private cMVCFOP     := SuperGetMV("MV_CFOPFIN",.F.,"1201-1202-1410-1411-5921-6921")
Private cFntCtrb	:= ""
	...
	For nCountNF := 1 To Len(aCupRefLoj)
				...
				#IFDEF TOP
					...
					//Verifica se existe Template DCL
	      			IF cVerAmb >= "4.00" .And. (ExistTemplate("PROCMSG")) //Tratativa para Grupo de Repasse de Combustiveis
						BeginSql Alias cAliasSD2
								...
								D2_ORIGLAN,D2_VALCF3,D2_VALIPI,D2_VALACRS,D2_PICM,D2_PDV,D2_BRICMSO,D2_ICMRETO,D2_BRICMSD,D2_ICMRETD,D2_NUMSEQ %Exp:cField% 
								FROM %Table:SD2% SD2
								WHERE
					...
					Else
						BeginSql Alias cAliasSD2
							...
							D2_QUANT,D2_TOTAL,D2_DESCON,D2_VALFRE,D2_SEGURO,D2_PEDIDO,D2_ITEMPV,D2_DESPESA,D2_VALBRUT,D2_VALISS,D2_PRUNIT,
							D2_CLASFIS,D2_PRCVEN,D2_IDENTB6,D2_CODISS,D2_DESCZFR,D2_PREEMB,D2_DESCZFC,D2_DESCZFP,D2_LOTECTL,D2_NUMLOTE,D2_ICMSRET,D2_VALPS3,
							D2_ORIGLAN,D2_VALCF3,D2_VALIPI,D2_VALACRS,D2_PICM,D2_PDV,D2_NUMSEQ %Exp:cField% 
							FROM %Table:SD2% SD2
							WHERE
							SD2.D2_FILIAL  = %xFilial:SD2% AND
							SD2.D2_SERIE   = %Exp:SF2->F2_SERIE% AND
		...
		//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
				...
				for nLoop =1 to nCont
					if !"Num. Serie Item "+ Alltrim(_aNumSer[nLoop][1]) $ cMensCli
						cMensCli += "Num. Serie Item "+ AllTrim(_aNumSer[nLoop][1]) + ": "
					endif
					if !AllTrim(_aNumSer[nLoop][2]) $ cMensCli
						cMensCli += AllTrim(_aNumSer[nLoop][2]+iif(nLoop==nCont,"",","))
					endif 
				next
			    if nCont > 0
					cMensCli += "##"// CHR(13)
				EndIf
				    ...
				    If (cAliasSD2)->D2_TIPO == "N"  
						...
						If SA7->(Dbseek( xFilial("SA7") + (cAliasSD2)->(D2_CLIENTE) + (cAliasSD2)->(D2_LOJA) + (cAliasSD2)->(D2_COD) ) )
							cCodProd  := FwCutOff(Alltrim(SA7->A7_CODCLI)) 
						 	cDescProd := FwCutOff(Alltrim(SA7->A7_DESCCLI))          						
						EndIf
					ElseIf (cAliasSD2)->D2_TIPO == "B"
   						SA5->(DbselectArea("SA5")) 	                        
   						SA5->(dbSetOrder(1)) 
						If SA5->(DbSeek( xFilial("SA5") + (cAliasSD2)->(D2_CLIENTE) + (cAliasSD2)->(D2_LOJA) + (cAliasSD2)->(D2_COD) + "      " + "   " ))
							cCodProd  := FwCutOff(Alltrim(SA5->A5_CODPRF)) 
							cDescProd := FwCutOff(Alltrim(SA5->A5_DESREF))	            
						EndIf
					EndIf
					...
					if empty(alltrim(cDescProd))
					   	cDescProd := IIF(Empty(SC6->C6_DESCRI), FwCutOff(Alltrim(Alltrim(SB1->B1_DESC) + " " + Alltrim(SB1->B1_DAUX)), .T.), FwCutOff(Alltrim(Alltrim(SC6->C6_DESCRI) + " " + Alltrim(SB1->B1_DAUX)), .T.))
         			EndIf
		...
		Else				
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
		       ...
		       EndIf   		
				...
				//cDescProd := FwCutOff(Alltrim(Alltrim(SB1->B1_DESC) + " " + Alltrim(SB1->B1_DAUX)), .T.)
				...
				If (cAliasSD1)->D1_TIPO == "N" 
					SA5->(DbselectArea("SA5"))
					SA5->(dbSetOrder(1)) 	         
					If SA5->(DbSeek( xFilial("SA5") + (cAliasSD1)->(D1_FORNECE) + (cAliasSD1)->(D1_LOJA) + (cAliasSD1)->(D1_COD) + "      " + "   " )) 
						cCodProd  := FwCutOff(Alltrim(SA5->A5_CODPRF)) 
						cDescProd := FwCutOff(Alltrim(SA5->A5_DESREF))	            
					EndIf
				ElseIf (cAliasSD1)->D1_TIPO == "B"
					SA7->(DbSelectArea("SA7"))
					SA7->(dbSetOrder(1)) 	         
					If SA7->(Dbseek( xFilial("SA7") + (cAliasSD1)->(D1_FORNECE) + (cAliasSD1)->(D1_LOJA) + (cAliasSD1)->(D1_COD) ))
						cCodProd  := FwCutOff(Alltrim(SA7->A7_CODCLI)) 
					 	cDescProd := FwCutOff(Alltrim(SA7->A7_DESCCLI)) 	            						
					EndIf
				EndIf
...
EndIf                                               
// NfeTag nao estava funcionando, entao preenchemos manualmente
cString += '<CEST>' + ConvType(aProd[41]) + '</CEST>'
...
Do Case
	...
	Case (cVeramb >= "3.10" .and. (!Empty(aNfVinc) .Or. !Empty(aRefECF) .Or. !Empty(aNfVincRur))) .and. ( (aNota[5] $ "D|B" .And. SF4->F4_AJUSTE <> "S") .or. (aNota[5] $ "N" .And. SF4->F4_PODER3 == "D") )
     ...
     documento fiscal referenciado conforme  NT 2013/005 v 1.20.
   	 *(aNota[5]) $ "D" .and. Empty(aNfVinc).and. Alltrim(SFT->FT_CFOP) $ "1201-1202-1410-1411-5921-6921"  */
    Case (aNota[5]) $ "D" .and. Empty(aNfVinc).and. Alltrim(SFT->FT_CFOP) $ cMVCFOP
   	 	cTPNota:= "4" 
