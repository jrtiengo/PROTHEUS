...
User Function MT410ALT()
   ...
   // ###################################################################################
   // Limpa o conteúdo do campo C5_LIBEROK liberado o pedido para proceguir o processo ##
   // ###################################################################################
   dbSelectArea("SC5")
   dbSetOrder(1)
 
   If dbSeek( IIF(Empty(Alltrim(M->C5_FILIAL)), cFilAnt, M->C5_FILIAL) + M->C5_NUM)
...
/*
      // ####################################################################
      // Verifica se existe algum produto com indicação de frete gratuíto. ##
      ...
      WHILE !T_GRATUITO->( EOF() )
	     ...
	     DbSetOrder(1)
    	    ...
    	    SC6->C6_STATUS := "02" 
  	        SC6->C6_BLQ    := "S"
            SC6->C6_ZTBL   := SC6->C6_ZTBL + "SIMF-"
	        U_GrvLogSts(T_GRATUITO->C6_FILIAL,T_GRATUITO->C6_NUM, T_GRATUITO->C6_ITEM, "02", "MT410ALT")
		    Msunlock()
	     Endif
   ...
   If !T_CONDICAO->( EOF() )
      ...
      If T_CONDICAO->E4_BVDA == "S"
         ...
         Else
            ...
            @ C(017),C(005) Say "A Condição de Pagamento utilizada nesta Pedido de Venda permite que seja emitido o Boleto Bancário de" Size C(217),C(008) COLOR CLR_BLACK PIXEL OF oDlgBol
            ...
            @ C(051),C(005) Button "Gerar Boleto(s)"             Size C(077),C(012) PIXEL OF oDlgBol ACTION(U_AUTOM636( IIF(Empty(Alltrim(M->C5_FILIAL)), cFilAnt, M->C5_FILIAL), M->C5_NUM, .F.))
