...
// ------------------------------------------------------------------------------- *
// Referencia: AUTOM262.PRW                                                        *
// Parâmetros: Nenhum                                                              *
// Tipo......: (X) Programa  ( ) Gatilho                                           *
// ------------------------------------------------------------------------------- *
// Autor.....: Harald Hans Löschenkohl                                             *
// Data......: 07/11/2014                                                          *
// Objetivo..: Programa que imprime o comprovante de entrega de mercadorias da     *
//             assistência técnica.                                                *
//**********************************************************************************

User Function AUTOM262()

   Private aFiliais  := {}
   Private cComboBx1
   Private cEtiqueta := Space(06)
   Private cMemo1	 := ""
   Private oGet1
   Private oMemo1
   ...
   Do Case
      Case cEmpAnt == "01"
           aFiliais := { "01 - Porto Alegre", "02 - Caxias do Sul", "03 - Pelotas", "04 - Suprimentos", "05 - São Paulo", "06 - Espírito Santo", "07 - Suprimentos(Novo)" }
      Case cEmpAnt == "02"
           aFiliais := { "01 - Curitiba"  }
      Case cEmpAnt == "03"
           aFiliais := { "01 - Porto Alegre"  }
   EndCase

   DEFINE MSDIALOG oDlg TITLE "Comprovante de Entrega" FROM C(178),C(181) TO C(348),C(448) PIXEL

   @ C(002),C(002) Jpeg FILE "logoautoma.bmp" Size C(126),C(026) PIXEL NOBORDER OF oDlg

   @ C(031),C(003) GET oMemo1 Var cMemo1 MEMO Size C(126),C(001) PIXEL OF oDlg

   @ C(037),C(005) Say "Filial"      Size C(012),C(008) COLOR CLR_BLACK PIXEL OF oDlg
   @ C(061),C(005) Say "Nº Etiqueta" Size C(031),C(008) COLOR CLR_BLACK PIXEL OF oDlg
   
   @ C(046),C(005) ComboBox cComboBx1 Items aFiliais  Size C(124),C(010) PIXEL OF oDlg
   @ C(070),C(005) MsGet    oGet1     Var   cEtiqueta Size C(034),C(009) COLOR CLR_BLACK Picture "@!" PIXEL OF oDlg

   @ C(067),C(052) Button "Imprimir" Size C(037),C(012) PIXEL OF oDlg ACTION( IMPCOMPENTREGA(cComboBx1, cEtiqueta))
   @ C(067),C(091) Button "Voltar"   Size C(037),C(012) PIXEL OF oDlg ACTION( oDlg:End() )

   ACTIVATE MSDIALOG oDlg CENTERED 

Return(.T.)

// Função que imprime o comprovante de entrega de mercadorias
Static Function IMPCOMPENTREGA(_xFilial, _cNumOS)
	...
	Private _aObserv   := {}
    Private cSql       := ""
    Private cNumOS     := _cNumOS
    Private cFilOS     := Substr(_xFilial,01,02) 

    If Empty(Alltrim(cFilOS))
       MsgAlert("Filial não informada. Verifique ...")
       Return(.T.)
    Endif
           
    If Empty(Alltrim(_cNumOS))
       MsgAlert("OS a ser impressa não informada. Verifique ...")
       Return(.T.)
    Endif
    ...
    cSql += " WHERE A.AB6_FILIAL   = '" + Alltrim(cFilOS) + "'" + CHR(13)
    ...
    cSql += " WHERE YP_CHAVE  = '" + Alltrim(T_CHAMADO->AB7_MEMO1) + "'"
    cSql += "   AND YP_FILIAL = ''"
    ...
    For nContar = 1 to 1
       ...
       WHILE !T_COMENTARIO->( EOF() )
          ...
          If Len(Alltrim(ctexto)) <= 40
             oPrint:Say( _nLin, 0010, Alltrim(cTexto), oFont25  )
          Else
             oPrint:Say( _nLin, 0010, Alltrim(Substr(cTexto,01,40)), oFont25  )          
             _nLin := _nLin + 50                           
             oPrint:Say( _nLin, 0010, Alltrim(Substr(cTexto,41))   , oFont25  )                       
          Endif             
          _nLin := _nLin + 50              
          T_COMENTARIO->( DbSkip() )
       ENDDO   
