...
// Data......: 10/01/2018                                                           ##
// Objetivo..: Programa que gera o boleto bancário do banco Santander pela leitura  ##
//             das tabelas do pedido de venda. Utilizado para a condição  de paga-  ##
//             mento 120 - 7 Dias.                                                  ##
// ###################################################################################
   ...
   For nContar = 1 to Len(aParcelas)
	   ...
	   If DbSeek( x__Filial + x__Pedido )
          If !MsgYesNo("Atenção!"                                              + chr(13) + chr(10) + chr(13) + chr(10) + ;
                       "O boleto para o pedido de venda " + Alltrim(x__Pedido) + " já foi impresso." + chr(13) + chr(10)                     + ;
                       "Deseja reimprimí-lo?")
		     Loop
		  Endif   
	   EndIf
       ...
       // ############################################
       // Verifica se o Banco é o Banco selecionado ##
       // ############################################
          ...
          If x_Banco == "1"
             If ZS0->ZS0_BANCO <> "341"
                MsgAlert("Atenção!"                                                                         + chr(13) + chr(10) + chr(13) + chr(10) + ;
                         ...
                         "Selecione o Banco corrteo para impressão.")
                Loop
             Endif
          Else      
             If ZS0->ZS0_BANCO <> "033"
                MsgAlert("Atenção!"                                                                         + chr(13) + chr(10) + chr(13) + chr(10) + ;
                         "O boleto deste pedido de venda foi emitido pelo banco " + Alltrim(ZS0->ZS0_BANCO) + chr(13) + chr(10) + chr(13) + chr(10) + ;
                        "Selecione o Banco correto para impressão.")
                Loop
             Endif
          Endif      
       Endif   
...
//          MsgAlert("Atenção!"                                                          + chr(13) + chr(10) + chr(13) + chr(10) + ;
//                   "Boleto bancário não será impresso devido a condição de pagamento " + kCondicao + " - "                     + ;
//                   Posicione( "SE4", 1, xFilial("SE4") + kCondicao, "E4_DESCRI" ) + " estar parametrizada para NÃO EMITIR BOLETO BANCÁRIO.")
//          Loop
//       Endif
       ...
       // ###############################################
       // Prepara o número do documento a ser impresso ##
       // ###############################################
       If x_Operacao == "I"
...
//     nValorAbat    := xSAbatimento()
	   ...
	   aDadosTit := {x_Documento                                                    ,;  // [1] Numero do titulo
	                 T_PEDIDO->EMISSAO                                              ,;  // [2] Data da emissão do título
	                 Date()                                  		                ,;  // [3] Data da emissão do boleto
	                 aParcelas[nContar,01]    				                        ,;  // [4] Data do vencimento
	                 aParcelas[nContar,02]            			                    ,;  // [5] Valor do título
	                (xNossoNum + xDvNossoNum)                                       ,;  // [6] Nosso número (Ver fórmula para calculo)
                     ...
                     x__Pedido                                                      ,;// [16] Nº do Título
                     x_Parcela                                                      ,;// [17] Nº da Parcela
                     x__Filial                                                      ,;// [18] Filial de Origem
                     1                                                              ,;// [19] Moeda do Título
                     kCliente                                                       ,;// [20] Código do Cliente
                     kLoja}                                                           // [21] Código da Loja
