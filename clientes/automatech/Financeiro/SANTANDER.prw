...
// Parâmetros: lBord - Indica por onde foi chamado o programa                       ##
//                     .T. -> Chamado pela nota fiscal (Doc. de Saída)              ##
//                     .F. -> Chamado direto pelo Menu                              ##
//             cNumNota  -> Nº do Documento a ser emitido                           ##
//             cNumSerie -> Nº da Série do Documento a ser emitido                  ##
//             cPorOnde  -> Solicitação pelo usuário                                ##
//             cParcelas -> Dados da parcela a ser impressa                         ##
//                          002580|1|03|                                            ##
//                            |    | |                                              ##
//                            |    | +---> Pacela                                   ## 
//                            |    +-----> Prefixo                                  ##
...
User Function SANTANDER(lBord, cNumNota, cNumSerie, cPorOnde, cParcelas)
...
Static Function ImprimeDup()
   ...
   Do While !EOF()
      ...
      Else
         ...
         Else
            If T_SE1->E1_PORTADO == "033"
            Else
               cMensagem := "ATENÇÃO!" + chr(13) + chr(10) + chr(13) + chr(10) 
               cMensagem += "Boleto Bancário não pertence ao Banco Santander. Verifique!"
               Exit
            Endif
         Endif
      Endif         
...
//		 If DbSeek(xFilial("SEE") + xBanco + xAgencia + xConta + "001")
		 ...
		 If DbSeek(xFilial("SE1") + T_SE1->E1_PREFIXO + T_SE1->E1_NUM + T_SE1->E1_PARCELA + T_SE1->E1_TIPO)
			...
			RecLock("SE1",.F.)
			SE1->E1_NUMBCO  := StrZero(INT(VAL(xNossoNum)),7) + Alltrim(_DigNossoN)
			MsUnLock()
      ...
      Else
         ...
         xNossoNum   := INT(VAL(Substr(T_SE1->E1_NUMBCO,01,07)))
         xDvNossoNum := INT(VAL(Substr(T_SE1->E1_NUMBCO,08,01)))
      		 
         // ##############################################
         // Calcula o digito verificado do nosso número ##
         // ##############################################
         _cNossoNum  := Strzero(INT(VAL(Substr(T_SE1->E1_NUMBCO,01,07))),12)
         _DigNossoN  := Alltrim(Str(DigNossoNumero(_cNossoNum)))
         xNossoNum   := _cNossoNum
         xDvNossoNum := _DigNossoN
      ...
      // ###############################################################
      // Atualiza o nosso número na tabela do Contas as receber (SE1) ##
      // ###############################################################
// 	  DbSelectArea("SE1")
//	  DbSetOrder(1)
//	  If DbSeek(xFilial("SE1") + T_SE1->E1_PREFIXO + T_SE1->E1_NUM + T_SE1->E1_PARCELA + T_SE1->E1_TIPO)
...
//      nCobraTaxa := IIF(T_PARAMETROS->( EOF() ), 0, T_PARAMETROS->ZZ4_DTAX)
     ...
     Do Case
        ...
        Case cEmpAnt == "02" 
             kGrupo01 := "033" + "9" + "9" + "0872" + "." + Alltrim(Str(modulo10("033990872")))
             kGrupo02 := "504" + (Substr(xNossoNum,01,07)) + "." + Alltrim(Str(modulo10("504" + Substr(xNossoNum,01,07))))
             kGrupo03 := Substr((xNossoNum + xDvNossoNum),08,06) + "0" + "101" + "." + Alltrim(Str(modulo10(Substr((xNossoNum + xDvNossoNum),08,06) + "0" + "101")))
...
//   cValorNominal := STRZERO(INT((T_SE1->E1_VALOR - nValorAbat + nCobraTaxa) * 100),10) 
     cValorNominal := Replicate("0", 10 - len(Alltrim(STR((T_SE1->E1_VALOR - nValorAbat + nCobraTaxa) * 100)))) + Alltrim(STR((T_SE1->E1_VALOR - nValorAbat + nCobraTaxa) * 100))
...
//		  oPrint:Say  (nLinha - 0066,0820,CB_RN_NN[2],oFont16n)	// Linha Digitavel do Codigo de Barras
   ...
   Do Case
      ...
      Case cEmpAnt == "02"
           cCodigoBarras := "033" + "9" + kGrupo04 + cFatorVencto + cValorNominal + "9" + "0872504" + xNossoNum + xDvNossoNum + "0" + "101"
      Case cEmpAnt == "03"
