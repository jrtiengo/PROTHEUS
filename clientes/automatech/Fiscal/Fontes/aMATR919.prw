...
User Function aMATR919()

//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
//³Define Variaveis                                                        ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
Local Titulo      := OemToAnsi("Impressao do Registro de Entradas com ISS")  //"Impressao do Registro de Entradas com ISS"
Local cDesc1      := OemToAnsi("     Este programa ira emitir o relatorio de entradas com ISS " ) //"     Este programa ira emitir o relatorio de entradas com ISS "
Local cDesc2      := OemToAnsi("de acordo com os parametros configurados pelo usuario")  //"de acordo com os parametros configurados pelo usuario"
...
#IFDEF TOP
    ...
    If TcSrvType()<>"AS/400"
		...
		cQuery    += "F3_EMINFE >= '" + Dtos(mv_par01) + "' AND "
		cQuery    += "F3_EMINFE <= '" + Dtos(mv_par02) + "' AND "
		...
		EndIf
	   	//cQuery    += "F3_OBSERV  LIKE '%CANCELAD%' AND "
		cQuery    += "SF3.D_E_L_E_T_ = ' ' "               
		cQuery    += "ORDER BY F3_NFELETR "
...
While (cAliasSF3)->(!Eof())                  
	...
	While (cAliasSF3)->(!Eof()) .And. Month((cAliasSF3)->F3_ENTRADA) == nMes
        ...
        _nValnota := (cAliasSF3)->F3_VALCONT
        _cCancela :=  ""
        If Alltrim((cAliasSF3)->F3_OBSERV) == "NF CANCELADA"
           	nValDed := 0
        	nValLiq := 0
           _nValnota := 0  
           _cCancela :=  "CANCELADA"
		...
		aDetail := {StrZero(Day((cAliasSF3)->F3_ENTRADA),2),;
					"2015/"+(cAliasSF3)->F3_NFELETR,;				
                    		 	_cCancela,;
					TransForm(_nValnota,"@e 99,999,999.99"),;
