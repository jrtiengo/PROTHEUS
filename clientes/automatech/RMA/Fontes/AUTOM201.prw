...
User Function AUTOM201()
   ...
   If Alltrim(T_PARAMETROS->ZZ4_NRMA1) + ;
      ...
      MsgAlert("Atenção! Não foram indicados nenhum aprovador de RMA no Sistema. Verifique parametrizador.")
      Return(.T.)
   Endif

   If T_PARAMETROS->ZZ4_VRMA == 0
      MsgAlert("Atenção! Qtd de dias para cálculo da Data de Validade da RMA não parametrizado. Verifique parâmetros.")
      Return(.T.)
   Else
      nValidade := T_PARAMETROS->ZZ4_VRMA
   Endif
...
Static Function AbreAprova(_RMA, _ANO)
   ...
   cMotivo1   := T_DADOS->ZS4_CMOT
   cMotivo2   := T_DADOS->ZS4_CMTA   
   ...
   For nContar = 1 to Len(aMotivo)
       If Substr(aMotivo[nContar],01,06) == cMotivo1
   ...
   For nContar = 1 to Len(aMotivoA)
       If Substr(aMotivoA[nContar],01,06) == cMotivo2
   ...
   @ C(192),C(125) Say "Status"                      Size C(018),C(008) COLOR CLR_BLACK PIXEL OF oDlg
   ...
   @ C(190),C(144) ComboBox cComboBx7 Items aMotivoA         Size C(104),C(010) PIXEL OF oDlg
   @ C(203),C(124) GET   oMemo4       Var cConsideracao MEMO Size C(123),C(021) PIXEL OF oDlg
   ...
   @ C(189),C(251) Button "Aprova"  Size C(037),C(008) PIXEL OF oDlg ACTION( APRREPRMA("A", cNRMA, cARMA, aProvador[1], cDataP, cHoraP, cConsideracao, cEmailVend) )
   @ C(198),C(251) Button "Revisar" Size C(037),C(008) PIXEL OF oDlg ACTION( APRREPRMA("V", cNRMA, cARMA, aProvador[1], cDataP, cHoraP, cConsideracao, cEmailVend) )
   @ C(207),C(251) Button "Reprova" Size C(037),C(008) PIXEL OF oDlg ACTION( APRREPRMA("R", cNRMA, cARMA, aProvador[1], cDataP, cHoraP, cConsideracao, cEmailVend) )
...
Static Function APRREPRMA(__Tipo, __RMA, __Ano, __Aprovador, __DataLib, __HoraLib, __Observa, _EmailVend)
   ...
   If Substr(cComboBx7,01,06) == "000000"
      MsgAlert("Status da Liberação não informado. Verifique!")
      Return(.T.)
   Endif
   ...
   If MsgYesNo("Confirma a " + IIF(__Tipo == "A", "Aprovação da RMA?", "Reprovação da RMA?"))
      ...
      cSql += "   ZS4_CMTA = '" + Substr(cComboBx7,01,06) + "', "
      cSql += "   ZS4_DVAL =  " + Alltrim(Str(nValidade)) + ",  "
      cSql += "   ZS4_VALI = '" + Strzero(year(__dValidade),4) + Strzero(month(__dValidade),2) + Strzero(day(__dValidade),2) + "', "      
      cSql += "   ZS4_IMPR = ''"
      ...
      If !Empty(Alltrim(_EmailVend))
         ...
         Do Case
            Case __Tipo == "A"
                 cTexto += "Sua RMA foi APROVADA." + chr(13) + chr(10) + chr(13) + chr(10)
            Case __Tipo == "V"
                 cTexto += "Sua RMA está sendo retornada para sua REVISÃO." + chr(13) + chr(10) + chr(13) + chr(10)
            Case __Tipo == "R"     	         
                 cTexto += "Sua RMA foi REPROVADA." + chr(13) + chr(10) + chr(13) + chr(10)
      	 EndCase
         ...
         cTexto += "Descrição da Análise:" + chr(13) + chr(10)
         cTexto += Alltrim(__Observa) + chr(13) + chr(10) + chr(13) + chr(10)
         cTexto += "Att."  + chr(13)  + chr(10) + chr(13) + chr(10)
         cTexto += "Liberador(s) de RMA"

         U_AUTOMR20(cTexto, Alltrim(_EmailVend), "", "Análise de RMA" )   
