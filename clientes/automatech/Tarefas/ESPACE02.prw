...
// ------------------------------------------------------------------------------- *
// Referencia: ESPACE02.PRW                                                        *
// Parâmetros: Nenhum                                                              *
// Tipo......: (X) Programa  ( ) Gatilho                                           *
// ------------------------------------------------------------------------------- *
// Autor.....: Harald Hans Löschenkohl                                             *
// Data......: 13/09/2013                                                          *
// Objetivo..: Programa que realiza a Aprovação/Reprovação de Acessos              *
//**********************************************************************************

User Function ESPACE02()
   ...
   DEFINE MSDIALOG oDlg TITLE "Aprovação/Reprovação de Acessos" FROM C(178),C(181) TO C(595),C(882) PIXEL

   @ C(192),C(221) Button "Aprovar/Reprovar Solicitação" Size C(085),C(012) PIXEL OF oDlg ACTION( LibAcesso(aBrowse[oBrowse:nAt,02]) )
...
Static Function LibAcesso(_Codigo)
   ...
   // Aprovação
   @ C(019),C(173) CheckBox oCheckBox1  Var lAProvado  Prompt "Aprovado"     Size C(034),C(008) PIXEL OF oDlgL
   @ C(031),C(173) CheckBox oCheckBox2  Var lReprovado Prompt "Reprovado"    Size C(037),C(008) PIXEL OF oDlgL
   ...
   @ C(083),C(173) CheckBox oCheckBox3  Var lCheckBox3 Prompt "Pesquisar"    Size C(034),C(008) PIXEL OF oDlgL
   @ C(094),C(173) CheckBox oCheckBox4  Var lCheckBox4 Prompt "Visualizar"   Size C(033),C(008) PIXEL OF oDlgL
   @ C(104),C(173) CheckBox oCheckBox5  Var lCheckBox5 Prompt "Incluir"      Size C(027),C(007) PIXEL OF oDlgL
   @ C(114),C(173) CheckBox oCheckBox6  Var lCheckBox6 Prompt "Alterar"      Size C(028),C(008) PIXEL OF oDlgL
   @ C(124),C(173) CheckBox oCheckBox7  Var lCheckBox7 Prompt "Excluir"      Size C(028),C(008) PIXEL OF oDlgL
// @ C(083),C(226) CheckBox oCheckBox8  Var lCheckBox8 Prompt "Copiar"       Size C(026),C(008) PIXEL OF oDlgL
// @ C(094),C(226) CheckBox oCheckBox9  Var lCheckBox9 Prompt "Conhecimento" Size C(048),C(008) PIXEL OF oDlgL
// @ C(104),C(226) CheckBox oCheckBox10 Var lCheckBox10 Prompt "?"           Size C(042),C(008) PIXEL OF oDlgL
// @ C(114),C(225) CheckBox oCheckBox11 Var lCheckBox11 Prompt "Facilitador" Size C(037),C(008) PIXEL OF oDlgL
// @ C(124),C(226) CheckBox oCheckBox12 Var lCheckBox12 Prompt "?"           Size C(038),C(008) PIXEL OF oDlgL
   @ C(145),C(173) GET      oMemo3      Var cMemo3 MEMO                      Size C(112),C(059) PIXEL OF oDlgL

   @ C(206),C(208) Button "Salvar" Size C(037),C(012) PIXEL OF oDlgL ACTION( SalvaAprovacao() )
   @ C(206),C(248) Button "Voltar" Size C(037),C(012) PIXEL OF oDlgL ACTION( oDlgL:End() )
...
Static Function SalvaAprovacao()

   Local cSql    := ""
   Local cEmail  := ""
   Local c_email := ""

   If lAprovado == .F. .And. lReprovado == .F.
      MsgAlert("Necessário indicar Acesso Aprovado ou Reprovado.")
      Return .T.
   Endif
   
   If lAprovado == .T. .And. lReprovado == .T.
      MsgAlert("Indique apenas Aprovado ou Reprovado.")
      Return .T.
   Endif
   ...
   If DbSeek(xfilial("ZS3") + cCodigo)
      ...
      ZS3_APRO := IIF(lAprovado  == .T., '1', '0')
      ZS3_REPR := IIF(lReprovado == .T., '1', '0')
      ZS3_DAPR := cDataAnal
      ZS3_QUEM := cAprovador
      ZS3_LB01 := IIF(lCheckBox3  == .T., '1', '0')
      ZS3_LB02 := IIF(lCheckBox4  == .T., '1', '0')
      ZS3_LB03 := IIF(lCheckBox5  == .T., '1', '0')
      ZS3_LB04 := IIF(lCheckBox6  == .T., '1', '0')
      ZS3_LB05 := IIF(lCheckBox7  == .T., '1', '0')
      ZS3_LB06 := IIF(lCheckBox8  == .T., '1', '0')
      ZS3_LB07 := IIF(lCheckBox9  == .T., '1', '0')
      ZS3_LB08 := IIF(lCheckBox10 == .T., '1', '0')
      ZS3_LB09 := IIF(lCheckBox11 == .T., '1', '0')
      ZS3_LB10 := IIF(lCheckBox12 == .T., '1', '0')
      ZS3_OBSE := cMemo3
      MsUnLock()              

      // Envia e-mail ao usuário solicitante lhe informando se a solicitação de acesso foi aprovada ou reprovada.
      ...
      If lAprovado == .T.
         cEmail += "Sua solicitação de Liberação de Acesso ao Menu do Sistema Protheus foi APROVADA."   + chr(13) + chr(10)
         cEmail += "Você será avisado(a), via e-mail, quando este acesso estiver liberado em produção." + chr(13) + chr(10) + chr(13) + chr(10)
         cEmail += "Dados da solicitação:" + chr(13) + chr(10) + chr(13) + chr(10)
      Endif
         
      If lReprovado == .T.
         cEmail += "Sua solicitação de Liberação de Acesso ao Menu do Sistema Protheus foi REPORVADA." + chr(13) + chr(10) + chr(13) + chr(10)
         cEmail += "Dados da solicitação:" + chr(13) + chr(10) + chr(13) + chr(10)
      Endif
      ...
      cSql += " WHERE UPPER(A.ZZA_NOME) = '" + Alltrim(T_ACESSOS->ZS3_SOLI) + "'"
      ...
      If Empty(c_email)
      Else   
         U_AUTOMR20(cEmail ,c_email, "", "Aprovação/Reprovação de Acesso do Menu Sistema Protheus." )
      Endif

      // Envia e-mail ao liberador de acesso na produção
      cEmail := ""
      cEmail := "Prezado(a) Liberador(a):" + chr(13) + chr(10) + chr(13) + chr(10)
      cEmail += "Existe(m) Acessos de menu do Sistema Protheus pendentes a serem aplicados em produção." + chr(13) + chr(10)
      cEmail += "Favor verificar Sistema de Tarefas, Acessos Liberados."                                 + chr(13) + chr(10) + chr(13) + chr(10)
      cEmail += "Att." + chr(13)                  + chr(13) + chr(10)
      cEmail += "Sistema de Controle de Tarefas"  + chr(13) + chr(10)
      ...
      If T_MASTER->( EOF() )
         c_email := ""
      Else
         c_email := T_MASTER->ZZJ_APRO
      Endif
         
      // Envia e-mail ao Aprovador
      If Empty(c_email)
      Else   
         U_AUTOMR20(cEmail ,c_email, "", "Aviso de Liebração de Acesso de Menu Sistema Protheus." )
      Endif
