...
#DEFINE MAXITEMC   047  // REINER ERA 18 PASSEI P/50                  // Máxima de caracteres por linha de produtos/serviços
#DEFINE MAXMENLIN  080  // REINER ERA 080 PASSEI P/110                // Máximo de caracteres por linha de dados adicionais
#DEFINE MAXMSG     013                                                // Máximo de dados adicionais por página
#DEFINE MAXVALORC  010  //FABIANO ERA 008 PASSEI P/010                // Máximo de caracteres por linha de valores numéricos
				...
				For nX := 1 To nLenNotas
					If !Empty(aXML[nX][2])
						...
						Else
							...
							cCodAutDPEC := ""
							cCodRetNFE	:= ""
						EndIf                                                                             // Reiner - da versao anterior da Danfe --> "If (!Empty(cAutoriza) .Or. !Empty(cCodAutDPEC) .Or. !cModalidade$"1,3,4,5,6")"
						//If (!Empty(cAutoriza) .Or. !Empty(cCodAutDPEC) .Or. Alltrim(aXML[nX][8]) $ "2,5,7") //Reiner - era assim
						If (!Empty(cAutoriza) .Or. !Empty(cCodAutDPEC) .Or. Alltrim(aXML[nX][8]) $ "2,1,3,4,5,6,7") // Reiner - inclui mais estas opcoes - 1,3,4,5,6	
							If aNotas[nX][02]=="E"
								DBClearFilter()
								dbSelectArea("SF1")
								dbSetOrder(1)
...
For nX := 1 To nForTo
	...
	nLinCalc+=10
Next nX 
/*oDanfe:Say(nLinCalc,098,SubStr(cStrAux,1,23),oFont12N:oFont)
nLinCalc+=10
oDanfe:Say(nLinCalc,098,SubStr(cStrAux,25),oFont12N:oFont)
nLinCalc+=10*/
...
If nfolha==1
	If lMv_Logod
		//oDanfe:SayBitmap(045,005,cLogoD,090,090)  //FABIANO
	Else
		//oDanfe:SayBitmap(045,005,cLogo,090,090)   //FABIANO
	EndIF
Endif
