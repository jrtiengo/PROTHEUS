...
If cTipo == "1"
	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
			...
			Else
				...
				If !Empty(SF2->F2_DUPL)	
					...
					While !Eof() .And. xFilial("SE1") == (cAliasSE1)->E1_FILIAL .And.;
						...
						SF2->F2_DOC == (cAliasSE1)->E1_NUM     
							If (cAliasSE1)->E1_TIPO = MVNOTAFIS .OR. (cAliasSE1)->E1_TIPO = 'DP' .OR. ((Alltrim((cAliasSE1)->E1_ORIGEM) $ 'LOJA701|FATA701|LOJA010') .AND. (cAliasSE1)->E1_TIPO $ cWhere)
								...
								If cValLiqB == "1" // 1 Baixa - 2 Emissao
				    ...
				    EndIf
				Else  // REINER
				      		// INCLUI A OPÇÃO ELSE P/CONTAS A PAGAR (SE2)
				    dbSelectArea("SE2")
					dbSetOrder(1)	
					#IFDEF TOP
						lQuery  := .T.
						...
						BeginSql Alias cAliasSE2
							...
							SE2.E2_PREFIXO = %Exp:SF2->F2_PREFIXO% AND 
							SE2.E2_NUM = %Exp:SF2->F2_DUPL% AND 
							((SE2.E2_TIPO = %Exp:MVNOTAFIS%) OR
							 ((SE2.E2_ORIGEM = 'LOJA701' OR SE2.E2_ORIGEM = 'FATA701') AND SE2.E2_TIPO IN (%Exp:cWhere%))) AND
							SE2.%NotDel%
							ORDER BY %Order:SE2%
						EndSql
						
					#ELSE
						MsSeek(xFilial("SE2")+SF2->F2_PREFIXO+SF2->F2_DOC)
					#ENDIF
					While !Eof() .And. xFilial("SE2") == (cAliasSE2)->E2_FILIAL .And.;
						SF2->F2_PREFIXO == (cAliasSE2)->E2_PREFIXO .And.;
						SF2->F2_DOC == (cAliasSE2)->E2_NUM
						If 	(cAliasSE2)->E2_TIPO = MVNOTAFIS .OR. ((cAliasSE2)->E2_ORIGEM = 'LOJA701' .AND. (cAliasSE2)->E2_TIPO $ cWhere)
							If cValLiqB == "1"
								aadd(aDupl,{(cAliasSE2)->E2_PREFIXO+(cAliasSE2)->E2_NUM+(cAliasSE2)->E2_PARCELA,(cAliasSE2)->E2_VENCORI;
								,((cAliasSE2)->E2_VALOR-(cAliasSE2)->E2_PIS-(cAliasSE2)->E2_COFINS-(cAliasSE2)->E2_CSLL-(cAliasSE2)->E2_INSS)})					
							Else
		...
		//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
						...
						// INICIO REINER  -- DESCRICAO DO PRODUTO
	            If empty(MemoLine(SC6->C6_DESCR,50,1,1,.T.))
	               IF Empty(SC6->C6_DESCRI)
	                  cDescXML := ALLTRIM(SB1->B1_DESC)
	               Else   
		              cDescXML := ALLTRIM(SC6->C6_DESCRI)
		           EndIf   
		        Else   
		           cDescXML := RTRIM(MemoLine(SC6->C6_DESCR,50,1,1,.T.))+" " // 50 Maior Linha
		           For j:=2 To 3
		               if empty(MemoLine(SC6->C6_DESCR,50,j,1,.T.)) 
		                  j := 4
		               Else       
		                  cDescXML += RTRIM(MemoLine(SC6->C6_DESCR,50,J,1,.T.))+" " 
		               EndIf
		           Next j
		        EndIf   
			    ...
			    If dbSeek(xFilial("SA7")+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA)
			       If !Empty(A7_CODCLI)
			          cProdSB1 := AllTrim(A7_CODCLI)    
			       Else
			          cProdSB1 := (cAliasSD2)->D2_COD   
			       EndIf   
			       If !Empty(A7_DESCCLI)
			          cDescXML := AllTRIM(A7_DESCCLI)
			       EndIf
			    Else
			       cProdSB1 := (cAliasSD2)->D2_COD  
			    EndIf 
