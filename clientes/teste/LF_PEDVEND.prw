...
User Function LF_PEDVEND()
	...
	//    MsgAlert("Excel n√£o instalado", "# Aten√ß√£o")
	//EndIf
...
Static Function ProcessI(Cabec1,Cabec2,Titulo,nLin)
	...
	if (MV_PAR10 == 1)
		Cabec2 := ""
		Cabec1 := "Filial  Pedido  Tipo  Emiss„o      Cliente                               Vendedor                      Faturado  Pedido MKP                     Vlr. ICMS       Vlr. PIS     Vlr. COF.     Vlr. IPI        Vlr. Pedido  "
		...
		Cabec1 := ">Filial  Pedido  Tipo  Emiss„o      Cliente                               Vendedor                     Faturado  Pedido MKP "
	...
	Do While TMPPV->(!EOF())
		...
		if (MV_PAR10 == 1)
			...
			@nLin,105    PSAY iif(Empty(TMPPV->C5_NOTA),"N„o","Sim")
			...
			for _n := 1 to len(_aItSC6)
				...
				if (_cPedido <> TMPPV->C5_NUM)
					...
					@nLin,104    PSAY iif(Empty(TMPPV->C5_NOTA),"N„o","Sim")
					@nLin,114    PSAY Alltrim(TMPPV->C5_PEDMKP)
					nLin++
				endif
...
Static Function CalcImp(_cNum) 
	...
	/*cQuery := " SELECT SC6.C6_ITEM,SC6.C6_PRODUTO,SC6.C6_SERIORI,SC6.C6_CLI,SC6.C6_LOJA,SC6.C6_TES,SC6.C6_QTDVEN,SC6.C6_PRCVEN,SC6.C6_VALDESC,SC5.C5_NUM,SC5.C5_ACRSFIN,SC6.C6_NUM,SC5.C5_CLIENTE,SC5.C5_LOJACLI,SC5.C5_TIPOCLI,"
	cQuery += " SC6.C6_ITEM,SC6.C6_VALOR,SC6.C6_NFORI,SC6.C6_PRUNIT,SC6.C6_CF,SC6.C6_ITEMORI,SC6.C6_IPIDEV,SC6.C6_PICMRET,SC6.C6_ABATMAT,SC6.C6_ABATINS,SC6.C6_ABATISS,SC6.C6_CLASFIS,SC6.C6_ABSCINS,SC6.C6_TES , SC5.C5_TIPO"
	cQuery += " FROM SC5010 SC5 , SC6010 SC6 "
	cQuery += " WHERE SC6.C6_FILIAL = '"+ cFilAnt+"' "
	cQuery += " AND   SC5.C5_FILIAL = '"+ cFilAnt +"' "
	cQuery += " AND SC5.C5_NUM = '"+ Alltrim(_cNum) +"' "
	cQuery += " AND SC6.C6_NUM = SC5.C5_NUM "
	//cQuery += " AND SC6.C6_ITEM = '01' "
	//cQuery += " AND SC6.C6_NOTA = '' "
	cQuery += " AND SC6.D_E_L_E_T_ = ' ' AND SC6.C6_FILIAL = SC5.C5_FILIAL "
	...
	TcQuery cQuery New Alias "TMPCALC"
	*/
	...
	Do While ! SC6->(EoF()) .and. SC6->C6_NUM == TMPPV->C5_NUM .and. SC6->C6_FILIAL == cFilAnt
		...
		if (!_lCalcCab)
			...
			MaFisIni(TMPPV->C5_CLIENTE,TMPPV->C5_LOJACLI,If(TMPPV->C5_TIPO$'DB',"F","C"),TMPPV->C5_TIPO,TMPPV->C5_TIPOCLI,MaFisRelImp("",{"SC5","SC6"}),,,"SB1","MTR700") //MaFisRelImp("MT100", {"SF2", "SD2"}
			_lCalcCab := .F.
		endif
		nPos := 0
		cNfOri     := Nil
