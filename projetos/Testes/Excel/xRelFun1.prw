...
Static Function ImpRel(lSchedule)
	...
	If (cAlias)->(EoF())
		If lSchedule
			Conout("Nenhuma Matricula foi encontrada!")
		Else
	...
	oFWMsExcel:AddColumn("Funcionários", "Dados Funcionários", "SALARIO", 1, 1)
	...
	While (cAlias)->(!Eof())
		...
		If lSchedule
			Conout("Processando registros " + cValToChar(nConta) + " de " + cValToChar(nTotal) + "...")
		Else
		...
		oFWMsExcel:AddRow("Funcionários", "Dados Funcionários", {;
			...
			Transform((cAlias)->RA_SALARIO, "@E 9999999.99") ;
			})
	...
	Else
		...
		cAssunto := 'Lista de funcionários Geral'
		cCorpo 	 := u_xBodyMail('Geral')
		aAnexos  := {}
